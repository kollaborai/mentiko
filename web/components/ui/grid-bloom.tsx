...
export default function GridBloom({
  ...
  // r3f's <Canvas> misreads its size (stays at the 300x150 default) when it mounts
  // inside an absolutely-positioned banner that finishes layout after mount, and its
  // initial measure never self-corrects here — only a window 'resize' re-measures.
  // Fire a short burst after mount so at least one lands once r3f's listener is attached.
  // ponytail: resize burst; drop it if a future r3f fixes initial measure in this setup.
  useEffect(() => {
    const timers = [0, 120, 300, 600].map((ms) =>
