...
// Build a decision "guided options" run in the exact shape that jammed devv:
// a valid decision-result.json on disk, EMPTY run metadata (identity only
// recoverable from run.goal), and a dead agent session. `withBlockedSummary`
// toggles the two real completion routes:
//   true  -> a fresh `${agent}-summary.json` exists, so the handoff salvage
//            synthesizes the declared completion event (the agent-died variant).
//   false -> no summary, so completion falls through to the decision-terminal
//            backstop that accepts the run from decision-result.json directly.
// Both must reach `completed` and fire the server-side import.
function setupDecisionRun(opts: { withBlockedSummary: boolean }) {
  ...
  return { runDir, runJsonPath, chainPath, eventsDir, stateDir };
}

function runCompletion(fixture: ReturnType<typeof setupDecisionRun>) {
...
// the identity-gated import path also early-returned. Completion must own delivery:
// it completes the run from the artifact and fires the server-side import (identity
// recovered from run.goal), regardless of the agent's CLI import.
describe("runner-v2 decision import: server owns delivery when the agent CLI import fails", () => {
  ...
  it("completes despite a blocked summary and empty run metadata (handoff-salvage route, identity from goal)", async () => {
    const fixture = setupDecisionRun({ withBlockedSummary: true });
    runCompletion(fixture);
    ...
    expect(readRunJson(fixture.runJsonPath)).toMatchObject({
      status: "completed",
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining(`/api/decisions/${DECISION_ID}/import`),
  ...
  it("completes from decision-result.json alone via the decision-terminal backstop (no summary to salvage)", async () => {
    const fixture = setupDecisionRun({ withBlockedSummary: false });
    runCompletion(fixture);

    // With no summary artifact the handoff salvage cannot fire, so completion
    // reaches the decision-terminal branch: a run-scoped decision-result.json IS
    // the deliverable, accepted directly. This is the backstop that keeps the run
    // completing even when the agent leaves nothing but its real output on disk.
    expect(readRunJson(fixture.runJsonPath)).toMatchObject({
      status: "completed",
      agents: [{ id: "decision-option-strategist", status: "complete" }],
    });
    expect(readRunnerV2AttemptState(fixture.runJsonPath).attempts.at(-1)).toMatchObject({
      agentId: "decision-option-strategist",
      terminalReason: "completed_from_decision_artifact",
    });
    // Same server-owned delivery, reached through the other route.
    expect(fetchMock).toHaveBeenCalledWith(
      expect.stringContaining(`/api/decisions/${DECISION_ID}/import`),
      expect.objectContaining({ method: "POST" }),
    );
