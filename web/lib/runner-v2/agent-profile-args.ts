...
/**
 * `--dangerously-skip-permissions` is not a flag the claude CLI accepts on its
 * own; it expands to the pair below. Expansion produces an argv fragment, which
 * `splitProfileArgumentString` then tokenizes — the combined string must never
 * reach the CLI as one argument.
 */
