...
function normalizedAgentForRegistry(agent: AgentDefinition, id: string): AgentDefinition {
  const taskToolNormalized = normalizeMcpTaskToolDeclarations(agent);
  const authorities = normalizeAgentAuthorities(taskToolNormalized.authorities);
  return {
    ...taskToolNormalized,
    id,
...
export function extractInlineAgents(
  ...
  agents.forEach((entry, index) => {
    ...
    const agentDef: AgentDefinition = {
      ...
      deliverable: agent.deliverable as string | undefined,
      verification: agent.verification as string | undefined,
      final_verifier: agent.final_verifier as boolean | undefined,
      verifies_acceptance_criteria: agent.verifies_acceptance_criteria as boolean | undefined,
      success_assertion: agent.success_assertion as string | undefined,
