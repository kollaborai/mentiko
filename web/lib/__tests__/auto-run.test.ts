...
describe("getAutoRunCandidates", () => {
  ...
  it("releases an old retry scope only after the exact scoped terminal run is verified, then re-admits the retry", () => {
    const retryMetadata = {
      auto_run: true,
      chain_id: "release-review",
    ...
    expect(mockTaskUpdate).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        metadata: expect.objectContaining({
          ...
          retry_source_run_id: "run-legacy-retry",
    ...
    const { task_run_scope: _releasedScope, ...repairedMetadata } = retryMetadata;
    const repaired = {
      id: "TASK-LEGACY-RETRY",
      title: "Legacy retry scope",
      status: "open",
