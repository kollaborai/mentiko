...
import { signUp, signIn, isMockOAuth, authClient } from "@/lib/auth/auth-client";
...
function SignUpForm() {
  ...
  // extract invite token from redirect URL if present (e.g. /invite/abc123)
  const inviteToken = redirectTo.startsWith("/invite/")
    ? redirectTo.replace("/invite/", "")
    : null;
  ...
  // if coming from invite link, pre-fill email from invite
  useEffect(() => {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (loginResult.error) {
          const loginUrl = `/login?created=1&email=${encodeURIComponent(email)}&redirect=${encodeURIComponent(redirectTo)}`;
  ...
  const signupBlocked = providers && providers.publicEmailSignup === false && !inviteToken;

  if (!providers) {
    return (
      <div className="w-full max-w-md text-center text-xs text-muted-foreground py-12">
        Loading…
      </div>
    );
  }
  ...
  if (signupBlocked) {
    return (
      <div className="w-full max-w-md">
        <div className="bg-card rounded-md p-6 space-y-4 text-center">
          <h1 className="text-base font-semibold tracking-tight">Invitation required</h1>
          <p className="text-xs text-muted-foreground leading-relaxed">
            This Mentiko instance does not allow open registration. Ask an organization owner or admin to send
            you an invite link, then open it and create your account from there.
          </p>
          <Link
            href={loginHref}
      ...
      <div className="bg-card rounded-md p-6">
        <div className="text-center mb-5">
          ...
          <h1 className="text-base font-semibold tracking-tight">
            {inviteOrgName
              ? `join ${inviteOrgName}`
              : "Create your Mentiko account"}
          </h1>
          <p className="mt-1 text-xs text-foreground/50">
            {inviteOrgName
              ? "Use the email from your invite to join this workspace."
              : "Fresh install? Start here. The first account becomes the workspace owner."}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <input
            ...
            className={`w-full bg-background rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 placeholder:text-muted-foreground ${
              emailReadOnly ? "opacity-60 cursor-not-allowed" : "focus:ring-primary"
            }`}
          ...
          <p className="text-[10px] text-muted-foreground leading-relaxed">
            Passwords must be at least 12 characters. This account belongs to
            this Mentiko instance; it is separate from your AI CLI logins.
          </p>
        ...
        {providers.publicEmailSignup !== false &&
