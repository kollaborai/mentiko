...
function ProfileCard({ profile }: { profile: AgentProfile }) {
  ...
  const metaParts = [];
  if (profile.duration_ms !== undefined) metaParts.push(formatDuration(profile.duration_ms));
            ...
            {profile.duration_ms !== undefined && (
              <div>
                ...
                <p className="text-xs font-mono text-foreground/60">
                  {formatDuration(profile.duration_ms)}
                </p>
              </div>
            )}
          </div>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-foreground/50">
            agent performance metrics and resource usage
          </p>
        </div>
        <Button
          size="sm"
