...
monitor-{sessionName}               // per-agent completion monitor session
...
4. session destroyed on completion`}</CodeBlock>
      ...
      <section className="mb-6">
        ...
        <p className="text-xs text-foreground/60 leading-relaxed mb-3">
          ...
          marks orphaned runs stopped, and closes completed auto-run tasks. The TypeScript background-worker watchdog cleans only sessions proven safe after a fresh PTY liveness check.
        </p>
        <div className="bg-card rounded-md p-3 text-xs text-foreground/60 space-y-1">
          ...
          <div>Watchdog removes monitor-* sessions only when terminal-run ownership and fresh PTY state prove cleanup safe</div>
          <div>Runs every 60 seconds in the supervised background worker process</div>
        </div>
      </section>
