...
describe("POST /api/mentiko-mcp/ops/decisions", () => {
  ...
  test("returns classic decision state instead of requiring guided flow", async () => {
    (getDecision as jest.Mock).mockReturnValue({
      id: "decision-classic",
      status: "briefed",
      prompt: "classic decision",
    ...
    const req = {
      url: "http://localhost:3000/api/mentiko-mcp/ops/decisions?id=decision-classic",
      headers: new Headers({ authorization: "Bearer test-token" }),
      nextUrl: new URL("http://localhost:3000/api/mentiko-mcp/ops/decisions?id=decision-classic"),
    } as unknown as Parameters<typeof GET>[0];
    const res = await GET(req);
