...
NEVER hardcode paths. NEVER use process.cwd() or __dirname for data.
...
AI generation/job-runner workspace context:
  when a UI surface or API route starts an AI generation job, preserve the
  selected workspace all the way through the stack. Do not "fix" model/CLI
  errors by changing runner flags if the real problem is missing workspace
  context.

  required checks before editing generation flows:
  - UI component/page passes the current workspacePath from useWorkspace()
    in the request body when the generated output should be codebase-aware.
  - API route accepts workspacePath, calls resolveAuthorizedWorkspacePath(),
    adds WORKSPACE_CONTEXT to the prompt, and stores the authorized path in
    createJob(..., { ..., workspacePath: authorizedWorkspacePath }).
  - launchJobRunner() receives the request namespace/org and gives the
    detached runner request-scoped MENTIKO_NAMESPACE_ROOT / MENTIKO_ORG_ROOT.
  - job-runner.mjs resolves job.input.workspacePath/JOB_WORKSPACE_CWD to a
    registered workspace and logs activity "cwd <repo-path>" before spawn.
  - if a CLI complains about trust/git state, first verify the job cwd is the
    intended repo. Only add bypass/skip flags after confirming workspace
    propagation is correct.

  audit pattern:
    rg -n "createJob\\(|launchJobRunner\\(|/api/.*/generate|workspacePath" web/app web/components web/lib lib
  For every createJob/launchJobRunner path that is meant to use project
  context, prove where workspacePath enters, where it is authorized, and where
  it becomes the child process cwd.
...
linode CLI:
  - installed: linode-cli v5.65.1 (pip3)
  - auth: LINODE_CLI_TOKEN env var (configured via `linode-cli configure`)
  - usage: `linode-cli linodes list`, `linode-cli domains list`, etc.

platform build compatibility (IMPORTANT):
  when developing in this repo, remember the platform gets built standalone
  and runs inside a container. things that will break the tenant image:
  - importing node_modules that aren't in web/package.json (standalone strips)
  - referencing filesystem paths outside /opt/mentiko/ at runtime
  - server components that depend on dev-only env vars
  - ws-terminal.ts must be esbuild-compatible (no imports that cant bundle)
  - process-manager.ts must compile with tsc (commonjs, no fancy imports)
  - bin/ and lib/ bash scripts must work on debian slim (not macOS-specific)
  - md5 command is macOS-only; use _md5() wrapper (md5 -q || md5sum)
  - BETTER_AUTH_SECRET must be set in production (code throws without it)
