#!/usr/bin/env node
// GENERATED FROM web/lib/runner-v2/monitor-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
function selectTranscriptFromCapture(capture, resolve5, identity = {}) {
  ...
  const candidates = uuids.flatMap((uuid, position) => {
    const path2 = resolve5(uuid);
    if (!path2) return [];
    const score = scoreTranscriptIdentity(path2, uuid, identity);
    return score === null ? [] : [{ path: path2, position, score }];
...
async function main() {
  ...
  const runDir = process.env.MENTIKO_RUN_DIR || (process.env.RUNS_DIR ? (0, import_node_path5.join)(process.env.RUNS_DIR, runId) : "");
  if (!runDir) {
    console.error("monitor-v2 unsupported: missing MENTIKO_RUN_DIR/RUNS_DIR");
    process.exit(64);
  }
  const runJsonPath = (0, import_node_path5.join)(runDir, "run.json");
  let run;
  try {
  ...
  const interval = positiveInt2(intervalArg, MONITOR_DEFAULTS.checkIntervalSec);
  const maxStaleCount = positiveInt2(maxStaleArg, MONITOR_DEFAULTS.maxStaleCount);
  const io = createLiveMonitorIO({
    ...
    eventsDir: process.env.EVENTS_DIR || config_default.eventsDir,
    stateDir: process.env.STATE_DIR || (0, import_node_path5.join)(runDir, "state"),
  ...
  const result = await runChainMonitor(sessionName, io, {
    maxStaleCount,
    workspaceType: process.env.WORKSPACE_TYPE || "local",
    advisorStaleThreshold: positiveInt2(process.env.MENTIKO_ADVISOR_STALE_COUNT, MONITOR_DEFAULTS.advisorStaleThreshold),
    maxTotalNudges: positiveInt2(process.env.MENTIKO_MONITOR_MAX_NUDGES, MONITOR_DEFAULTS.maxTotalNudges)
  }, interval);
  console.log(JSON.stringify({ status: "handled", reason: result.reason, ticks: result.ticks }));
}
function resolveAgentIdFromRun(run, sessionName) {
...
main().catch((error) => {
  console.error(`monitor-v2 crashed without fallback: ${error instanceof Error ? error.message : String(error)}`);
  process.exit(64);
});
