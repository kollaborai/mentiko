...
function selectTranscriptFromCapture(capture, resolve5, identity = {}, findByInstructionPath) {
...
async function sleepBefore(delayMs, deadline) {
  ...
  await new Promise((resolve5) => setTimeout(resolve5, Math.min(delayMs, remainingMs)));
...
var SHARED_TMP_DIR = (0, import_fs6.realpathSync)("/tmp");
...
function createCompletionLaunchContext(input) {
  ...
  const dir = (0, import_fs6.mkdtempSync)((0, import_path4.join)(SHARED_TMP_DIR, CONTEXT_DIR_PREFIX));
...
function isCompletionLaunchContextPath(path2) {
  ...
  return [REAL_TMP_DIR, SHARED_TMP_DIR].includes((0, import_path4.dirname)(parent)) && /^mentiko-completion-context-[A-Za-z0-9_-]+$/.test(dirName);
}
...
async function launchRunnerV2CompletionPty(input) {
  ...
  const completionTmpDir = typeof env.TMPDIR === "string" && env.TMPDIR.trim() ? env.TMPDIR : (0, import_os3.tmpdir)();
  ...
  try {
    launched = await pty.spawn(
      ...
      { cwd: config_default.codeRoot, env: { TMPDIR: completionTmpDir } }
    );
  } catch (error) {
