...
function findTranscriptJsonl(root, uuid, depth) {
  ...
  for (const entry of entries) {
    ...
    if (entry.includes(uuid) && entry.endsWith(".jsonl")) return path2;
    try {
      if ((0, import_node_fs.statSync)(path2).isDirectory()) {
...
function createLiveMonitorIO(context) {
  ...
  return {
    ...
    observe: async (session) => {
      ...
      return {
        processGone: context.workspaceType === "local" ? await monitorAgentProcessGone(session, context.env) : false,
...
async function monitorAgentProcessGone(sessionName, env) {
...
async function resolveTranscriptJsonl(sessionName, env, context) {
  ...
  if (explicit && (0, import_node_fs4.existsSync)(explicit)) {
...
function monitorCompletionLatch(options) {
