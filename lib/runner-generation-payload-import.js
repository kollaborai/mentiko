...
function artifactPath(root, path) {
  ...
  if ((0, import_node_fs.existsSync)(path) && (0, import_node_fs.lstatSync)(path).isSymbolicLink()) throw new Error("Generation artifact path must not be a symbolic link.");
...
function ownedArtifactFile(root, name) {
  if ((0, import_node_path.basename)(name) !== name) return null;
  const path = (0, import_node_path.join)(root, name);
  if (!(0, import_node_fs.existsSync)(path) || (0, import_node_fs.lstatSync)(path).isSymbolicLink() || !(0, import_node_fs.lstatSync)(path).isFile()) return null;
  const resolved = (0, import_node_fs.realpathSync)(path);
  const rel = (0, import_node_path.relative)(root, resolved);
  return rel && !rel.startsWith("..") && !(0, import_node_path.isAbsolute)(rel) ? resolved : null;
}
