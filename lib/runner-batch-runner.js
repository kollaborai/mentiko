...
function assertBatchRunRecord(value) {
  ...
  for (const chain of value.chains) {
    ...
    ids.add(id);
    if (typeof chain.file !== "string" || !(0, import_node_path2.isAbsolute)(chain.file)) throw new BatchRunRecordValidationError("Batch chain file must be absolute.");
...
function canonicalizeBatchesDir(batchesDir) {
  if (typeof batchesDir !== "string" || !(0, import_node_path2.isAbsolute)(batchesDir)) throw new BatchRunRecordValidationError("Configured batches root must be absolute.");
  (0, import_node_fs2.mkdirSync)(batchesDir, { recursive: true });
  return (0, import_node_fs2.realpathSync)(batchesDir);
}
function resolveBatchPaths(batchesDir, batchId) {
  const root = canonicalizeBatchesDir(batchesDir);
  const id = requireBatchId(batchId);
  const batchDir = (0, import_node_path2.resolve)(root, id);
  const relation = (0, import_node_path2.relative)(root, batchDir);
  if (relation !== id || relation.startsWith("..") || (0, import_node_path2.isAbsolute)(relation)) throw new BatchRunRecordValidationError("Batch path escapes configured root.");
  return { batchesDir: root, batchDir, batchJsonPath: (0, import_node_path2.join)(batchDir, "batch.json") };
}
function resolveExistingBatchPaths(batchesDir, batchId) {
  const paths = resolveBatchPaths(batchesDir, batchId);
  if (!(0, import_node_fs2.existsSync)(paths.batchDir) || (0, import_node_fs2.lstatSync)(paths.batchDir).isSymbolicLink()) throw new BatchRunRecordValidationError("Batch directory is missing or unsafe.");
  if (!(0, import_node_fs2.existsSync)(paths.batchJsonPath) || (0, import_node_fs2.lstatSync)(paths.batchJsonPath).isSymbolicLink()) throw new BatchRunRecordValidationError("Batch record is missing or unsafe.");
  const realDir = (0, import_node_fs2.realpathSync)(paths.batchDir);
  const realRoot = (0, import_node_fs2.realpathSync)(paths.batchesDir);
  if ((0, import_node_path2.dirname)(realDir) !== realRoot || (0, import_node_path2.basename)(realDir) !== batchId) throw new BatchRunRecordValidationError("Batch path escapes configured root.");
  const realJson = (0, import_node_fs2.realpathSync)(paths.batchJsonPath);
  if ((0, import_node_path2.dirname)(realJson) !== realDir || (0, import_node_path2.basename)(realJson) !== "batch.json") throw new BatchRunRecordValidationError("Batch record path escapes batch directory.");
...
function readBatchRunRecord(batchesDir, batchId) {
  ...
  const record = parseBatchRunRecord((0, import_node_fs2.readFileSync)(batchJsonPath, "utf8"));
...
async function mutateBatchRunRecord(batchesDir, batchId, mutation) {
  ...
  try {
    const record = parseBatchRunRecord((0, import_node_fs2.readFileSync)(paths.batchJsonPath, "utf8"));
...
function writeBatchChainResult(batchesDir, batchId, result) {
  ...
  const target = (0, import_node_path2.join)(chainDir, "result.json");
  writeJsonAtomic(target, result);
  return target;
}
function writeJsonAtomic(path, value) {
  const temporary = (0, import_node_path2.join)((0, import_node_path2.dirname)(path), `.${(0, import_node_path2.basename)(path)}.${process.pid}.${(0, import_node_crypto.randomBytes)(4).toString("hex")}.tmp`);
...
function resolveExistingBatchChainDir(paths, chainId) {
  ...
  const chainDir = (0, import_node_path2.resolve)(paths.batchDir, id);
  if ((0, import_node_path2.relative)(paths.batchDir, chainDir) !== id || (0, import_node_path2.isAbsolute)((0, import_node_path2.relative)(paths.batchDir, chainDir))) {
    throw new BatchRunRecordValidationError("Batch chain path escapes batch directory.");
  }
  if (!(0, import_node_fs2.existsSync)(chainDir) || (0, import_node_fs2.lstatSync)(chainDir).isSymbolicLink()) {
    throw new BatchRunRecordValidationError("Batch chain directory is missing or unsafe.");
  }
  const realChainDir = (0, import_node_fs2.realpathSync)(chainDir);
  if ((0, import_node_path2.dirname)(realChainDir) !== paths.batchDir || (0, import_node_path2.basename)(realChainDir) !== id) {
    throw new BatchRunRecordValidationError("Batch chain path escapes batch directory.");
  }
  return realChainDir;
}
...
async function runBatch(input) {
  ...
  if (!(0, import_node_path3.isAbsolute)(input.runsDir) || !(0, import_node_path3.isAbsolute)(input.chainRunnerPath) || !(0, import_node_path3.isAbsolute)(input.cwd)) {
    throw new Error("Batch runner requires absolute configured paths.");
  }
  if (!(0, import_node_fs3.existsSync)(input.chainRunnerPath)) throw new Error("Configured chain runner is missing.");
...
async function markBatchChainLaunchFailed(batchesDir, batchId, chainId, reason) {
...
async function runBatchChain(input, chainId) {
  ...
  try {
    await mutateBatchRunRecord(input.batchesDir, input.batchId, (record) => {
      ...
      const run = createRunRecord({
        runId: `run-${Date.now()}-${(0, import_node_crypto2.randomBytes)(4).toString("hex")}`,
        chainName: chainId,
        goal: chain.goal
      });
      ...
      let child2;
      try {
        child2 = (0, import_node_child_process.spawn)(input.chainRunnerPath, [chain.file], {
          cwd: input.cwd,
      ...
      claimed = { chain, runId: run.id, started: started2, child: child2 };
      return {
        ...record,
        chains: record.chains.map((candidate) => candidate.id === chainId ? { ...candidate, run_id: run.id, started: started2, status: "running", ...child2.pid ? { pid: child2.pid } : {} } : candidate)
      };
    });
  } catch (error) {
  ...
  const status = outcome.cancelled ? "cancelled" : outcome.code === 0 ? "complete" : "failed";
...
function waitForChild(child, isCancelled) {
  return new Promise((resolveWait) => {
    ...
    child.once("close", (code) => {
      clearInterval(timer);
      resolveWait({ code, stdout, stderr, cancelled: cancellationSent });
    });
  });
}
function joinRunJsonPath(runsDir, runId) {
  return (0, import_node_path3.join)(runsDir, runId, "run.json");
}

// lib/runner-v2/batch-runner-cli.ts
async function runBatchRunnerCli(argv, environment = process.env, write = (line) => console.log(line)) {
  ...
  for (const path of [batchesDir, runsDir, chainRunnerPath, cwd]) if (!(0, import_node_path4.isAbsolute)(path)) throw new Error("Batch runner paths must be absolute.");
