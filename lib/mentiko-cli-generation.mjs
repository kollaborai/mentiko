...
function isPayloadCompatibleWithKind(obj, kind) {
  ...
  if (kind === "chain_recommendation") {
    const recommendation = obj.recommendation && typeof obj.recommendation === "object" && !Array.isArray(obj.recommendation)
      ? obj.recommendation
      : obj;
...
// Normalize the salvaged payload to the shape the /complete route expects for this kind.
// Chain generation jobs are post-processed via result.output (a chain JSON *string*); when
// the agent emits the raw chain object instead, wrap it so postProcessChain still runs.
// Task and other kinds consume the object directly -- pass through untouched.
