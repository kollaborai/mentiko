...
// Kind-aware payload contract — generated from the canonical TypeScript source
// used by the in-process consumers (web/lib/runs/job-store.ts +
// auto-run/route.ts), so the bare-node and typed doors cannot carry independent
// parsers. It lives under web/lib/ so the Next standalone build traces it
// natively; the Dockerfile assemble step flattens web/lib/ into
// /opt/mentiko/lib/ (Dockerfile ~L80-81), so at tenant runtime the generated
// contract is a sibling of this CLI, while in the dev checkout it sits at
// ../web/lib/. Resolve whichever path exists.
const contractUrl =
  [
    new URL("./generation/payload-contract.runtime.js", import.meta.url),
    new URL("../web/lib/generation/payload-contract.runtime.js", import.meta.url),
...
// Artifacts written by typed completion activity capture (
