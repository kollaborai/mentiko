#!/usr/bin/env node
// GENERATED FROM web/lib/runner-v2/readiness-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
__export(readiness_cli_exports, {
  decideStartupRecovery: () => decideStartupRecovery,
...
function runReadinessCli(argv, write = (line) => console.log(line)) {
  ...
  if (command === "classify") {
    ...
    if (!(0, import_node_fs3.existsSync)(profilePath)) {
      write(JSON.stringify({ status: "unknown", reason: "profile file missing" }));
      return;
    }
    if (!(0, import_node_fs3.existsSync)(capturePath)) {
    ...
    try {
      result = classifyCliReadiness({
        ...
        output: (0, import_node_fs3.readFileSync)(capturePath, "utf8"),
...
function waitForReadiness(input) {
  if (!(0, import_node_fs3.existsSync)(input.profilePath)) return { result: { status: "unknown", reason: "profile file missing" }, exitCode: 4 };
...
function writeStartupRecoveryArtifacts(recovery, readiness2, output) {
  ...
  writeAtomic((0, import_node_path3.join)(directory, `${recovery.agentId}-startup-capture.txt`), output);
  writeAtomic((0, import_node_path3.join)(directory, `${recovery.agentId}-startup-readiness.json`), `${JSON.stringify(readiness2, null, 2)}
`);
}
function appendRecoveryDecision(recovery, decision) {
  ...
  const path2 = (0, import_node_path3.join)(directory, `${recovery.agentId}-startup-recovery-decisions.jsonl`);
  if ((0, import_node_fs3.existsSync)(path2) && (0, import_node_fs3.lstatSync)(path2).isSymbolicLink()) throw new Error("startup recovery decision log must not be a symlink");
  (0, import_node_fs3.appendFileSync)(path2, `${JSON.stringify(decision)}
`, { mode: 384 });
}
...
function writeCaptureAtomically(path2, content) {
  if ((0, import_node_fs3.existsSync)(path2)) assertRegularInput(path2, "capture");
  writeAtomic(path2, content);
}
function writeAtomic(path2, content) {
  const directory = ensureDirectory((0, import_node_path3.dirname)(path2), "capture directory");
  const target = (0, import_node_path3.join)(directory, (0, import_node_path3.basename)(path2));
  if ((0, import_node_fs3.existsSync)(target)) assertRegularInput(target, "capture");
  const temporary = `${target}.${process.pid}.tmp`;
  (0, import_node_fs3.writeFileSync)(temporary, content, { mode: 384 });
...
function ensureDirectory(path2, label) {
  ...
  if (!(0, import_node_fs3.existsSync)(candidate)) (0, import_node_fs3.mkdirSync)(candidate, { recursive: true, mode: 448 });
  assertExistingDirectoryComponents(candidate, label, path2);
  return candidate;
}
function canonicalizeTrustedSystemAlias(path2) {
  const normalized = (0, import_node_path3.resolve)(path2);
  for (const alias of ["/tmp", "/var", "/etc"]) {
    if (normalized !== alias && !normalized.startsWith(`${alias}/`)) continue;
    const canonical = (0, import_node_fs3.realpathSync)(alias);
    return normalized === alias ? canonical : (0, import_node_path3.join)(canonical, normalized.slice(alias.length + 1));
  }
  return normalized;
}
function assertExistingDirectoryComponents(path2, label, originalPath) {
  const absolute = (0, import_node_path3.resolve)(path2);
  const root = (0, import_node_path3.parse)(absolute).root;
  let current = root;
  for (const part of absolute.slice(root.length).split(import_node_path3.sep).filter(Boolean)) {
    current = (0, import_node_path3.join)(current, part);
    if (!(0, import_node_fs3.existsSync)(current)) return;
    const entry = (0, import_node_fs3.lstatSync)(current);
...
function assertRegularInput(path2, label) {
  const entry = (0, import_node_fs3.lstatSync)(path2);
