...
function appendRecoveryDecision(recovery, decision) {
  ...
  const path2 = (0, import_node_path2.join)(recovery.artifactDir, `${recovery.agentId}-startup-recovery-decisions.jsonl`);
...
function writeAtomic(path2, content) {
  ...
  (0, import_node_fs2.renameSync)(temporary, path2);
}
function ensureDirectory(path2, label) {
  if ((0, import_node_fs2.existsSync)(path2)) {
    const entry2 = (0, import_node_fs2.lstatSync)(path2);
    if (entry2.isSymbolicLink() || !entry2.isDirectory()) throw new Error(`${label} must be a non-symlink directory: ${path2}`);
    return;
  }
  (0, import_node_fs2.mkdirSync)(path2, { recursive: true, mode: 448 });
  const entry = (0, import_node_fs2.lstatSync)(path2);
  if (entry.isSymbolicLink() || !entry.isDirectory()) throw new Error(`${label} must be a non-symlink directory: ${path2}`);
}
