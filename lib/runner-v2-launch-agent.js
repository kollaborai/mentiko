...
async function main() {
  ...
  const chain = JSON.parse((0, import_node_fs8.readFileSync)(chainPath, "utf8"));
  ...
  const results = await Promise.all(uniqueAgentIds.map((agentId) => startRunnerV2Bootstrap({
    ...
    runId,
    agentId,
    chainId: chain.id || (0, import_node_path8.basename)(chainPath, ".json"),
    chainName: chain.name || chain.id || (0, import_node_path8.basename)(chainPath, ".json"),
