...
function writeAgentNodeChainSnapshot(input) {
  const rawChain = JSON.parse((0, import_fs8.readFileSync)(input.chainPath, "utf8"));
...
async function sleepBefore(delayMs, deadline) {
  ...
  await new Promise((resolve8) => setTimeout(resolve8, Math.min(delayMs, remainingMs)));
...
function nodeRecordPath(runWorkspace2, attemptId) {
  return (0, import_node_path9.join)(runWorkspace2.isolationRoot, "nodes", `${digest(attemptId, 32)}.json`);
}
function assertNodeWorkspaceRecord(runWorkspace2, node, expected) {
  const nodeKey = digest(expected.attemptId, 32);
  const expectedWorktreeRoot = (0, import_node_path9.join)(runWorkspace2.worktreesRoot, nodeKey);
  const expectedWorkspacePath = runWorkspace2.relativeWorkspacePath === "." ? expectedWorktreeRoot : (0, import_node_path9.join)(expectedWorktreeRoot, runWorkspace2.relativeWorkspacePath);
...
function allocateGitNodeWorkspace(input) {
  ...
  return withExclusiveFileClaim(
    (0, import_node_path9.join)(runWorkspace2.isolationRoot, "claims", "nodes", `${nodeKey}.claim`),
    () => {
      ...
      if (!baseCommit) {
        baseCommit = withExclusiveFileClaim(
          (0, import_node_path9.join)(runWorkspace2.isolationRoot, "claims", "integration.claim"),
          () => {
      ...
      const worktreeRoot = (0, import_node_path9.join)(runWorkspace2.worktreesRoot, nodeKey);
      ...
      const workspacePath = runWorkspace2.relativeWorkspacePath === "." ? worktreeRoot : (0, import_node_path9.join)(worktreeRoot, runWorkspace2.relativeWorkspacePath);
...
function reconcilePendingIntegrationReceipts(runWorkspace2) {
  const receiptsRoot = (0, import_node_path9.join)(runWorkspace2.isolationRoot, "receipts", "integrations");
...
function cleanupRoot(runWorkspace2) {
  return (0, import_node_path11.join)(runWorkspace2.isolationRoot, "cleanup");
}
function pendingPath(runWorkspace2, mode, attemptId) {
  return (0, import_node_path11.join)(cleanupRoot(runWorkspace2), "pending", `${cleanupId(mode, attemptId)}.json`);
}
function completedPath(runWorkspace2, jobId) {
  return (0, import_node_path11.join)(cleanupRoot(runWorkspace2), "completed", `${jobId}.json`);
}
...
function writeJsonOnce3(path2, value2) {
  ...
  const temporary = (0, import_node_path11.join)(
    ...
    `.${(0, import_node_path11.basename)(path2)}.${process.pid}.${(0, import_node_crypto6.randomBytes)(8).toString("hex")}.tmp`
  );
  try {
...
async function recoverInterruptedRoutedBootstrap(input) {
  ...
  const runJsonPath = (0, import_node_path13.join)(input.context.runDir, "run.json");
  const run = readRunJson(runJsonPath);
...
async function runRoutedLaunchJob(input) {
  ...
  try {
    const chain = JSON.parse((0, import_node_fs12.readFileSync)(claimed.chainPath, "utf8"));
...
async function runLaunchTarget(input) {
  ...
  const context = {
    ...
    agentId: input.agentId,
    chainId: input.chain.id || (0, import_node_path14.basename)(input.job.chainPath, ".json"),
    chainName: input.chain.name || input.chain.id || (0, import_node_path14.basename)(input.job.chainPath, ".json"),
...
async function main() {
  ...
  const runJsonPath = (0, import_node_path15.join)(runDir, "run.json");
