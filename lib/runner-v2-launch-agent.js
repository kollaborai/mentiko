...
async function main() {
  ...
  const chain = JSON.parse((0, import_node_fs10.readFileSync)(chainPath, "utf8"));
  ...
  const stopHeartbeat = startLaunchCoordinatorHeartbeat({
    runJsonPath: (0, import_node_path10.join)(runDir, "run.json"),
    pid: process.pid,
  ...
  try {
    const results = await Promise.all(uniqueAgentIds.map((agentId) => startRunnerV2Bootstrap({
      ...
      runId,
      agentId,
      chainId: chain.id || (0, import_node_path10.basename)(chainPath, ".json"),
      chainName: chain.name || chain.id || (0, import_node_path10.basename)(chainPath, ".json"),
...
async function dispatchCoordinator(input) {
  const runJsonPath = (0, import_node_path10.join)(input.runDir, "run.json");
  ...
  coordinator.unref();
  const timeoutMs = positiveInteger2(process.env.MENTIKO_LAUNCH_QUEUE_ACCEPT_TIMEOUT_MS, 12e4);
...
function latestAttemptIds(runJsonPath, runId, agentIds) {
  const attempts = readRunnerV2AttemptState(runJsonPath).attempts;
...
function queuedLaunchAccepted(runJsonPath, runId, agentIds, before) {
  const run = readRunJson(runJsonPath);
  const attempts = readRunnerV2AttemptState(runJsonPath).attempts;
  return agentIds.every((agentId) => {
    ...
    return Boolean(agent?.session) && (agent?.status === "pending" || agent?.status === "running" || agent?.status === "blocked");
  });
}
