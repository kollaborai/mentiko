#!/usr/bin/env node
// GENERATED FROM web/lib/runner-v2/launch-agent-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
function validateRunRecord(value) {
  ...
  if (Array.isArray(value.agents)) {
    const agentIds = value.agents.filter(isPlainRecord).map((agent) => agent.id).filter((id) => typeof id === "string");
...
async function awaitSubmissionOperation(operation, deadline) {
  ...
  try {
    return await new Promise((resolve3, reject) => {
      timeout = setTimeout(() => resolve3(SUBMISSION_DEADLINE_EXCEEDED), remainingMs);
      try {
        void operation().then(resolve3, reject);
      } catch (error) {
        reject(error);
      }
    });
...
async function main() {
  ...
  const chain = JSON.parse((0, import_node_fs4.readFileSync)(chainPath, "utf8"));
  ...
  const results = await Promise.all(uniqueAgentIds.map((agentId) => startRunnerV2Bootstrap({
    ...
    runId,
    agentId,
    chainId: chain.id || (0, import_node_path4.basename)(chainPath, ".json"),
    chainName: chain.name || chain.id || (0, import_node_path4.basename)(chainPath, ".json"),
