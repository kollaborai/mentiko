...
function githubErrorTitle(runJsonPath, agentId) {
  const run = readExpectedRun2(runJsonPath);
  return `Agent Error: ${agentId} failed in ${run.chain || "unknown"}`;
}
function githubErrorBody(input) {
  const run = readExpectedRun2(input.runJsonPath);
  const output = input.outputFile && (0, import_node_fs5.existsSync)(input.outputFile) ? tailLines((0, import_node_fs5.readFileSync)(input.outputFile, "utf8"), 100) : "";
