...
function updateRunStatus(runJsonPath, status, statusMessage, now = /* @__PURE__ */ new Date(), onMutation) {
  return updateRunJson(runJsonPath, (current) => {
    ...
    return {
      ...current,
      status,
      ...statusMessage ? { status_message: statusMessage } : successfulTerminal ? { status_message: void 0 } : {},
      ...TERMINAL_RUN_STATUSES.has(status) && (!current.completed || successfulTerminal && current.status !== "completed") ? { completed: nowIso(now) } : {}
    };
  }, void 0, onMutation);
}
...
function generationAuditMetadata(run, current, generationKind, auditStatus) {
...
function taskSummaryNote(run, summary, status, agents, completed) {
