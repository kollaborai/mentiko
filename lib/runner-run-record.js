...
async function runRunRecordCli(argv, environment = process.env, write = (line) => console.log(line)) {
  ...
  if (parsed.command === "set-status") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--status", "--message", "--reason"]));
    ...
    write(JSON.stringify(updateRunStatus(
      ...
      void 0,
      void 0,
      reason ? { actor: "system", reason } : void 0
    )));
    return;
  }
  ...
  if (parsed.command === "set-agent-status") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--agent-id", "--status", "--reason"]));
    const status = required(parsed.values, "--status");
