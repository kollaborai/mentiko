...
function stopRunFromWatchdog(runJsonPath, now = /* @__PURE__ */ new Date()) {
  const timestamp = nowIso2(now);
  return updateRunJson(runJsonPath, (value) => {
    const current = requireCurrent(value, runJsonPath);
    return {
...
async function runRunRecordCli(argv, environment = process.env, write = (line) => console.log(line)) {
  ...
  if (parsed.command === "create") {
    ...
    const chainName = optional(parsed.values, "--chain") || chainNameFromFile(required(parsed.values, "--chain-file"));
  ...
  if (parsed.command === "list") {
    ...
    const records = (0, import_node_fs6.readdirSync)(runsDir, { withFileTypes: true }).filter((entry) => entry.isDirectory() && entry.name.startsWith("run-")).map((entry) => readRunRecordAt(runsDir, entry.name)).filter((record) => !chain || record.chain === chain);
