#!/usr/bin/env node
// GENERATED FROM web/lib/runner-v2/run-record-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
// lib/runner-v2/run-record-cli.ts
var run_record_cli_exports = {};
__export(run_record_cli_exports, {
  configuredRunsDir: () => configuredRunsDir,
  runRunRecordCli: () => runRunRecordCli
});
module.exports = __toCommonJS(run_record_cli_exports);
...
function readLockSnapshot(lockDir) {
  let pidText = "";
  try {
    pidText = (0, import_fs.readFileSync)(`${lockDir}/pid`, "utf-8").trim();
  } catch {
    return void 0;
  }
  let ownerToken;
  try {
    ownerToken = (0, import_fs.readFileSync)(`${lockDir}/owner`, "utf-8").trim() || void 0;
  } catch {
  }
...
function acquireLock(lockDir) {
  ...
  for (; ; ) {
    ...
    let created = false;
    try {
      (0, import_fs.mkdirSync)(lockDir);
      created = true;
      try {
        (0, import_fs.writeFileSync)(`${lockDir}/owner`, ownerToken, { flag: "wx" });
        (0, import_fs.writeFileSync)(`${lockDir}/pid`, String(process.pid), { flag: "wx" });
      } catch (error) {
...
function breakDeadLock(lockDir, observed) {
  ...
  try {
    (0, import_fs.mkdirSync)(takeoverClaim);
    try {
      (0, import_fs.writeFileSync)(`${takeoverClaim}/owner`, claimOwner, { flag: "wx" });
      (0, import_fs.writeFileSync)(`${takeoverClaim}/pid`, String(process.pid), { flag: "wx" });
    } catch (error) {
  ...
  try {
    ...
    try {
      (0, import_fs.renameSync)(lockDir, quarantined);
    } catch (error) {
    ...
    if (snapshotsEqual(current, moved) && moved && snapshotHolderIsDead(moved)) {
      try {
        (0, import_fs.unlinkSync)(`${quarantined}/pid`);
      } catch {
      }
      try {
        (0, import_fs.unlinkSync)(`${quarantined}/owner`);
      } catch {
      }
      (0, import_fs.rmdirSync)(quarantined);
      return true;
    }
    try {
      (0, import_fs.renameSync)(quarantined, lockDir);
    } catch {
    }
...
function readTakeoverClaimSnapshot(claimDir) {
  try {
    const entries = (0, import_fs.readdirSync)(claimDir).sort();
    let pidText;
    let ownerToken;
    try {
      pidText = (0, import_fs.readFileSync)(`${claimDir}/pid`, "utf8").trim() || void 0;
    } catch {
    }
    try {
      ownerToken = (0, import_fs.readFileSync)(`${claimDir}/owner`, "utf8").trim() || void 0;
    } catch {
    }
    return { pidText, ownerToken, entries, mtimeMs: (0, import_fs.statSync)(claimDir).mtimeMs };
  } catch {
    return void 0;
  }
}
...
function recoverAbandonedTakeoverClaim(claimDir) {
  ...
  try {
    ...
    try {
      (0, import_fs.renameSync)(claimDir, quarantined);
    } catch (error) {
      if (error.code === "ENOENT") return false;
      throw error;
    }
    ...
    try {
      (0, import_fs.renameSync)(quarantined, claimDir);
    } catch {
    }
    return false;
  } catch {
...
function readOwnerToken(lockDir) {
  try {
    return (0, import_fs.readFileSync)(`${lockDir}/owner`, "utf-8").trim() || void 0;
  } catch {
    return void 0;
  }
}
function cleanupIncompleteAcquisition(lockDir, ownerToken) {
  if (readOwnerToken(lockDir) === ownerToken) {
    try {
      (0, import_fs.unlinkSync)(`${lockDir}/pid`);
    } catch {
    }
    try {
      (0, import_fs.unlinkSync)(`${lockDir}/owner`);
    } catch {
    }
  }
  try {
    (0, import_fs.rmdirSync)(lockDir);
  } catch {
  }
}
function releaseLock(lockDir, ownerToken) {
  ...
  try {
    (0, import_fs.renameSync)(lockDir, retired);
  } catch {
    return;
  }
  if (readOwnerToken(retired) !== ownerToken) {
    try {
      (0, import_fs.renameSync)(retired, lockDir);
    } catch {
    }
    return;
  }
  try {
    (0, import_fs.unlinkSync)(`${retired}/pid`);
  } catch {
  }
  try {
    (0, import_fs.unlinkSync)(`${retired}/owner`);
  } catch {
  }
  try {
    (0, import_fs.rmdirSync)(retired);
  } catch {
  }
}
...
function writeRunJsonAtomic(runJsonPath, data) {
  ...
  try {
    (0, import_fs.writeFileSync)(tmp, JSON.stringify(data, null, 2), { flag: "wx" });
    (0, import_fs.renameSync)(tmp, runJsonPath);
  } catch (error) {
    try {
...
function writeRunJsonExclusive(runJsonPath, data) {
  ...
  try {
    (0, import_fs.writeFileSync)(tmp, JSON.stringify(data, null, 2), { flag: "wx" });
    (0, import_fs.linkSync)(tmp, runJsonPath);
  } finally {
    try {
      (0, import_fs.unlinkSync)(tmp);
    } catch {
...
function isAgentStatus(value) {
  return typeof value === "string" && RUN_AGENT_STATUSES.includes(value);
}
function canonicalizeRunsDir(runsDir) {
  ...
  if (!(0, import_node_path.isAbsolute)(runsDir)) throw new Error("Configured runs root must be absolute.");
  const absolute = (0, import_node_path.resolve)(runsDir);
  const missingSegments = [];
  let existingAncestor = absolute;
  while (!(0, import_node_fs.existsSync)(existingAncestor)) {
    const parent = (0, import_node_path.dirname)(existingAncestor);
    if (parent === existingAncestor) break;
    missingSegments.unshift((0, import_node_path.basename)(existingAncestor));
    existingAncestor = parent;
  }
  if (!(0, import_node_fs.statSync)(existingAncestor).isDirectory()) {
    throw new Error("Configured runs root must be a directory.");
  }
  return (0, import_node_path.resolve)((0, import_node_fs.realpathSync)(existingAncestor), ...missingSegments);
}
function resolveRunRecordPaths(runsDir, runId) {
  ...
  const runDir = (0, import_node_path.resolve)(canonicalRunsDir, canonicalRunId);
  const relation = (0, import_node_path.relative)(canonicalRunsDir, runDir);
  if (relation !== canonicalRunId || relation.startsWith("..") || (0, import_node_path.isAbsolute)(relation)) {
    throw new Error("Run record path escapes the configured runs root.");
  }
  return {
    runsDir: canonicalRunsDir,
    runDir,
    runJsonPath: (0, import_node_path.join)(runDir, "run.json")
  };
}
function resolveExistingRunRecordPaths(runsDir, runId) {
  const paths = resolveRunRecordPaths(runsDir, runId);
  const runDirStat = (0, import_node_fs.lstatSync)(paths.runDir);
  ...
  const realRunsDir = (0, import_node_fs.realpathSync)(paths.runsDir);
  const realRunDir = (0, import_node_fs.realpathSync)(paths.runDir);
  if ((0, import_node_path.dirname)(realRunDir) !== realRunsDir || (0, import_node_path.basename)(realRunDir) !== runId) {
    throw new Error("Run record path escapes the configured runs root.");
  }
  const runJsonPath = (0, import_node_path.join)(realRunDir, "run.json");
  const runJsonStat = (0, import_node_fs.lstatSync)(runJsonPath);
  ...
  const realRunJsonPath = (0, import_node_fs.realpathSync)(runJsonPath);
  if ((0, import_node_path.dirname)(realRunJsonPath) !== realRunDir || (0, import_node_path.basename)(realRunJsonPath) !== "run.json") {
...
function readRunRecordAt(runsDir, runId) {
  ...
  const record = parseRunRecord((0, import_node_fs.readFileSync)(paths.runJsonPath, "utf8"));
  if (record.id !== runId) {
...
function createRunRecordFile(runsDir, record) {
  ...
  (0, import_node_fs.mkdirSync)(paths.runsDir, { recursive: true });
  try {
    (0, import_node_fs.mkdirSync)(paths.runDir);
  } catch (error) {
    if (error.code === "EEXIST") {
  ...
  try {
    ...
    if (!(0, import_node_fs.existsSync)(paths.runJsonPath)) {
      try {
        (0, import_node_fs.rmdirSync)(paths.runDir);
      } catch {
      }
    }
    throw error;
...
function readRunJson(runJsonPath) {
  return parseRunRecord((0, import_fs2.readFileSync)(runJsonPath, "utf-8"));
}
function updateRunJson(runJsonPath, update, onLockTimeout, onMutation) {
  return withRunJsonLock(runJsonPath, () => {
    const current = (0, import_fs2.existsSync)(runJsonPath) ? readRunJson(runJsonPath) : void 0;
...
// lib/runner-v2/run-record-operations.ts
var import_node_fs2 = require("node:fs");
var import_node_path2 = require("node:path");
function nowIso2(now = /* @__PURE__ */ new Date()) {
  return now.toISOString();
}
function requireCurrent(current, runJsonPath) {
  if (!current) throw new Error(`run.json not found: ${runJsonPath}`);
  return current;
}
function upsertAgent(agents, agentId, update) {
...
function markRunAgentBlocked(runJsonPath, agentId, reason, now = /* @__PURE__ */ new Date()) {
  const timestamp = nowIso2(now);
...
function markRunAgentFailed(runJsonPath, agentId, reason, now = /* @__PURE__ */ new Date()) {
  const timestamp = nowIso2(now);
  return updateRunJson(runJsonPath, (value) => {
    const current = requireCurrent(value, runJsonPath);
    return {
...
function updateRunActivityManifest(runJsonPath, agentId, diffLines, fileCount, now = /* @__PURE__ */ new Date()) {
  ...
  return updateRunJson(runJsonPath, (value) => {
    ...
    const upsert = (artifact) => {
      artifacts = artifacts.filter((item) => !isRecord(item) || item.agentId !== agentId || item.type !== artifact.type);
...
function updateRunActivityManifestFromArtifacts(runJsonPath, agentId, now = /* @__PURE__ */ new Date()) {
  const artifactsDir = (0, import_node_path2.join)((0, import_node_path2.dirname)(runJsonPath), "artifacts");
  const diffPath = (0, import_node_path2.join)(artifactsDir, `${agentId}-diff.patch`);
  const changedPath = (0, import_node_path2.join)(artifactsDir, `${agentId}-files-changed.json`);
  const diffLines = (0, import_node_fs2.existsSync)(diffPath) ? ((0, import_node_fs2.readFileSync)(diffPath, "utf8").match(/\n/g) || []).length : 0;
  let fileCount = 0;
  if ((0, import_node_fs2.existsSync)(changedPath)) {
    const changed = JSON.parse((0, import_node_fs2.readFileSync)(changedPath, "utf8"));
...
function startPeerRun(runJsonPath, firstSession, secondSession) {
  return updateRunJson(runJsonPath, (value) => {
    const current = requireCurrent(value, runJsonPath);
    ...
    return {
      ...
      status: "running",
      completed: void 0,
      sessions: Array.from(/* @__PURE__ */ new Set([...current.sessions || [], firstSession, secondSession])),
      agents
    };
  });
}
function completePeerRun(runJsonPath, rounds, now = /* @__PURE__ */ new Date()) {
...
function buildRunSummaryFromFiles(runJsonPath, now = /* @__PURE__ */ new Date()) {
  const run = readRunJson(runJsonPath);
  const expectedRunId = (0, import_node_path2.basename)((0, import_node_path2.dirname)(runJsonPath));
  if (run.id !== expectedRunId) {
    throw new Error(`Run record id ${run.id} does not match directory ${expectedRunId}`);
  }
  const artifactsDir = (0, import_node_path2.join)((0, import_node_path2.dirname)(runJsonPath), "artifacts");
  const summaries = (0, import_node_fs2.existsSync)(artifactsDir) ? (0, import_node_fs2.readdirSync)(artifactsDir).filter((name) => name.endsWith("-summary.json") && name !== "run-summary.json").sort().flatMap((name) => {
    const path2 = (0, import_node_path2.join)(artifactsDir, name);
    try {
      const value = JSON.parse((0, import_node_fs2.readFileSync)(path2, "utf8"));
      if (!isRecord(value)) return [];
      return [{
        ...value,
        agentId: stringValue(value.agentId) || stringValue(value.agent_id) || (0, import_node_path2.basename)(name, "-summary.json"),
        _file: path2
      }];
    } catch {
      return [];
...
function writeRunSummaryArtifact(runJsonPath, now = /* @__PURE__ */ new Date()) {
  const summary = buildRunSummaryFromFiles(runJsonPath, now);
  const artifactPath = (0, import_node_path2.join)((0, import_node_path2.dirname)(runJsonPath), "artifacts", "run-summary.json");
  (0, import_node_fs2.mkdirSync)((0, import_node_path2.dirname)(artifactPath), { recursive: true });
  const temporaryPath = `${artifactPath}.${process.pid}.${Date.now()}.tmp`;
  (0, import_node_fs2.writeFileSync)(temporaryPath, `${JSON.stringify(summary, null, 2)}
`, { flag: "wx" });
  (0, import_node_fs2.renameSync)(temporaryPath, artifactPath);
  const timestamp = nowIso2(now);
  const run = updateRunJson(runJsonPath, (value) => {
    const current = requireCurrent(value, runJsonPath);
    const artifacts = runArtifacts(current).filter((artifact) => !isRecord(artifact) || artifact.type !== "run-summary");
    artifacts.push({ type: "run-summary", path: artifactPath, timestamp });
    return { ...current, summary, artifacts };
  });
  return { run, summary };
}
function buildRunSummary(run, summaries, now = /* @__PURE__ */ new Date()) {
  const ranks = new Map(run.agents.map((agent, index) => [agent.id, index]));
  const ordered = [...summaries].sort((left, right) => (ranks.get(left.agentId) ?? -1) - (ranks.get(right.agentId) ?? -1));
  ...
  if (summaries.some((summary) => statusFailed(summary) || explicit(summary, /(?:overall )?result:\s*fail/i))) {
    outcome = "fail";
  } else if (summaries.some((summary) => stringValue(summary.status).toLowerCase() === "partial" || explicit(summary, /(?:overall )?result:\s*partial\s*-?\s*pass|partial\s*-?\s*pass/i))) {
    outcome = "partial_pass";
  } else if (summaries.some((summary) => explicit(summary, /(?:overall )?result:\s*pass|pipeline passes|passes all/i))) {
    outcome = "pass";
  ...
  const finalExecutiveSummary = ordered.map((summary) => summary.executiveSummary).filter((value) => typeof value === "string").at(-1);
  ...
  return {
    ...
    recommendation: outcome === "partial_pass" ? "review_before_next_task" : outcome === "fail" ? "fix_or_rerun" : outcome === "pass" || outcome === "complete" ? "move_forward" : "inspect_run",
    summary: finalExecutiveSummary ?? (run.status === "completed" ? "Run completed (no agent verdict \u2014 inspect run)." : `Run status: ${run.status || "unknown"}`),
...
// lib/runner-v2/run-task-sync.ts
...
function isStringRecord(value) {
  return isPlainRecord2(value) && Object.values(value).every((entry) => typeof entry === "string");
}

// lib/runner-v2/event-lifecycle.ts
function requireConfiguredEventsDir(eventsDir) {
  requireNonEmpty("eventsDir", eventsDir);
  if (!(0, import_node_path3.isAbsolute)(eventsDir)) {
    throw new Error(`eventsDir must be an absolute configured path: ${eventsDir}`);
  }
  const root = (0, import_node_path3.resolve)(eventsDir);
...
function isRegularDirectory(path2) {
  try {
    return (0, import_node_fs3.lstatSync)(path2).isDirectory();
  } catch {
    return false;
  }
}
...
function emitRunnerEvent(input) {
  ...
  const temporaryPath = (0, import_node_path4.join)(
    eventsDir,
    `.${(0, import_node_path4.basename)(requestedFilename)}.${process.pid}.${(0, import_node_crypto.randomUUID)()}.tmp`
  );
  (0, import_node_fs4.writeFileSync)(temporaryPath, content, { encoding: "utf8", flag: "wx" });
  try {
    ...
    try {
      (0, import_node_fs4.unlinkSync)(temporaryPath);
    } catch {
    }
  }
}
...
function persistWithoutClobber(input) {
  const requestedPath = (0, import_node_path4.join)(input.eventsDir, input.requestedFilename);
  try {
    (0, import_node_fs4.linkSync)(input.temporaryPath, requestedPath);
    return resultFor(requestedPath, input.requestedFilename, input.content);
  } catch (error) {
    if (!isAlreadyExists(error)) throw error;
  }
  const existingContent = (0, import_node_fs4.readFileSync)(requestedPath, "utf8");
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    const collisionPath = (0, import_node_path4.join)(input.eventsDir, collisionFilename);
    try {
      (0, import_node_fs4.linkSync)(input.temporaryPath, collisionPath);
      return resultFor(collisionPath, collisionFilename, input.content);
    } catch (error) {
      if (!isAlreadyExists(error)) throw error;
    }
    const collisionContent = (0, import_node_fs4.readFileSync)(collisionPath, "utf8");
...
function collisionSafeFilename(filename, idempotencyKey) {
  ...
  if (idempotencyKey) {
    const digest = (0, import_node_crypto.createHash)("sha256").update(idempotencyKey).digest("hex").slice(0, 20);
...
// lib/runner-v2/run-task-sync.ts
async function syncLinkedTaskFromRun(runJsonPath, status, context) {
  ...
  const agents = summarizedRun.agents.map((agent) => `${agent.id || "unknown"}|${agent.status || "unknown"}`).join(",");
  const artifacts = Array.isArray(summarizedRun.artifacts) ? summarizedRun.artifacts : [];
  const blockedReason = status === "blocked" ? terminalReason(summarizedRun, status) : void 0;
  const updatedMetadata = {
    ...
    ...status !== "blocked" ? { last_run_blocked_reason: void 0 } : {}
  };
...
function readExpectedRun(runJsonPath) {
  const run = readRunJson(runJsonPath);
  const expectedRunId = (0, import_node_path5.basename)((0, import_node_path5.dirname)(runJsonPath));
  if (run.id !== expectedRunId) {
    throw new Error(`Run record id ${run.id} does not match directory ${expectedRunId}`);
  }
  return run;
...
async function getTask(taskId, context) {
  ...
  return {
    ...
    metadata: isRecord2(issue.metadata) ? issue.metadata : {}
  };
}
async function patchTask(taskId, body, context) {
  await requestJson(taskUrl(context.apiBase, taskId), {
    method: "PATCH",
    body: JSON.stringify(body)
  }, context);
}
async function addTaskComment(taskId, text, context) {
...
async function requestJson(url, init, context) {
  ...
  if (!isRecord2(value)) throw new Error(`${init.method || "GET"} ${url} returned a non-object JSON payload`);
  return value;
}
function generationAuditMetadata(run, current, generationKind, auditStatus, reason) {
...
function normalizedAuditStatus(status) {
...
function taskSummaryNote(run, summary, status, agents, completed, blockedReason) {
...
function terminalReason(run, status) {
  if (status !== "blocked") return void 0;
...
function taskUrl(apiBase, taskId) {
...
function recordValue(value, key) {
  ...
  if (!isRecord2(nested)) throw new Error(`Task API response is missing object field ${key}`);
  return nested;
}
...
// lib/runner-v2/run-record-queries.ts
var import_node_fs5 = require("node:fs");
var import_node_path6 = require("node:path");
function runGoal(runJsonPath) {
  return readExpectedRun2(runJsonPath).goal;
}
function runStatus(runJsonPath) {
  return readExpectedRun2(runJsonPath).status;
}
function runWorkspacePath(runJsonPath) {
  return readExpectedRun2(runJsonPath).workspacePath || "";
}
function runStartedAt(runJsonPath) {
  return readExpectedRun2(runJsonPath).started;
}
function runCompletedAt(runJsonPath) {
  return readExpectedRun2(runJsonPath).completed || "";
}
function completedAgentLines(runJsonPath) {
  return readExpectedRun2(runJsonPath).agents.filter((agent) => agent.status === "complete").map((agent) => `- ${agent.name || agent.id} (${agent.id || "unknown"})`).join("\n");
}
function countRunningRuns(runsDir, excludeRunId) {
  const canonicalRunsDir = canonicalizeRunsDir(runsDir);
  if (!(0, import_node_fs5.existsSync)(canonicalRunsDir)) return 0;
  return (0, import_node_fs5.readdirSync)(canonicalRunsDir, { withFileTypes: true }).filter((entry) => entry.isDirectory() && isRunId(entry.name) && entry.name !== excludeRunId).map((entry) => readRunRecordAt(canonicalRunsDir, entry.name)).filter((run) => run.status === "running").length;
}
function deleteRunsOwnedByUser(runsDir, userId) {
  if (!userId) throw new Error("user id must not be empty");
  const canonicalRunsDir = canonicalizeRunsDir(runsDir);
  if (!(0, import_node_fs5.existsSync)(canonicalRunsDir)) return [];
  const deleted = [];
  for (const entry of (0, import_node_fs5.readdirSync)(canonicalRunsDir, { withFileTypes: true })) {
    ...
    const runDir = (0, import_node_path6.join)(canonicalRunsDir, entry.name);
    (0, import_node_fs5.rmSync)(runDir, { recursive: true });
    deleted.push(runDir);
  }
  return deleted;
}
function deleteRunsOlderThan(runsDir, days, now = /* @__PURE__ */ new Date()) {
  ...
  for (const entry of (0, import_node_fs5.readdirSync)(canonicalRunsDir, { withFileTypes: true })) {
    if (!entry.isDirectory() || !isRunId(entry.name)) continue;
    const runDir = (0, import_node_path6.join)(canonicalRunsDir, entry.name);
    if ((0, import_node_fs5.statSync)(runDir).mtimeMs > cutoff) continue;
...
function readExpectedRun2(runJsonPath) {
  const runId = (0, import_node_path6.basename)((0, import_node_path6.dirname)(runJsonPath));
  return readRunRecordAt((0, import_node_path6.dirname)((0, import_node_path6.dirname)(runJsonPath)), runId);
}
...
async function runRunRecordCli(argv, environment = process.env, write = (line) => console.log(line)) {
  ...
  if (parsed.command === "list") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--chain"]));
    const chain = optional(parsed.values, "--chain");
    const records = ((0, import_node_fs6.existsSync)(runsDir) ? (0, import_node_fs6.readdirSync)(runsDir, { withFileTypes: true }) : []).filter((entry) => entry.isDirectory() && entry.name.startsWith("run-")).map((entry) => readRunRecordAt(runsDir, entry.name)).filter((record) => !chain || record.chain === chain);
    write(JSON.stringify(records));
    return;
  }
  if (parsed.command === "count-running") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--exclude-run-id"]));
    write(String(countRunningRuns(runsDir, optional(parsed.values, "--exclude-run-id"))));
    return;
  }
  if (parsed.command === "delete-user-runs") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--user-id"]));
    for (const path2 of deleteRunsOwnedByUser(runsDir, required(parsed.values, "--user-id"))) write(path2);
    return;
  }
  if (parsed.command === "cleanup-old-runs") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--days"]));
    for (const path2 of deleteRunsOlderThan(runsDir, nonNegativeInteger(parsed.values, "--days"))) write(path2);
    return;
  }
  ...
  if (parsed.command === "inspect") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id"]));
  ...
  if (parsed.command === "workspace-path" || parsed.command === "started-at" || parsed.command === "completed-at") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id"]));
    write(parsed.command === "workspace-path" ? runWorkspacePath(runJsonPath) : parsed.command === "started-at" ? runStartedAt(runJsonPath) : runCompletedAt(runJsonPath));
    return;
  }
  ...
  if (parsed.command === "set-status") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--status", "--message"]));
  ...
  if (parsed.command === "set-agent-status") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--agent-id", "--status"]));
    const status = required(parsed.values, "--status");
  ...
  if (parsed.command === "mark-agent-blocked" || parsed.command === "mark-agent-failed") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--agent-id", "--reason"]));
    const agentId = required(parsed.values, "--agent-id");
    const reason = required(parsed.values, "--reason");
    const run = parsed.command === "mark-agent-blocked" ? markRunAgentBlocked(runJsonPath, agentId, reason) : markRunAgentFailed(runJsonPath, agentId, reason);
    write(JSON.stringify(run));
    return;
  }
  if (parsed.command === "activity-manifest") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--agent-id"]));
  ...
  if (parsed.command === "peer-start") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--first-session", "--second-session"]));
  ...
  if (parsed.command === "peer-complete") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--rounds"]));
  ...
  if (parsed.command === "sync-task") {
    rejectUnexpected(parsed, /* @__PURE__ */ new Set(["--runs-dir", "--run-id", "--status"]));
    const status = required(parsed.values, "--status");
...
function configuredRunsDir(flagValue, environmentValue) {
  ...
  const canonicalFlag = flag ? canonicalizeRunsDir(flag) : void 0;
  const canonicalEnvironment = environment ? canonicalizeRunsDir(environment) : void 0;
...
function present(values, flag) {
  const value = values.get(flag);
  if (value === void 0) throw new Error(`Missing required run record argument: ${flag}`);
  return value;
}
...
function chainNameFromFile(path2) {
  const value = JSON.parse((0, import_node_fs6.readFileSync)(path2, "utf8"));
  if (typeof value !== "object" || value === null || Array.isArray(value)) {
    throw new Error(`Chain file must contain a JSON object: ${path2}`);
  }
  const name = value.name;
  if (typeof name !== "string" || name.trim() === "") {
    throw new Error(`Chain file must contain a non-empty name: ${(0, import_node_path7.basename)(path2)}`);
  }
  return name;
}
