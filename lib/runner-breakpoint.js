...
// lib/runner-v2/breakpoint-cli.ts
var breakpoint_cli_exports = {};
__export(breakpoint_cli_exports, {
  runBreakpointCli: () => runBreakpointCli
});
module.exports = __toCommonJS(breakpoint_cli_exports);

// lib/runs/breakpoint-store.ts
...
function tryAcquireExclusiveFileClaim(claimDir, options) {
  ...
  (0, import_fs.mkdirSync)((0, import_path2.dirname)(claimDir), { recursive: true });
...
function cleanupOrphanedReleaseQuarantines(claimDir, isAlive, identity, freshMs) {
  const parent = (0, import_path2.dirname)(claimDir);
  const prefix = `${(0, import_path2.basename)(claimDir)}.release-`;
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    const path2 = (0, import_path2.join)(parent, entry);
...
// lib/runs/breakpoint-store.ts
var BREAKPOINT_DIR = config_default.debugDir;
var CLAIM_WAIT_MS = 1e3;
var SEGMENT = /^[A-Za-z0-9][A-Za-z0-9_.-]{0,127}$/;
var BreakpointRecordValidationError = class extends Error {
  constructor(message) {
    super(message);
    this.name = "BreakpointRecordValidationError";
  }
};
function resolveBreakpointPaths(chainId, debugDir = BREAKPOINT_DIR) {
  assertSegment(chainId, "chain id");
  if (!debugDir || !(0, import_node_path.isAbsolute)(debugDir)) throw new BreakpointRecordValidationError("Breakpoint debug directory must be absolute.");
  (0, import_node_fs.mkdirSync)(debugDir, { recursive: true, mode: 448 });
  const canonicalDebugDir = (0, import_node_fs.realpathSync)(debugDir);
  const chainDir = contained(canonicalDebugDir, chainId);
  if ((0, import_node_fs.existsSync)(chainDir) && (0, import_node_fs.lstatSync)(chainDir).isSymbolicLink()) {
...
function loadBreakpoints(chainId, debugDir = BREAKPOINT_DIR) {
  const { recordPath } = resolveBreakpointPaths(chainId, debugDir);
  if (!pathPresent(recordPath)) return emptyState(chainId);
  return parseBreakpointState((0, import_node_fs.readFileSync)(recordPath, "utf8"), chainId);
}
function shouldPause(chainId, agentId, debugDir = BREAKPOINT_DIR) {
...
function pauseAt(chainId, agentId, debugDir = BREAKPOINT_DIR) {
  assertSegment(agentId, "agent id");
  return mutateBreakpoints(chainId, debugDir, (state) => {
    return {
      ...state,
      pausedAt: agentId,
      pausedAtTimestamp: (/* @__PURE__ */ new Date()).toISOString(),
      resumeRequested: false,
      breakpoints: state.breakpoints.map((candidate) => candidate.agentId === agentId ? { ...candidate, hitCount: (candidate.hitCount ?? 0) + 1 } : candidate)
    };
  });
}
function breakpointRecordExists(chainId, debugDir = BREAKPOINT_DIR) {
  return pathPresent(resolveBreakpointPaths(chainId, debugDir).recordPath);
}
function consumeResumeRequest(chainId, debugDir = BREAKPOINT_DIR) {
  ...
  mutateBreakpoints(chainId, debugDir, (state) => {
    ...
    return consumed ? { ...state, resumeRequested: false, pausedAt: void 0, pausedAtTimestamp: void 0 } : state;
  });
  return consumed;
}
function mutateBreakpoints(chainId, debugDir, mutation) {
  const paths = resolveBreakpointPaths(chainId, debugDir);
  return withExclusiveFileClaim(`${paths.recordPath}.lock`, () => {
    const current = pathPresent(paths.recordPath) ? parseBreakpointState((0, import_node_fs.readFileSync)(paths.recordPath, "utf8"), chainId) : emptyState(chainId);
    return writeBreakpointState(paths, validateBreakpointState(mutation(clone(current)), chainId));
  }, { waitTimeoutMs: CLAIM_WAIT_MS });
}
function writeBreakpointState(paths, state) {
  (0, import_node_fs.mkdirSync)(paths.chainDir, { recursive: true, mode: 448 });
  if ((0, import_node_fs.lstatSync)(paths.chainDir).isSymbolicLink()) throw new BreakpointRecordValidationError("Breakpoint chain directory must not be a symlink.");
  if (pathPresent(paths.recordPath)) assertRegularFile(paths.recordPath);
  const published = { ...state, lastUpdated: (/* @__PURE__ */ new Date()).toISOString() };
  const temporary = (0, import_node_path.join)(paths.chainDir, `.${(0, import_node_path.basename)(paths.recordPath)}.${process.pid}.${(0, import_node_crypto.randomUUID)()}.tmp`);
  (0, import_node_fs.writeFileSync)(temporary, `${JSON.stringify(published, null, 2)}
`, { encoding: "utf8", mode: 384, flag: "wx" });
  (0, import_node_fs.renameSync)(temporary, paths.recordPath);
  return published;
}
function parseBreakpointState(raw, expectedChainId) {
  let value;
  try {
    value = JSON.parse(raw);
  } catch {
    throw new BreakpointRecordValidationError("Breakpoint record contains invalid JSON.");
  }
  return validateBreakpointState(value, expectedChainId);
}
function validateBreakpointState(value, expectedChainId) {
...
function validateBreakpoint(value) {
  ...
  if (hitCount !== void 0 && (typeof hitCount !== "number" || !Number.isSafeInteger(hitCount) || hitCount < 0)) throw new BreakpointRecordValidationError("Breakpoint hitCount must be a non-negative integer.");
  return { agentId, enabled: record.enabled, ...condition ? { condition } : {}, ...hitCount === void 0 ? {} : { hitCount } };
}
function emptyState(chainId) {
  return { chainId, breakpoints: [], resumeRequested: false, lastUpdated: (/* @__PURE__ */ new Date()).toISOString() };
}
function clone(value) {
  return JSON.parse(JSON.stringify(value));
}
function object(value, label) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new BreakpointRecordValidationError(`${label} must be an object.`);
  return value;
}
function string(value, label) {
  if (typeof value !== "string" || !value.trim()) throw new BreakpointRecordValidationError(`${label} must be a non-empty string.`);
  return value;
}
...
function timestamp(value, label) {
  ...
  if (Number.isNaN(Date.parse(result))) throw new BreakpointRecordValidationError(`${label} must be an ISO timestamp.`);
  return result;
}
function optionalTimestamp(value, label) {
  return value === void 0 ? void 0 : timestamp(value, label);
}
function rejectUnknown(record, allowed, label) {
  for (const key of Object.keys(record)) if (!allowed.includes(key)) throw new BreakpointRecordValidationError(`${label} has unsupported field ${key}.`);
}
function assertSegment(value, label) {
  if (!SEGMENT.test(value)) throw new BreakpointRecordValidationError(`Invalid ${label}.`);
}
function contained(root, child) {
  ...
  if (!rel || rel.startsWith("..") || (0, import_node_path.isAbsolute)(rel)) throw new BreakpointRecordValidationError("Breakpoint path escapes its debug root.");
  return path2;
}
function assertRegularFile(path2) {
  const stat = (0, import_node_fs.lstatSync)(path2);
  if (!stat.isFile() || stat.isSymbolicLink()) throw new BreakpointRecordValidationError("Breakpoint record must be a regular file.");
}
...
// lib/runner-v2/breakpoint-cli.ts
function runBreakpointCli(argv, write = (line) => console.log(line)) {
  ...
  rejectUnexpected(values, /* @__PURE__ */ new Set(["--chain-id", "--debug-dir", "--agent-id"]));
  if (command === "check") {
  ...
  if (!breakpointRecordExists(chainId, debugDir)) {
    write("missing");
    return;
  }
...
function rejectUnexpected(values, allowed) {
  for (const key of values.keys()) if (!allowed.has(key)) throw new Error(`${key} is not valid for runner-breakpoint`);
}
function isCommand(value) {
  return value === "check" || value === "pause" || value === "consume-resume";
}
function usage() {
  return "usage: runner-breakpoint <check|pause|consume-resume> --chain-id <id> --debug-dir <absolute-path> [--agent-id <id>]";
}
if (require.main === module) {
  try {
    runBreakpointCli(process.argv.slice(2));
  } catch (error) {
