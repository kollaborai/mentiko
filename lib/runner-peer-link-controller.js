#!/usr/bin/env node
// GENERATED FROM web/lib/links/peer-link-controller-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
// lib/links/peer-link-controller-cli.ts
var import_node_fs5 = require("node:fs");

// lib/links/peer-link-controller.ts
...
function startPeerRun(runJsonPath, firstSession, secondSession) {
  return updateRunJson(runJsonPath, (value2) => {
    const current = requireCurrent(value2, runJsonPath);
...
// lib/links/peer-link-controller.ts
var liveDependencies = {
  transport: pty,
  sleep: (ms) => new Promise((resolve3) => setTimeout(resolve3, ms)),
  relay(command, capture) {
    const result = (0, import_node_child_process.spawnSync)("/bin/sh", ["-lc", command], { input: capture, encoding: "utf8", timeout: 12e4 });
    return result.status === 0 && result.stdout.trim() ? result.stdout.trim() : capture;
  }
};
async function runPeerLinkController(context, partial = {}) {
  const dependencies = { ...liveDependencies, ...partial };
  const profilesDir = (0, import_node_path4.join)(resolveOrgRoot(context), "agent-profiles");
  const firstProfile = context.agent1Profile ? resolveExactProfile(profilesDir, context.agent1Profile) : resolveDefaultProfile(profilesDir);
  const secondProfile = context.agent2Profile ? resolveExactProfile(profilesDir, context.agent2Profile) : firstProfile;
  ...
  const outputDir = (0, import_node_path4.join)(resolveNamespaceRoot(context), "peer-output");
  const meetingDir = (0, import_node_path4.join)(resolveNamespaceRoot(context), "peer-escalations", context.managerSession);
  const meetingPath = (0, import_node_path4.join)(meetingDir, "meeting.json");
  const replyPath = (0, import_node_path4.join)(meetingDir, "reply.txt");
  const runJsonPath = (0, import_node_path4.join)(context.runDir, "run.json");
  (0, import_node_fs4.mkdirSync)(outputDir, { recursive: true });
  (0, import_node_fs4.mkdirSync)(meetingDir, { recursive: true });
  (0, import_node_fs4.writeFileSync)(meetingPath, JSON.stringify({
  ...
  await dependencies.transport.sendKeys(firstSession, `cd ${shellQuote3(context.workspacePath)} && ${firstCommand}`);
  await dependencies.transport.sendKeys(secondSession, `cd ${shellQuote3(context.workspacePath)} && ${secondCommand}`);
  ...
  const maxRounds = context.maxRounds === 0 ? Number.POSITIVE_INFINITY : Math.max(1, context.maxRounds ?? 20);
  let rounds = 0;
  let steerMessage = "";
  try {
    while (rounds < maxRounds) {
      ...
      (0, import_node_fs4.writeFileSync)(meetingPath, JSON.stringify({ ...JSON.parse((0, import_node_fs4.readFileSync)(meetingPath, "utf8")), round: rounds }, null, 2));
...
function resolveNamespaceRoot(context) {
  if (process.env.MENTIKO_NAMESPACE_ROOT) return process.env.MENTIKO_NAMESPACE_ROOT;
  return context.orgId === "default" ? (0, import_node_path4.dirname)(context.runsDir) : (0, import_node_path4.dirname)((0, import_node_path4.dirname)((0, import_node_path4.dirname)(context.runsDir)));
}
function resolveOrgRoot(context) {
  return context.orgId === "default" ? resolveNamespaceRoot(context) : (0, import_node_path4.join)(resolveNamespaceRoot(context), "orgs", context.orgId);
}
async function waitForStable(session, dependencies) {
  let previous = "";
  let stable = 0;
  for (let attempt = 0; attempt < 180; attempt += 1) {
    ...
    if ((0, import_node_crypto.createHash)("sha256").update(capture).digest("hex") === previous) stable += 1;
    else {
      previous = (0, import_node_crypto.createHash)("sha256").update(capture).digest("hex");
      stable = 0;
    }
...
function persistPeerOutput(dir, session, round, content) {
  (0, import_node_fs4.writeFileSync)((0, import_node_path4.join)(dir, `${safeName(session)}-r${round}-${Date.now()}.txt`), content);
}
function copyPeerOutputsToArtifacts(outputDir, runDir, first, second) {
  const artifactsDir = (0, import_node_path4.join)(runDir, "artifacts");
  (0, import_node_fs4.mkdirSync)(artifactsDir, { recursive: true });
  for (const [session, name] of [[first, "agent1-output.txt"], [second, "agent2-output.txt"]]) {
    const prefix = `${safeName(session)}-`;
    const files = (0, import_node_fs4.existsSync)(outputDir) ? (0, import_node_fs4.readdirSync)(outputDir).filter((file) => file.startsWith(prefix)).sort() : [];
    (0, import_node_fs4.writeFileSync)((0, import_node_path4.join)(artifactsDir, name), files.map((file) => (0, import_node_fs4.readFileSync)((0, import_node_path4.join)(outputDir, file), "utf8")).join("\n"));
  }
}
function consumeReply(path2) {
  if (!(0, import_node_fs4.existsSync)(path2)) return "";
  const reply = (0, import_node_fs4.readFileSync)(path2, "utf8").trim();
  (0, import_node_fs4.unlinkSync)(path2);
  return /^(?:continue|c|go)$/i.test(reply) ? "" : reply;
}
function appendSteerMessage(relay, steerMessage) {
  return steerMessage ? `${relay}

Also, one more thing: ${steerMessage}` : relay;
}
function markEscalated(runJsonPath, round, trigger) {
  updateRunJson(runJsonPath, (run) => {
    if (!run) throw new Error(`missing link run: ${runJsonPath}`);
    const escalations = Array.isArray(run.escalations) ? run.escalations : [];
    return { ...run, status: "stalled", escalations: [...escalations, { id: `esc-${Date.now()}`, round, trigger, created_at: (/* @__PURE__ */ new Date()).toISOString() }] };
  });
}
function safeName(value2) {
  return value2.replace(/[^A-Za-z0-9._-]/g, "-");
}
function shellQuote3(value2) {
  return `'${value2.replace(/'/g, "'\\''")}'`;
}
function isDone(value2) {
  return /STATUS:DONE\s*$/m.test(value2);
}
...
async function main() {
  ...
  await runPeerLinkController(JSON.parse((0, import_node_fs5.readFileSync)(contextPath, "utf8")));
}
main().catch((error) => {
