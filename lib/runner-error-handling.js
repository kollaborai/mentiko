#!/usr/bin/env node
// GENERATED FROM web/lib/runner-v2/error-handling-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
function runRecordSchemaPath() {
  const codeRoot = process.env.MENTIKO_CODE_ROOT || (0, import_node_path3.resolve)(process.cwd(), "..");
  const libDir = process.env.LIB_DIR || (0, import_node_path3.join)(codeRoot, "lib");
...
function detectAgentError(reportFile) {
  if (!(0, import_node_fs4.existsSync)(reportFile)) return 0;
  assertRegularFile(reportFile, "Agent report");
  const text = (0, import_node_fs4.readFileSync)(reportFile, "utf8");
...
function scheduleChainRunner(chainFile, agentId, delaySeconds) {
  ...
  const bundle = (0, import_node_path4.resolve)(process.argv[1] || "");
  if (!bundle || !(0, import_node_path4.isAbsolute)(bundle)) throw new Error("Typed error-handling bundle path is unavailable for dispatch.");
  const child = (0, import_node_child_process.spawn)(process.execPath, [bundle, "dispatch", "--delay-seconds", String(delaySeconds), "--chain-file", chainFile, "--agent-id", agentId], {
    detached: true,
    stdio: "ignore",
    env: process.env
  });
...
function dispatchChainRunner(chainFile, agentId, delaySeconds) {
  ...
  return new Promise((resolveDispatch) => {
    setTimeout(() => {
      ...
      const child = (0, import_node_child_process.spawn)(process.execPath, [plan.launcher, ...plan.args], {
        detached: true,
        stdio: "ignore",
        env: plan.env
      });
...
function buildTypedRelaunchPlan(chainFile, agentId, bundlePath = (0, import_node_path4.resolve)(process.argv[1] || "")) {
  assertExternalPath(chainFile, "chain file");
  if (!agentId.trim()) throw new Error("agent id must not be empty");
  ...
  const runDir = process.env.MENTIKO_RUN_DIR || (0, import_node_path4.dirname)(chainFile);
  if (!(0, import_node_path4.isAbsolute)(runDir)) throw new Error(`Typed relaunch run directory must be absolute: ${runDir}`);
  return {
...
function resolveTypedLauncher(bundlePath) {
  if (!bundlePath || !(0, import_node_path4.isAbsolute)(bundlePath)) throw new Error("Typed error-handling bundle path is unavailable for typed launch dispatch.");
  const compiled = (0, import_node_path4.join)((0, import_node_path4.dirname)(bundlePath), "runner-v2-launch-agent.js");
  if (!(0, import_node_fs4.existsSync)(compiled) || !(0, import_node_fs4.statSync)(compiled).isFile()) {
    throw new Error(`Typed runner-v2 launch-agent bundle missing: ${compiled}`);
  }
  return compiled;
}
function authorizeTypedRelaunch(runJsonPath, runId) {
  if (!(0, import_node_fs4.existsSync)(runJsonPath)) return;
  try {
    updateRunJson(runJsonPath, (current) => {
      ...
      return { ...current, resumedAt: (/* @__PURE__ */ new Date()).toISOString() };
    });
  } catch {
  }
}
...
function errorDetailsFromReport(reportFile, errorType2) {
  if (!(0, import_node_fs4.existsSync)(reportFile)) return `Agent ${errorType2}`;
  assertRegularFile(reportFile, "Agent report");
  const line = (0, import_node_fs4.readFileSync)(reportFile, "utf8").split(/\r?\n/).find((candidate) => /error|failed|exception/i.test(candidate));
...
function assertExternalPath(path, label) {
  if (!path || !(0, import_node_path4.isAbsolute)(path)) throw new Error(`${label} path must be absolute.`);
}
...
async function runErrorHandlingCli(argv, write = console.log) {
  ...
  if (command === "dispatch") {
    rejectUnexpected(values, /* @__PURE__ */ new Set(["--delay-seconds", "--chain-file", "--agent-id"]));
    await dispatchChainRunner(required(values, "--chain-file"), required(values, "--agent-id"), number(values, "--delay-seconds"));
    return 0;
  }
  rejectUnexpected(values, /* @__PURE__ */ new Set(["--state-dir", "--run-id", "--session-prefix", "--agent-id", "--error-type", "--report-file", "--chain-file", "--agents-dir"]));
