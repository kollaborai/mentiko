...
function emitLocally(argv) {
  // Strip --web-url (and its value): it is a global flag for opsRequest, not for
  // the local typed writer, which rejects unknown arguments.
  const clean = [];
  for (let i = 0; i < argv.length; i += 1) {
    if (argv[i] === "--web-url") { i += 1; continue; }
    clean.push(argv[i]);
  }
  const child = spawn(process.execPath, [LOCAL_EMITTER, ...clean], { stdio: "inherit" });
...
async function main() {
  ...
  try {
    ...
    const code = await emitLocally(argv);
