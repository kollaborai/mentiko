...
const KOLLAB_PIPE_TIMEOUT = "8min";
...
function hasTimeoutArg(args) {
  return args.some((arg) => arg === "--timeout" || String(arg).startsWith("--timeout="));
}

function ensureKollabPipeTimeout(cli, args) {
  if (!isKollabCli(cli) || hasTimeoutArg(args)) return args;
  return [...args, "--timeout", KOLLAB_PIPE_TIMEOUT];
}

function hasExplicitKollabProfileArg(args) {
  return args.some((arg, idx) => {
    const value = String(arg);
    return value === "--profile" ? Boolean(args[idx + 1]) : value.startsWith("--profile=");
  });
}
...
function kollabProfileEnvPrefix(profileName) {
  const normalized = String(profileName || "")
    .trim()
    .replace(/[^a-zA-Z0-9]/g, "_")
    .toUpperCase();
  return normalized ? `KOLLAB_${normalized}_` : "";
}

function usableProfileEnvValue(value) {
  return typeof value === "string" && value.trim() && !SECRET_REF_PATTERN.test(value)
    ? value.trim()
    : "";
}

function resolveKollabProviderEnv(profileEnv) {
  if (usableProfileEnvValue(profileEnv.ANTHROPIC_AUTH_TOKEN) || usableProfileEnvValue(profileEnv.ANTHROPIC_API_KEY)) {
    return {
      provider: "anthropic",
      apiKey: usableProfileEnvValue(profileEnv.ANTHROPIC_AUTH_TOKEN) || usableProfileEnvValue(profileEnv.ANTHROPIC_API_KEY),
      baseUrl: usableProfileEnvValue(profileEnv.ANTHROPIC_BASE_URL),
    };
  }
  if (usableProfileEnvValue(profileEnv.OPENAI_API_KEY)) {
    return {
      provider: "openai",
      apiKey: usableProfileEnvValue(profileEnv.OPENAI_API_KEY),
      baseUrl: usableProfileEnvValue(profileEnv.OPENAI_BASE_URL) || usableProfileEnvValue(profileEnv.OPENAI_API_BASE),
    };
  }
  if (usableProfileEnvValue(profileEnv.GEMINI_API_KEY) || usableProfileEnvValue(profileEnv.GOOGLE_API_KEY)) {
    return {
      provider: "gemini",
      apiKey: usableProfileEnvValue(profileEnv.GEMINI_API_KEY) || usableProfileEnvValue(profileEnv.GOOGLE_API_KEY),
      baseUrl: "",
    };
  }
  if (usableProfileEnvValue(profileEnv.OPENROUTER_API_KEY)) {
    return {
      provider: "openrouter",
      apiKey: usableProfileEnvValue(profileEnv.OPENROUTER_API_KEY),
...
function applyKollabProfileEnv(childEnv, cli, args, profileEnv) {
  if (!isKollabCli(cli)) return;
  const profileName = getArgValue(args, "--profile");
  const prefix = kollabProfileEnvPrefix(profileName);
  if (!prefix) return;

  const providerEnv = resolveKollabProviderEnv(profileEnv);
  if (!providerEnv?.apiKey) return;

  childEnv[`${prefix}API_KEY`] = providerEnv.apiKey;
  childEnv[`${prefix}PROVIDER`] = providerEnv.provider;
  if (providerEnv.baseUrl) {
    childEnv[`${prefix}BASE_URL`] = providerEnv.baseUrl;
  }
}

function describeCliLaunch(cli, args, env) {
  const details = [];
  ...
  if (env.KOLLAB_NO_AUTO_DETECT === "1") details.push("autodetect off");
...
const resolvedArgs = ensureKollabPipeTimeout(resolvedCli, resolvedProfile.cliArgs);
...
applyKollabProfileEnv(childEnv, resolvedCli, resolvedArgs, profileEnv);
if (isKollabCli(resolvedCli) && hasExplicitKollabProfileArg(resolvedArgs)) {
  childEnv.KOLLAB_NO_AUTO_DETECT = "1";
}

addActivity(describeCliLaunch(resolvedCli, resolvedArgs, childEnv));
