...
function taskApiHeaders({ contentType = false } = {}) {
  const authHeader = `Authorization: Bearer ${process.env.BETTER_AUTH_SECRET || ""}`;
  const headers = [
    `-H ${shellQuote(authHeader)}`,
    `-H ${shellQuote(`x-namespace-id: ${NAMESPACE_ID}`)}`,
    `-H ${shellQuote(`x-org-id: ${ORG_ID}`)}`,
  ];
  if (contentType) {
    headers.push(`-H ${shellQuote("Content-Type: application/json")}`);
  }
  return headers.join(" ");
}
...
export function isGenerationAuditRun(runData) {
  return typeof runData?.metadata?.generationKind === "string" &&
    runData.metadata.generationKind.length > 0;
}

function taskAuditStatus(status) {
  if (status === "completed" || status === "complete") return "complete";
  if (status === "failed" || status === "stopped" || status === "cancelled" || status === "error") return "failed";
  return "running";
}

export function taskAuditMetadataForRun(runData, runId, status) {
  const generationKind = runData?.metadata?.generationKind;
  if (generationKind === "chain_recommendation") {
    return {
      analysis_status: taskAuditStatus(status),
      recommendation_run_id: runId,
      ...(runData.chainId ? { recommendation_chain_id: runData.chainId } : {}),
    };
  }

  if (generationKind === "chain_generation") {
    return {
      generation_status: taskAuditStatus(status),
      generated_chain_run_id: runId,
      ...(runData.chainId ? { generated_chain_source_chain_id: runData.chainId } : {}),
    };
  }
...
function emitEvent(eventName, sourceAgent, data, runId = process.env.MENTIKO_RUN_ID || process.env.RUN_ID || "") {
  const p = paths();
  mkdirSync(p.eventsDir, { recursive: true });
  const ts = new Date();
  const tsStr = ts.toISOString().replace(/[:.]/g, "").slice(0, 15);
  const fileName = runId
    ? `${runId}-${sourceAgent}-${eventName}.event`
    : `${sourceAgent}-${eventName}.event`;
...
function buildInstructions(agent, chain, round, maxRounds) {
  ...
  const runIdLine = runId ? `  run_id: ${runId}\n` : "";
  const eventFileName = runId
    ? `${runId}-${sessionPrefix}-${emits}.event`
    : `${sessionPrefix}-${emits}.event`;
...
${runIdLine}  filename: ${eventFileName}
...
${agent.prompt}`;
    ...
    instructions += `\n\nWHEN COMPLETE:
Write an event file to ${p.eventsDir}/ named ${eventFileName} with:
  event: ${emits}
  source: ${sessionPrefix}
${runIdLine}  filename: ${eventFileName}
...
function loadTaskContext(taskId) {
  ...
  try {
    const output = execSync(
      `curl -sf ${taskApiHeaders()} "${apiBase}/api/tasks/${taskId}"`,
    ...
    try {
      const commentsOut = execSync(
        `curl -sf ${taskApiHeaders()} "${apiBase}/api/tasks/${taskId}/comments"`,
        { encoding: "utf-8", stdio: ["ignore", "pipe", "pipe"] }
      );
...
export class ChainRunner {
  ...
  async launchAgent(agent, round = 1) {
    ...
    const mentikoBin = join(codeRootEnv, "bin", "mentiko");
    const tierEnv = {
      PATH: `${codeRootEnv}/bin:${process.env.PATH || ""}`,
      MENTIKO_BIN: mentikoBin,
      MENTIKO_GLOBAL_ROOT,
      ...
      MENTIKO_JOB_IMPORT_TOKEN: process.env.MENTIKO_JOB_IMPORT_TOKEN || "",
      MENTIKO_GENERATION_JOB_ID: process.env.MENTIKO_GENERATION_JOB_ID || "",
      MENTIKO_GENERATION_KIND: process.env.MENTIKO_GENERATION_KIND || "",
    };
  ...
  _updateTaskNote(note) {
    ...
    try {
      execSync(
        `curl -sf -X PATCH ${taskApiHeaders({ contentType: true })} ` +
        `-d ${jsonPayload({ notes: note })} "${apiBase}/api/tasks/${this.taskId}"`,
        { stdio: "ignore" }
      );
    } catch (err) {
  ...
  _updateTaskSummary(status) {
    ...
    try {
      ...
      const auditMetadata = taskAuditMetadataForRun(runData, this.runId, status);

      if (isGenerationAuditRun(runData)) {
        if (auditMetadata) {
          execSync(
            `curl -sf -X PATCH ${taskApiHeaders({ contentType: true })} ` +
            `-d ${jsonPayload({ metadata: auditMetadata })} "${apiBase}/api/tasks/${this.taskId}"`,
            { stdio: "ignore" }
          );
          log(`  task audit run recorded (${runData.metadata.generationKind})`);
        } else {
          log(`  task execution summary skipped for audit run (${runData.metadata.generationKind})`);
        }
        return;
      }
      ...
      execSync(
        `curl -sf -X POST ${taskApiHeaders({ contentType: true })} ` +
        `-d ${jsonPayload({ text: summaryNote, author: "chain-runner" })} "${apiBase}/api/tasks/${this.taskId}/comments"`,
        { stdio: "ignore" }
      );
      ...
      execSync(
        `curl -sf -X PATCH ${taskApiHeaders({ contentType: true })} ` +
        `-d ${jsonPayload({ metadata })} "${apiBase}/api/tasks/${this.taskId}"`,
        { stdio: "ignore" }
      );
      ...
      if (status === "completed") {
        execSync(
          `curl -sf -X POST ${taskApiHeaders()} "${apiBase}/api/tasks/${this.taskId}/close"`,
          { stdio: "ignore" }
        );
      }
  ...
  async run() {
    ...
    try {
      ...
      while (currentAgent && round <= this.maxRounds && !this._aborted) {
        ...
        if (emits) {
          emitEvent(emits, currentAgent.id, `round=${round}`, this.runId);
