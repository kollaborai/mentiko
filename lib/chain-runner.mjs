...
function loadAgentProfile(profileId) {
  const p = paths();
  const file = join(p.profilesDir, `${profileId}.json`);
...
function buildProfileCommand(profile) {
  ...
  if (profile.permission_flag) cmdParts.push(profile.permission_flag);
 ...
 * priority: agent.agent_profile > chain.default_agent_profile > namespace default
 */
function resolveAgentProfile(agent, chain) {
  // 1. agent-level override
  if (agent.agent_profile) return agent.agent_profile;

  // 2. chain default
  if (chain.default_agent_profile) return chain.default_agent_profile;

  // 3. namespace default -- scan profile files for isDefault=true
  const p = paths();
...
export class ChainRunner {
  ...
  async launchAgent(agent, round = 1) {
    ...
    const profileId = agent.agent_profile || this.chain.default_agent_profile;
    if (profileId) {
    ...
    // legacy fallback
    if (!profileId) {
      const chainCli = this.chain.config?.cli || "claude";
    ...
    const tierEnv = {
      MENTIKO_GLOBAL_ROOT,
      MENTIKO_CODE_ROOT: process.env.MENTIKO_CODE_ROOT || join(dirname(fileURLToPath(import.meta.url)), ".."),
