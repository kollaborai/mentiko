 ...
 *
 * PTY-based session manager for all agent orchestration. Uses:
...
const BRIDGE_PATH = join(__dirname, "pty-bridge.py");

function requireTerminal() {
  try {
    return require("@xterm/headless").Terminal;
  } catch (err) {
    if (err?.code !== "MODULE_NOT_FOUND") throw err;
    const webRequire = createRequire(join(__dirname, "..", "web", "package.json"));
    return webRequire("@xterm/headless").Terminal;
  }
}

const Terminal = requireTerminal();

export function formatRenderedCaptureForTerminalStream(text) {
  return text
    .replace(/\r\n/g, "\n")
    .replace(/\r/g, "\n")
    .replace(/\n/g, "\r\n");
...
class PtySession {
  constructor(name, bridgeProc, opts = {}) {
    ...
    // activity tracking (terminal-agnostic)
    this._lastHash = null;
    this._stableCount = 0;
    ...
    this.proc.stdout.on("data", (data) => {
      ...
      // detect activity from screen changes (terminal-agnostic)
      // hash the visible screen content, not the buffer object
      ...
      const currentHash = screenContent;
      if (currentHash !== this._lastHash) {
        this._lastActivityTime = Date.now();
        this._stableCount = 0;
        if (this._isIdle) {
          this._isIdle = false;
          this.events.emit("activity", { type: "active", at: this._lastActivityTime });
        }
        this._lastHash = currentHash;
      } else {
        this._stableCount++;
        if (this._stableCount > 10 && !this._isIdle) {
          this._isIdle = true;
  ...
  getActivity() {
    return {
      isActive: !this._isIdle,
      lastActivityAt: this._lastActivityTime,
      stableDuration: this._isIdle ? Date.now() - this._lastActivityTime : 0
    };
  }
   ...
   * returns the fully-rendered terminal screen.
   *
  ...
  /**
   * Reconcile a stale-alive session against the real OS process. `exited` is
   * flipped in exactly one place -- the bridge process's Node 'exit' handler --
   * which can be MISSED when the bridge lingers after its agent grandchild dies,
   * is reparented/orphaned, or is killed out-of-band. Without this probe the
   * session reports alive forever, which (a) shows a dead session as alive in the
   * UI and (b) masks its run from the run-reconciler, which trusts this flag and
   * skips any run with a "live" session -- holding a concurrency slot. Probe the
   * real pids and synthesize the exit when the process is provably gone.
   *
   * ESRCH-only: a signalable pid (even a reused one) counts as alive -- a
   * false-alive merely defers reaping, a false-dead would kill a working session.
   * Skips freshly-spawned sessions whose childPid has not yet arrived over stderr.
   * Returns true iff it reaped the session on this call.
   */
  reconcileLiveness(graceMs = 10000) {
    if (this.exited) return false;
    // childPid arrives asynchronously via the bridge's stderr "PID:" line; give a
    // just-spawned session time to report it before judging its liveness.
    if (Date.now() - this.createdAt.getTime() < graceMs) return false;

    const gone = (pid) => {
      if (!pid) return false;
      try { process.kill(pid, 0); return false; } catch (e) { return e.code === "ESRCH"; }
    };
    // Dead if the bridge process itself is gone (whole tree dead, 'exit' missed)
    // or the agent grandchild is gone (agent finished, bridge lingering).
    const bridgeGone = gone(this.bridgePid);
    const agentGone = this.childPid != null && gone(this.childPid);
    if (!bridgeGone && !agentGone) return false;

    this.exited = true;
    if (this.exitCode === null) this.exitCode = -1; // observed via reap, not a real proc exit code
    this.exitedAt = new Date();
    this.events.emit("exit", { exitCode: this.exitCode, signal: null, reaped: true });
    return true;
  }
...
export class PtyManager {
  ...
  spawn(name, cmd, args = [], opts = {}) {
    ...
    const ptyEnv = { ...process.env, ...opts.env };
    delete ptyEnv.CLAUDECODE;
    // Strip BASH_FUNC_* exported functions — bash internals that corrupt child
    // shells when a stale daemon environment propagates them into every session
    for (const key of Object.keys(ptyEnv)) {
      if (key.startsWith('BASH_FUNC_')) delete ptyEnv[key];
    }
  ...
  /**
   * Sweep every session and flip any stale-alive one to exited when its process is
   * provably gone (see PtySession.reconcileLiveness). Cheaper and cleaner than
   * probing inside the hot isAlive()/list() read path. Returns the count reaped.
   */
  reconcileDeadSessions(graceMs) {
    let reaped = 0;
    for (const s of this.sessions.values()) {
      try { if (s.reconcileLiveness(graceMs)) reaped++; } catch { /* never let one session break the sweep */ }
    }
    return reaped;
  }
...
function getSocketDir() {
  return join(process.env.PTY_MANAGER_SOCKET_DIR || homedir(), ".pty-manager");
}
...
function startDaemon() {
  ...
  function listen() {
    ...
    // Periodically reconcile stale-alive sessions. The bridge 'exit' event is the
    // only thing that flips a session to exited, and it can be missed (bridge
    // lingers after its agent dies, reparenting, external kill) -- leaving a dead
    // session flagged alive forever, which masks its run from the run-reconciler.
    // A lightweight ESRCH probe every 30s is cheaper than probing the hot read path.
    const livenessSweep = setInterval(() => {
      try {
        const reaped = mgr.reconcileDeadSessions();
        if (reaped > 0) console.log(`pty-manager: reconciled ${reaped} stale-alive session(s)`);
      } catch (err) {
        console.error("pty-manager: liveness sweep failed:", err);
      }
    }, 30000);
    livenessSweep.unref?.();
        ...
        while ((nl = buf.indexOf("\n")) !== -1) {
          ...
          try {
            ...
            if (req.cmd === "attach") {
              ...
              if (screen) {
                conn.write(formatRenderedCaptureForTerminalStream(screen) + "\r\n");
              }
...
async function handleCommand(mgr, req, daemonStartedAt, config) {
  ...
  switch (cmd) {
    ...
    case "spawn": {
      ...
      // API keys come from agent profiles + secrets vault, not server env
      const safeEnvKeys = ["PATH", "HOME", "USER", "TERM", "LANG", "SHELL", "TMPDIR",
        "NAMESPACE_ID", "AGENT_CHAIN_ROOT", "AGENT_CHAIN_CLI", "PTY_DAEMON",
        // Typed completion must retain the exact bootstrap attempt identity;
        // without it, a long-lived daemon can leak a stale attempt into the
        // monitor and make workspace integration address the wrong node record.
        "MENTIKO_AGENT_ATTEMPT_ID",
        // Typed launch/monitor processes must retain their explicit data-root
        // contract. These are path and scope selectors, never shell code or
        // secret-bearing profile values.
        "MENTIKO_GLOBAL_ROOT", "MENTIKO_CODE_ROOT", "MENTIKO_NAMESPACE_ROOT",
        "MENTIKO_ORG_ROOT", "MENTIKO_PROJECT_ROOT", "RUNS_DIR", "EVENTS_DIR", "STATE_DIR"];
      const safeEnv = {};
...
/**
 * Scan for running daemons on sockets other than the current SOCKET_PATH.
 * Used to give a useful error message when `p list`/`p status` can't find
 * the expected socket (e.g. daemon started with a different @name).
 */
async function discoverOtherDaemons() {
  ...
  for (const sock of socks) {
    const sockFile = join(dir, sock);
    if (sockFile === SOCKET_PATH) continue; // already tried this one
    const name = sock.replace(".sock", "");
    try {
      const res = await sendCommandTo(sockFile, { cmd: "status" });
      if (res.ok) found.push({ name, sockFile, pid: res.status.pid });
    } catch { /* stale or unavailable socket */ }
  }
  return found;
}
...
async function cli() {
  ...
  try {
    ...
    if (command === "status" || command === "list") {
      // The expected socket wasn't found — check if a daemon is running
      // under a different name (e.g. started with @name or a different
      // PTY_MANAGER_SOCKET_DIR).  This was the source of "not running"
      // false-negatives when the daemon was alive at pid 9117.
      const others = await discoverOtherDaemons();
      if (others.length > 0) {
        console.log(`pty-manager: no daemon at expected socket (${SOCKET_PATH})`);
        console.log("found daemon(s) running under a different name:");
        for (const d of others) {
          console.log(`  @${d.name}  pid=${d.pid}  socket=${d.sockFile}`);
          console.log(`  hint: p @${d.name} ${command}`);
        }
      } else {
...
const _basename = process.argv[1] && process.argv[1].split("/").pop();
const isMain =
  _basename && /^(pty-manager(\.mjs)?|pty-mgr|p)$/.test(_basename);
