#!/usr/bin/env node
// GENERATED FROM web/lib/runner-v2/activity-capture-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
// lib/runner-v2/activity-capture-cli.ts
var activity_capture_cli_exports = {};
__export(activity_capture_cli_exports, {
  runActivityCaptureCli: () => runActivityCaptureCli
});
module.exports = __toCommonJS(activity_capture_cli_exports);

// lib/runner-v2/activity-capture.ts
...
function validateAgentProfile(value, path2) {
  if (!isRecord2(value)) throw new Error(`Invalid agent profile JSON at ${path2}`);
...
function readiness(value, path2) {
  if (!isRecord2(value) || typeof value.enabled !== "boolean") throw new Error(`Invalid readiness configuration at ${path2}`);
  return value;
}
function profileEnv(value, path2) {
  if (!isRecord2(value)) throw new Error(`Profile env must be an object at ${path2}`);
...
var GIT_SHA_PATTERN = /^[A-Fa-f0-9]{7,64}$/;
var CONVERSATION_WINDOW_SECONDS = 30;
function captureAgentActivity(input) {
  const agentId = requireAgentId(input.agentId);
  const runId = requireRunId(input.runId);
  const projectRoot2 = requireDirectory(input.projectRoot, "project root");
  ...
  const artifactsDir = (0, import_node_path4.join)(paths.runDir, "artifacts");
  ...
  captureGitActivity({ ...input, agentId, runId, projectRoot: projectRoot2, artifactsDir }, result);
  captureConversations({ ...input, agentId, projectRoot: projectRoot2, artifactsDir }, result);
...
function captureAgentStartProvenance(input) {
  const agentId = requireAgentId(input.agentId);
  const runId = requireRunId(input.runId);
  const projectRoot2 = requireDirectory(input.projectRoot, "project root");
  const runsDir = canonicalizeRunsDir(input.runsDir);
  ...
  const gitProbe = runGit(projectRoot2, ["rev-parse", "HEAD"]);
  const gitSha = gitProbe.status === 0 && GIT_SHA_PATTERN.test(gitProbe.stdout.trim()) ? gitProbe.stdout.trim() : "";
  const gitBeforePath = (0, import_node_path4.join)(artifactsDir, `${agentId}-git-before.txt`);
  const startedAtPath = (0, import_node_path4.join)(artifactsDir, `${agentId}-started-at.txt`);
...
function captureGitActivity(input, result) {
  const beforeShaPath = (0, import_node_path4.join)(input.artifactsDir, `${input.agentId}-git-before.txt`);
  if (!(0, import_node_fs4.existsSync)(beforeShaPath)) {
    result.git.reason = "git-before artifact is absent";
    return;
  }
  assertRegularFile(beforeShaPath, "git-before artifact");
  const beforeSha = (0, import_node_fs4.readFileSync)(beforeShaPath, "utf8").trim();
  ...
  const diffPath = (0, import_node_path4.join)(input.artifactsDir, `${input.agentId}-diff.patch`);
  ...
  const changedOutput = committedNames.length > 0 ? committedNames : `${runGitRequired(input.projectRoot, ["diff", "--name-status", "--staged"])}${runGitRequired(input.projectRoot, ["diff", "--name-status"])}`;
  const changed = dedupeChangedFiles(parseGitNameStatus(changedOutput));
  const changedPath = (0, import_node_path4.join)(input.artifactsDir, `${input.agentId}-files-changed.json`);
...
function captureConversations(input, result) {
  const startedAtPath = (0, import_node_path4.join)(input.artifactsDir, `${input.agentId}-started-at.txt`);
  if (!(0, import_node_fs4.existsSync)(startedAtPath)) {
    result.conversations.reason = "started-at artifact is absent";
    return;
  }
  assertRegularFile(startedAtPath, "started-at artifact");
  const startedAt = Date.parse((0, import_node_fs4.readFileSync)(startedAtPath, "utf8").trim());
  if (!Number.isFinite(startedAt)) throw new Error(`Invalid started-at artifact: ${startedAtPath}`);
  const conversationPath = (0, import_node_path4.join)(input.artifactsDir, `${input.agentId}-conversations.json`);
  const files = input.profileFile ? findConversationFiles(input.profileFile, input.projectRoot, startedAt) : [];
  writeAtomic(conversationPath, `${JSON.stringify(files.map((path2) => ({ path: path2 })), null, 2)}
`);
...
function captureOutput(input, result) {
  if (!input.reportFile) {
    result.output.reason = "report file is absent";
    return;
  }
  if (!(0, import_node_fs4.existsSync)(input.reportFile)) {
    result.output.reason = "report file does not exist";
    return;
  }
  assertRegularFile(input.reportFile, "report file");
  const outputPath = (0, import_node_path4.join)(input.artifactsDir, `${input.agentId}-output.txt`);
  if ((0, import_node_path4.resolve)(input.reportFile) === (0, import_node_path4.resolve)(outputPath)) {
    throw new Error("Report file cannot be the activity output artifact itself.");
  }
  const output = (0, import_node_fs4.readFileSync)(input.reportFile);
...
function updateManifest(input) {
  if (!(0, import_node_fs4.existsSync)(input.runJsonPath)) {
...
function findConversationFiles(profilePath, projectRoot2, startedAtMs) {
  assertRegularFile(profilePath, "agent profile");
  const { cli, logPath } = profileTranscriptConfig(profilePath);
  if (!logPath) return [];
  const expanded = logPath.startsWith("~") ? (0, import_node_path4.join)(process.env.HOME || "", logPath.slice(2)) : logPath;
  const absoluteLogPath = (0, import_node_path4.isAbsolute)(expanded) ? (0, import_node_path4.resolve)(expanded) : (0, import_node_path4.resolve)(projectRoot2, expanded);
  const slug = encodeCwdSlug(cli, projectRoot2);
  const searchDir = slug ? (0, import_node_path4.join)(absoluteLogPath, slug) : absoluteLogPath;
  if (!(0, import_node_fs4.existsSync)(searchDir)) return [];
  assertDirectoryNotSymlink(searchDir, "conversation log directory");
  let candidateDir = searchDir;
  if (cli === "codex") {
    const dateDir = (0, import_node_path4.join)(searchDir, localDatePath(startedAtMs));
    if ((0, import_node_fs4.existsSync)(dateDir)) {
  ...
  const startSeconds = startedAtMs / 1e3;
  const matching = files.filter((path2) => {
    const stamp = fileBirthOrMtime(path2);
  ...
  return files.sort((left, right) => fileBirthOrMtime(right) - fileBirthOrMtime(left)).slice(0, 1);
}
function listJsonlFiles(directory, depth = 0) {
  ...
  for (const entry of (0, import_node_fs4.readdirSync)(directory, { withFileTypes: true })) {
    const path2 = (0, import_node_path4.join)(directory, entry.name);
    const stat = (0, import_node_fs4.lstatSync)(path2);
    if (stat.isSymbolicLink()) continue;
    if (stat.isFile() && entry.name.endsWith(".jsonl")) {
      files.push(path2);
      continue;
    }
    if (stat.isDirectory() && depth < 2) files.push(...listJsonlFiles(path2, depth + 1));
  }
  return files;
}
function fileBirthOrMtime(path2) {
  const stat = (0, import_node_fs4.statSync)(path2);
...
function localDatePath(timestampMs) {
...
function runGit(projectRoot2, args) {
  const result = (0, import_node_child_process.spawnSync)("git", ["-C", projectRoot2, ...args], { encoding: "utf8" });
...
function runGitRequired(projectRoot2, args) {
  const result = runGit(projectRoot2, args);
  if (result.status !== 0) {
...
function writeAtomic(path2, content) {
  ...
  if ((0, import_node_fs4.existsSync)(path2)) {
    const existing = (0, import_node_fs4.lstatSync)(path2);
    if (existing.isSymbolicLink()) throw new Error(`Activity artifact must not be a symbolic link: ${path2}`);
    if (!existing.isFile()) throw new Error(`Activity artifact must be a regular file: ${path2}`);
  }
  const temporaryPath = `${path2}.${process.pid}.${(0, import_node_crypto.randomUUID)()}.tmp`;
  (0, import_node_fs4.writeFileSync)(temporaryPath, content, { flag: "wx", mode: 384 });
  (0, import_node_fs4.renameSync)(temporaryPath, path2);
}
function ensureDirectory(path2, label) {
  if (!(0, import_node_path4.isAbsolute)(path2)) throw new Error(`${label} must be absolute: ${path2}`);
  (0, import_node_fs4.mkdirSync)(path2, { recursive: true });
  assertDirectoryNotSymlink(path2, label);
}
function assertDirectoryNotSymlink(path2, label) {
  const stat = (0, import_node_fs4.lstatSync)(path2);
  if (stat.isSymbolicLink()) throw new Error(`${label} must not be a symbolic link: ${path2}`);
  if (!stat.isDirectory()) throw new Error(`${label} must be a directory: ${path2}`);
}
function assertRegularFile(path2, label) {
  const stat = (0, import_node_fs4.lstatSync)(path2);
  if (stat.isSymbolicLink()) throw new Error(`${label} must not be a symbolic link: ${path2}`);
  if (!stat.isFile()) throw new Error(`${label} must be a regular file: ${path2}`);
}
function requireDirectory(path2, label) {
  if (!path2 || !(0, import_node_path4.isAbsolute)(path2)) throw new Error(`${label} must be an absolute path.`);
  if (!(0, import_node_fs4.existsSync)(path2)) throw new Error(`${label} does not exist: ${path2}`);
  assertDirectoryNotSymlink(path2, label);
  return (0, import_node_path4.resolve)(path2);
}
...
// lib/runner-v2/activity-capture-cli.ts
function runActivityCaptureCli(argv, write = (line) => console.log(line)) {
...
function parseValues(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (!key?.startsWith("--") || value === void 0 || values.has(key)) {
