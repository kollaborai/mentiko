#!/usr/bin/env node
// GENERATED FROM web/lib/runner-v2/standalone-agent-launch-cli.ts - DO NOT EDIT. Rebuild: node scripts/build-runner-bundles.mjs
"use strict";
...
// lib/runner-v2/standalone-agent-launch-cli.ts
var standalone_agent_launch_cli_exports = {};
__export(standalone_agent_launch_cli_exports, {
  parseStandaloneAgentLaunchArgs: () => parseStandaloneAgentLaunchArgs
});
module.exports = __toCommonJS(standalone_agent_launch_cli_exports);
...
// lib/runner-v2/standalone-agent-launch.ts
...
function removeDirectoryWithRetries(path3, attemptRemoval = (target) => {
  (0, import_fs3.rmSync)(target, { recursive: true, force: true });
}) {
  for (let attempt = 0; ; attempt += 1) {
    try {
      attemptRemoval(path3);
      return;
    } catch (error) {
...
function cleanupOrphanedReleaseQuarantines(claimDir, isAlive, identity, freshMs) {
  ...
  for (const entry of entries) {
    ...
    const path3 = (0, import_path4.join)(parent, entry);
    const owner = readOwner(path3);
    if (owner && ownerIsAlive(owner, isAlive, identity) || !owner && claimAgeMs(path3) < freshMs) continue;
    try {
      removeDirectoryWithRetries(path3);
    } catch {
    }
  }
}
...
function createRunnerAgentState(path3, input) {
  const state = {
    ...input,
    status: "running",
    retry_attempt: input.retry_attempt ?? "0"
  };
  writeRunnerAgentState(path3, state);
  return state;
}
function readRunnerAgentState(path3) {
  if (!(0, import_node_fs.existsSync)(path3)) return null;
  if ((0, import_node_fs.lstatSync)(path3).isSymbolicLink()) throw new Error(`Runner agent state must not be a symbolic link: ${path3}`);
  return parseRunnerAgentState((0, import_node_fs.readFileSync)(path3, "utf8"), (0, import_node_path.basename)(path3));
}
function updateRunnerAgentState(path3, update) {
  return withExclusiveFileClaim(`${path3}.lock`, () => {
    const current = readRunnerAgentState(path3);
    if (!current) throw new Error(`Runner agent state does not exist: ${path3}`);
    const next = update(current);
    writeRunnerAgentStateUnlocked(path3, next);
    return next;
  }, { waitTimeoutMs: 5e3 });
}
function transitionRunnerAgentState(path3, status, reason, at = (/* @__PURE__ */ new Date()).toISOString()) {
  return updateRunnerAgentState(path3, (current) => {
...
function writeRunnerAgentState(path3, state) {
  withExclusiveFileClaim(`${path3}.lock`, () => writeRunnerAgentStateUnlocked(path3, state), { waitTimeoutMs: 5e3 });
}
function writeRunnerAgentStateUnlocked(path3, state) {
  validateRunnerAgentState(state);
  const directory = (0, import_node_path.dirname)(path3);
  ...
  const temporaryPath = `${path3}.${process.pid}.${Date.now()}.tmp`;
  (0, import_node_fs.writeFileSync)(temporaryPath, serializeRunnerAgentState(state), { mode: 384 });
  (0, import_node_fs.renameSync)(temporaryPath, path3);
}
...
function requireAbsoluteStateDir(path3) {
  if (!path3 || !path3.startsWith("/")) throw new Error("Runner agent state directory must be absolute");
...
function readStandaloneAgentSpec(specPath) {
  const absolute = (0, import_node_path2.resolve)(specPath);
  let stat;
  try {
    stat = (0, import_node_fs2.lstatSync)(absolute);
  } catch {
  ...
  for (const line of (0, import_node_fs2.readFileSync)(absolute, "utf8").split(/\r?\n/)) {
...
function getLogPath(namespaceId2, orgId2) {
  return orgPath(namespaceId2, orgId2, "logs", "system.jsonl");
}
function writeLog(namespaceId2, orgId2, level, source, message, detail) {
  try {
    const logPath = getLogPath(namespaceId2, orgId2);
    const dir = import_path5.default.dirname(logPath);
    if (!(0, import_fs4.existsSync)(dir)) (0, import_fs4.mkdirSync)(dir, { recursive: true });
    const entry = { ts: (/* @__PURE__ */ new Date()).toISOString(), level, source, message };
    if (detail) entry.detail = detail;
    (0, import_fs4.appendFileSync)(logPath, JSON.stringify(entry) + "\n");
  } catch {
  }
}
...
var defaultDependencies = {
  ...
  writeLog,
  exists: import_node_fs3.existsSync,
  lstat: import_node_fs3.lstatSync,
  config: config_default
};
async function launchStandaloneAgent(input, overrides = {}) {
  const dependencies = { ...defaultDependencies, ...overrides };
  const specPath = (0, import_node_path3.resolve)(input.specPath);
  const agent = readStandaloneAgentSpec(specPath);
  const workspacePath = (0, import_node_path3.resolve)(input.workspacePath || dependencies.config.globalRoot);
  ...
  if (input.monitor) {
    ...
    const monitorRuntime = (0, import_node_path3.join)(dependencies.config.libDir, "runner-v2-standalone-monitor.js");
...
function standaloneSessionName(globalRoot2, sessionPrefix, now) {
  const projectName = (0, import_node_path3.basename)((0, import_node_path3.resolve)(globalRoot2));
  const date = [
    now.getFullYear(),
...
function standaloneAgentInstruction(input) {
  return [
    `You are an autonomous AI agent working for ${(0, import_node_path3.basename)(input.workspacePath)}.`,
    "",
...
function runtimePathEnv(runtimeConfig) {
  return {
    ...ptyDaemonEnv(),
...
function resolveStandaloneCli(value, configuredDefault) {
...
function assertRegularFile(path3, label, exists, lstat) {
  if (!exists(path3)) throw new Error(`${label} missing: ${path3}`);
  const stat = lstat(path3);
  if (stat.isSymbolicLink() || !stat.isFile()) throw new Error(`${label} must be a non-symlink regular file: ${path3}`);
}
...
// lib/runner-v2/standalone-agent-launch-cli.ts
function parseStandaloneAgentLaunchArgs(argv) {
...
0 && (module.exports = {
  parseStandaloneAgentLaunchArgs
});
