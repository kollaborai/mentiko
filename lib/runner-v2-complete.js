...
function assertPublicationResult(runWorkspace, baseline, integrationCommit, publication, artifactPath) {
  ...
  const validStatus = (() => {
    switch (publication.status) {
      case "no-changes":
        return artifactPath === publicationArtifactPath(runWorkspace) && integrationTree === runWorkspace.baselineTree && !publication.publishedSnapshot;
      case "source-changed":
        return artifactPath === publicationConflictArtifactPath(
          runWorkspace,
          publication.sourceSnapshot
        ) && !sourceStillAtBaseline(baseline, publication.sourceSnapshot) && !publication.publishedSnapshot;
      case "already-published":
        return artifactPath === publicationArtifactPath(runWorkspace) && validSourceSnapshot(publication.publishedSnapshot) && sourceMatchesIntegrationAfterApply(
          baseline,
          publication.sourceSnapshot,
          integrationTree
      ...
      case "published":
        return artifactPath === publicationArtifactPath(runWorkspace) && validSourceSnapshot(publication.publishedSnapshot) && sourceStillAtBaseline(baseline, publication.sourceSnapshot) && sourceMatchesIntegrationAfterApply(
          baseline,
          publication.publishedSnapshot,
          integrationTree
        );
