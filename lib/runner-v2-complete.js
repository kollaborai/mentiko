...
function removeAgentSessionsAndReleaseCapacity(input) {
  const competingAttempt = readRunnerV2AttemptState(input.runJsonPath).attempts.find((attempt) => attempt.id !== input.attemptId && attempt.runId === input.runId && !isTerminalAgentAttemptPhase(attempt.phase) && (attempt.leaseId === input.sessionName || attempt.processEvidence?.ptySessionId === input.sessionName));
  if (competingAttempt) {
