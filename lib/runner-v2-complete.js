...
function assertPublicationResult(runWorkspace, baseline, integrationCommit, publication, artifactPath) {
  ...
  const validRollback = !publication.rollback || ["reverted", "not-needed", "failed"].includes(publication.rollback.status) && typeof publication.rollback.attemptedAt === "string" && Number.isFinite(Date.parse(publication.rollback.attemptedAt)) && (publication.rollback.status === "failed" ? typeof publication.rollback.error === "string" && publication.rollback.error.length > 0 : publication.rollback.error === void 0);
  if (!validRollback) {
...
function blockSourceWorkspaceChanged(input) {
  ...
  const detail = [
    ...
    ...input.publication.rollback?.status === "reverted" ? ["post-apply race detected; Mentiko result patch was reverted"] : input.publication.rollback?.status === "not-needed" ? ["post-apply race detected; source returned to the task-start snapshot before rollback"] : input.publication.rollback?.status === "failed" ? ["post-apply race detected; automatic rollback was unsafe, so source may contain Mentiko result changes"] : [],
