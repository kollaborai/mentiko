...
__export(entry_code_root_exports, {
...
var import_path10, BUILTIN_PLUGINS_DIR, MARKETPLACE_PLUGINS_DIR;
var init_plugin_registry = __esm({
  "lib/system/plugin-registry.ts"() {
    ...
    init_config();
    init_secrets_store();
    BUILTIN_PLUGINS_DIR = (0, import_path10.join)(config.root, "lib", "plugins");
    MARKETPLACE_PLUGINS_DIR = (0, import_path10.join)(config.globalRoot, "marketplace", "plugins");
  }
});
  ...
  "node_modules/bindings/bindings.js"(exports2, module2) {
    ...
    exports2.getRoot = function getRoot(file) {
      ...
      while (true) {
        ...
        if (exists(join19(dir, "package.json")) || exists(join19(dir, "node_modules"))) {
          return dir;
        }
        if (prev === dir) {
          throw new Error(
...
function externalEffectsLockPath(outboxPath) {
  return (0, import_path11.join)((0, import_path11.dirname)(outboxPath), ".external-effects.lock");
}
...
function enqueueExternalEffectsOnce(outboxPath, records) {
  (0, import_fs11.mkdirSync)((0, import_path11.dirname)(outboxPath), { recursive: true });
  return withExternalEffectsLock(outboxPath, () => {
    ...
    (0, import_fs11.appendFileSync)(outboxPath, missing.map((record) => JSON.stringify({
...
function knownExternalEffectIds(outboxPath) {
  const dir = (0, import_path11.dirname)(outboxPath);
  const paths = [outboxPath, (0, import_path11.join)(dir, "external-effects.dispatch.jsonl")];
  if ((0, import_fs11.existsSync)(dir)) {
    const prefix = `${(0, import_path11.basename)(outboxPath)}.claim-`;
    for (const entry of (0, import_fs11.readdirSync)(dir)) {
      if (entry.startsWith(prefix)) paths.push((0, import_path11.join)(dir, entry));
    }
  }
...
function readJsonlRecords(path2) {
  if (!(0, import_fs11.existsSync)(path2)) return [];
  try {
    return (0, import_fs11.readFileSync)(path2, "utf8").split("\n").filter(Boolean).flatMap((line) => {
      try {
...
var import_path11, import_fs11, ORPHANED_CLAIM_MIN_AGE_MS;
...
function readRunnerV2AttemptState(runJsonPath) {
  if (!(0, import_fs12.existsSync)(runJsonPath)) return { attempts: [] };
  const run = JSON.parse((0, import_fs12.readFileSync)(runJsonPath, "utf8"));
  return {
...
var import_fs12, ALLOWED_TRANSITIONS, TERMINAL_PHASES, NEXT_DESIRED_PHASE, AgentAttemptTransitionError;
  ...
  if (decision.fanIn || decision.waitFor || decision.quorum || decision.onError) {
    return decision.agentIds.map((agentId) => {
      ...
      return {
        ...
        logPath: (0, import_path12.join)(context.runDir, `fanout-${agentId}.log`)
      };
    });
  }
...
function runnerInvocation(context, agentIds) {
  const compiled = (0, import_path12.join)(config_default.codeRoot, "lib", "runner-v2-launch-agent.js");
  const development = (0, import_path12.join)(config_default.codeRoot, "web", "scripts", "runner-v2-launch-agent.cjs");
  return {
...
function applyEffect(effect, context) {
  ...
  if (effect.type === "event-side-effects") {
    ...
    completeFanGroupMemberLocked(context.stateDir, {
      ...
      const chainPath = plan.group.chainPath || (0, import_path13.join)((0, import_path13.dirname)(context.runJsonPath), "chain.json");
      const [launch] = buildRoutedLaunchPlans({
        ...
        chainPath,
        runDir: (0, import_path13.dirname)(context.runJsonPath),
...
function applyGenerationImport(plan, context) {
  const mentikoBin = (0, import_path13.join)(config_default.codeRoot, "bin", "mentiko");
  ...
  appendJsonl((0, import_path13.join)(context.stateDir, "generation-import.jsonl"), {
    jobId: plan.jobId,
...
function startLaunch(launch, context) {
  ...
  const logPath = launch.logPath || (0, import_path13.join)((0, import_path13.dirname)(context.runJsonPath), "launches.log");
  let logFd;
  try {
    logFd = (0, import_fs13.openSync)(logPath, "a");
  } catch {
    logFd = void 0;
  }
  ...
  const args = launch.cli ? [(0, import_fs13.existsSync)(launch.cli.compiledPath) ? launch.cli.compiledPath : launch.cli.developmentPath, ...launch.cli.args] : ["-lc", launch.command];
  ...
  if (logFd !== void 0) (0, import_fs13.closeSync)(logFd);
...
function attemptFingerprint(attempt) {
  return (0, import_crypto4.createHash)("sha256").update(stableSerialize2(attempt)).digest("hex");
}
function routedLaunchAcceptanceKey(runJsonPath, targetAgentIds, env) {
  ...
  const digest = (0, import_crypto4.createHash)("sha256").update(stableSerialize2({ occurrenceId, runId, targetAgentIds: [...targetAgentIds].sort() })).digest("hex").slice(0, 24);
...
function applyEventSideEffects(plan, context) {
  ...
  const expectedArchiveDir = (0, import_path13.join)((0, import_path13.resolve)(context.eventsDir), "archive");
  if (context.eventsArchiveDir && (0, import_path13.resolve)(context.eventsArchiveDir) !== expectedArchiveDir) {
    throw new Error(
...
function bindCompletionOperationIdentity(operation, context) {
  ...
  const digest = (0, import_crypto4.createHash)("sha256").update(stableSerialize2({ occurrenceId, operation: payload })).digest("hex").slice(0, 32);
  return {
    ...operation,
    occurrenceId,
...
function emitTypedEvent(operation, context) {
  ...
  const receiptPath = (0, import_path13.join)(context.stateDir, "completion-event-emissions.jsonl");
...
function markSchedule(operation, context) {
  ...
  const schedulesDir = context.schedulesDir || (0, import_path13.join)(context.stateDir, "schedules");
  const scheduleId = scheduleIdForChain(operation.chainPath);
  const nowSeconds = Math.floor(Date.now() / 1e3);
  const statePath = (0, import_path13.join)(schedulesDir, "state.json");
  const historyPath = (0, import_path13.join)(schedulesDir, `${scheduleId}.history`);
  const idempotencyKey = requiredOperationKey(operation);
  const occurrenceId = requiredOccurrenceId(operation);
  (0, import_fs13.mkdirSync)(schedulesDir, { recursive: true });
...
function clearRetryState(agentId, context) {
  ...
  if ((0, import_fs13.existsSync)(retryPath)) {
    (0, import_fs13.unlinkSync)(retryPath);
  }
}
...
function readTypedRetryAttempt(agentId, context) {
  ...
  const dir = retryDir(context);
  const legacyPath = (0, import_path13.join)(dir, `retry_${sanitizeFilePart(agentId)}.count`);
  ...
  if (!(0, import_fs13.existsSync)(path2)) return void 0;
  let parsed;
  try {
    parsed = JSON.parse((0, import_fs13.readFileSync)(path2, "utf8"));
  } catch (error) {
...
function typedRetryStatePath(agentId, context) {
  ...
  return (0, import_path13.join)(retryDir(context), `retry_${sanitizeFilePart(runId)}_${sanitizeFilePart(agentId)}.json`);
}
...
function circuitStatePath(dir, chainName, agentId) {
  ...
  const identityDigest = (0, import_crypto4.createHash)("sha256").update(stableSerialize2({ chainName, agentId })).digest("hex").slice(0, 24);
  const root = (0, import_path13.resolve)(dir);
  const path2 = (0, import_path13.resolve)(root, `circuit_${chainLabel}_${agentLabel}_${identityDigest}.json`);
  if ((0, import_path13.dirname)(path2) !== root) {
    throw new Error(`circuit breaker path escaped retry root: ${path2}`);
  }
  return path2;
}
function readCircuitState(path2, chainName, agentId) {
  if (!(0, import_fs13.existsSync)(path2)) return {};
  let parsed;
  try {
    parsed = JSON.parse((0, import_fs13.readFileSync)(path2, "utf8"));
  } catch (error) {
...
function dispatchWatchdogHooks(operation, context) {
  const hooksDir = context.hooksDir || (0, import_path13.join)(context.stateDir, "watchdog-hooks");
  (0, import_fs13.mkdirSync)(hooksDir, { recursive: true });
  const dispatchPath = (0, import_path13.join)(hooksDir, "dispatch.jsonl");
  const idempotencyKey = requiredOperationKey(operation);
  const occurrenceId = requiredOccurrenceId(operation);
...
function auditSessionPolicy(operation, context) {
  const auditPath = (0, import_path13.join)(context.stateDir, "session-policy.jsonl");
  const idempotencyKey = requiredOperationKey(operation);
...
function killAgentSessions(sessionName, options = {}) {
  const removed = [];
  const failed = [];
  const transport = (0, import_path13.join)(config_default.codeRoot, "bin", "p");
  ...
  if (failed.length && options.stateDir) {
    appendJsonl((0, import_path13.join)(options.stateDir, "pty-cleanup.jsonl"), {
...
function launchNextChain(operation, context) {
  ...
  if (!identity) {
    appendJsonl((0, import_path13.join)(context.stateDir, "next-chain.jsonl"), {
  ...
  const command = `bash ${shellEscape((0, import_path13.join)(config_default.codeRoot, "lib", "chain-runner.sh"))} ${shellEscape(identity.path)}`;
  const runsDir = context.runsDir || (0, import_path13.dirname)((0, import_path13.dirname)(context.runJsonPath));
  const ledgerPath = (0, import_path13.join)(context.stateDir, "next-chain.jsonl");
  const claimPath = (0, import_path13.join)(context.stateDir, "next-chain-claims", `${nextChainOperationKey(operation.parentRunId, identity)}.claim`);
  return withExclusiveFileClaim(claimPath, () => {
    ...
    let logFd;
    try {
      logFd = (0, import_fs13.openSync)((0, import_path13.join)((0, import_path13.dirname)(context.runJsonPath), "launches.log"), "a");
    } catch {
      logFd = void 0;
    }
    ...
    const child = (0, import_child_process2.spawnSync)("/bin/bash", ["-lc", command], {
...
function resolveNextChainIdentity(chainName, context) {
  const candidate = searchedNextChainPaths(chainName, context).find((path3) => (0, import_fs13.existsSync)(path3));
  if (!candidate) return void 0;
  const path2 = (0, import_fs13.realpathSync)(candidate);
  let parsed = {};
  try {
    parsed = JSON.parse((0, import_fs13.readFileSync)(path2, "utf8"));
  } catch {
...
function nextChainOperationKey(parentRunId, identity) {
  return (0, import_crypto4.createHash)("sha256").update(stableSerialize2({ parentRunId, path: identity.path, name: identity.name, chainId: identity.chainId })).digest("hex").slice(0, 24);
}
function findAcceptedNextChainChild(runsDir, parentRunId, identity) {
  if (!(0, import_fs13.existsSync)(runsDir)) return void 0;
  const matches = [];
  for (const entry of (0, import_fs13.readdirSync)(runsDir, { withFileTypes: true })) {
    if (!entry.isDirectory()) continue;
    const runJsonPath = (0, import_path13.join)(runsDir, entry.name, "run.json");
    if (!(0, import_fs13.existsSync)(runJsonPath)) continue;
    try {
...
function searchedNextChainPaths(chainName, context) {
  return [
    (0, import_path13.join)(context.chainsDir || config_default.chainsDir, chainName, "chain.json"),
    (0, import_path13.join)(context.sharedChainsDir || (0, import_path13.join)(config_default.codeRoot, "chains"), chainName, "chain.json")
  ];
}
...
function queueExternalEffect(operation, context) {
  ...
  enqueueExternalEffectsOnce((0, import_path13.join)(context.stateDir, "external-effects.jsonl"), [{
    idempotencyKey,
...
function externalEffectOperationId(operation, context) {
  ...
  const digest = (0, import_crypto4.createHash)("sha256").update(stableSerialize2(operation)).digest("hex").slice(0, 24);
...
function auditRollbackPlan(operation, context) {
  const auditPath = (0, import_path13.join)(context.stateDir, "rollback-plan.jsonl");
  const idempotencyKey = requiredOperationKey(operation);
  withExclusiveFileClaim(`${auditPath}.lock`, () => {
    if (hasAppliedOperationReceipt(auditPath, idempotencyKey)) return;
    appendJsonl(auditPath, {
...
function listExecutableHooks(hooksDir) {
  try {
    return (0, import_fs13.readdirSync)(hooksDir).filter((name) => name.endsWith(".sh")).map((name) => (0, import_path13.join)(hooksDir, name)).filter((path2) => {
      try {
...
function retryDir(context) {
  return context.retryDir || (0, import_path13.join)(context.stateDir, "retry");
}
function scheduleIdForChain(chainPath) {
  const normalized = chainPath.replace(/\\/g, "/");
  const marker = "/chains/";
  const markerIndex = normalized.indexOf(marker);
  const relative2 = markerIndex >= 0 ? normalized.slice(markerIndex + marker.length) : (0, import_path13.basename)(normalized);
...
function readJsonObject(path2) {
  try {
    return JSON.parse((0, import_fs13.readFileSync)(path2, "utf8"));
  } catch {
    return {};
  }
}
function readOptionalFile(path2) {
  try {
    return (0, import_fs13.readFileSync)(path2, "utf8");
  } catch {
    return "";
  }
}
...
function findEmittedEventByKey(eventsDir, idempotencyKey, expected) {
  for (const root of [eventsDir, (0, import_path13.join)(eventsDir, "archive")]) {
    let filenames;
    try {
      filenames = (0, import_fs13.readdirSync)(root);
    } catch {
      continue;
    }
    for (const filename of filenames) {
      if (!filename.endsWith(".event")) continue;
      const path2 = (0, import_path13.join)(root, filename);
      try {
        const record = parseRunnerEvent((0, import_fs13.readFileSync)(path2, "utf8"));
...
function writeFileAtomic2(path2, content) {
  const tmp = `${path2}.tmp.${process.pid}`;
  (0, import_fs13.writeFileSync)(tmp, content);
  (0, import_fs13.renameSync)(tmp, path2);
}
...
var import_fs13, import_path13, import_child_process2, import_crypto4, GenerationImportError, RoutedLaunchAcceptanceError, DEFAULT_LAUNCH_ACCEPT_TIMEOUT_MS, ACCEPTED_RUNNING_PHASES, ACCEPTED_BLOCKED_PHASES, TERMINAL_ATTEMPT_PHASES, ACCEPTED_TERMINAL_AGENT_STATUSES, REPLAY_SENSITIVE_DIRECT_OPERATIONS;
...
function loopStatePath(runDir) {
  return (0, import_path14.join)(runDir, "chain-loop-state.json");
}
function shellLoopStatePath(runDir) {
  return (0, import_path14.join)(runDir, "chain_loop_tracker.txt");
}
function readLoopState(runDir) {
  const path2 = loopStatePath(runDir);
  const parsed = (0, import_fs14.existsSync)(path2) ? JSON.parse((0, import_fs14.readFileSync)(path2, "utf8")) : {};
...
function restoreLoopMutations(mutations, beforeRestore) {
  ...
  const runDirs = Array.from(new Set(mutations.map((mutation) => (0, import_path14.dirname)(mutation.after.path))));
  if (runDirs.length !== 1) {
...
function writeJsonAtomic3(path2, data, onMutation) {
  ...
  (0, import_fs14.mkdirSync)((0, import_path14.dirname)(path2), { recursive: true });
  const tmp = `${path2}.tmp.${process.pid}`;
...
function readShellVisits(path2) {
  ...
  return (0, import_fs14.readFileSync)(path2, "utf8").split(/\r?\n/).map((line) => line.trim()).filter((line) => /^[^:\s]+:[^\s]+$/.test(line));
}
function writeShellLoopState(path2, visited, onMutation) {
  const before = snapshotLoopFile(path2);
  (0, import_fs14.mkdirSync)((0, import_path14.dirname)(path2), { recursive: true });
  const tmp = `${path2}.tmp.${process.pid}`;
  (0, import_fs14.writeFileSync)(tmp, visited.length > 0 ? `${visited.join("\n")}
` : "");
  (0, import_fs14.renameSync)(tmp, path2);
  onMutation?.({ before, after: snapshotLoopFile(path2) });
}
function snapshotLoopFile(path2) {
  return (0, import_fs14.existsSync)(path2) ? { path: path2, existed: true, content: (0, import_fs14.readFileSync)(path2, "utf8") } : { path: path2, existed: false };
}
function restoreLoopFileAtomic(snapshot) {
  if (!snapshot.existed) {
    (0, import_fs14.rmSync)(snapshot.path, { force: true });
    return;
  }
  (0, import_fs14.mkdirSync)((0, import_path14.dirname)(snapshot.path), { recursive: true });
  const tmp = `${snapshot.path}.restore.${process.pid}.${(0, import_crypto5.randomUUID)()}`;
  (0, import_fs14.writeFileSync)(tmp, snapshot.content || "", { flag: "wx" });
  (0, import_fs14.renameSync)(tmp, snapshot.path);
}
...
var import_crypto5, import_fs14, import_path14;
...
__export(completion_entrypoint_exports, {
...
function runRunnerV2CompletionEntrypoint(input) {
  ...
  const runJsonPath = (0, import_path15.join)(runDir, "run.json");
  if (!(0, import_fs15.existsSync)(runJsonPath)) {
  ...
  const archivedEvents = readEvents((0, import_path15.join)(eventsDir, "archive"));
  ...
  try {
    ...
    const adapter = applyTypedExecutorPlan(plan, {
      ...
      orgId: env.ORG_ID || "default",
      eventsDir,
      eventsArchiveDir: (0, import_path15.join)(eventsDir, "archive"),
      dryRun: input.dryRun,
      onRunMutation
    });
    if (input.dryRun) {
...
function alreadyCompletedGeneration(run, runJsonPath, stateDir, agentId) {
  ...
  const ledgerPath = (0, import_path15.join)(stateDir, "generation-import.jsonl");
  if (!(0, import_fs15.existsSync)(ledgerPath)) return false;
  try {
    return (0, import_fs15.readFileSync)(ledgerPath, "utf8").split("\n").some((line) => {
      if (!line.trim()) return false;
...
function maybeHandleQualityGateFailure(input) {
  const artifactsDir = (0, import_path15.join)(input.runDir, "artifacts");
  const summaryPath = (0, import_path15.join)(artifactsDir, `${input.agent.id}-summary.json`);
  const summary = readJsonObject2(summaryPath);
  ...
  const artifact = !input.dryRun ? runQualityGateEventArtifact({
    ...
    payload: {
      ...
      qualityGate: {
        ...
        summaryPath: (0, import_fs15.existsSync)(summaryPath) ? summaryPath : void 0,
      ...
      evidence: {
        changedFiles: [],
        liveSessions: [],
        artifacts: (0, import_fs15.existsSync)(summaryPath) ? [summaryPath] : []
      }
    }
  }) : { status: "planned" };
  if (!input.dryRun) {
...
function generationImportPlan(run, runDir, agentId, env) {
  ...
  const artifactsDir = (0, import_path15.join)(runDir, "artifacts");
  return {
    jobId,
    generationKind,
    runId: run.id,
...
function hasImportableGenerationPayload(input) {
  if (!(0, import_fs15.existsSync)(input.artifactsDir)) return false;
  let aliases;
  try {
    aliases = (0, import_fs15.readdirSync)(input.artifactsDir).filter((file) => file !== "generation-result.json" && (file.endsWith("-generation-result.json") || file.endsWith("-output.json") || file.endsWith("-result.json")));
  } catch {
    return false;
  }
  const candidates = [
    (0, import_path15.join)(input.artifactsDir, "generation-result.json"),
    ...aliases.map((file) => (0, import_path15.join)(input.artifactsDir, file))
  ];
  for (const path2 of candidates) {
    try {
      if (!(0, import_fs15.existsSync)(path2)) continue;
      const payload = JSON.parse((0, import_fs15.readFileSync)(path2, "utf8"));
      if (!isPayloadCompatibleWithKind(payload, input.generationKind)) continue;
      if (input.notBeforeMs !== void 0 && (0, import_fs15.statSync)(path2).mtimeMs < input.notBeforeMs) return false;
      return true;
    } catch {
      continue;
    }
...
function completionAttemptOccurrenceId(input) {
  const digest = (0, import_crypto6.createHash)("sha256").update(stableSerialize3(input)).digest("hex").slice(0, 32);
...
function readChain(path2) {
  if (!(0, import_fs15.existsSync)(path2)) throw unsupported(`chain not found: ${path2}`);
  const chain = JSON.parse((0, import_fs15.readFileSync)(path2, "utf8"));
...
function resolveRunDir(env, runId) {
  ...
  if (env.RUNS_DIR) return (0, import_path15.join)(env.RUNS_DIR, runId);
...
function ptyOutputChangedSinceMonitorCheck(sessionName, env) {
  ...
  if (!(0, import_fs15.existsSync)(statePath)) return false;
  const previous = (0, import_fs15.readFileSync)(statePath, "utf8").trim();
...
function resolvePtyMgrBin(env) {
  ...
  return codeRoot2 ? (0, import_path15.join)(codeRoot2, "bin", "pty-mgr") : "pty-mgr";
}
...
function monitorCompletionLatchAccepted(env) {
...
function readJsonObject2(path2) {
  if (!(0, import_fs15.existsSync)(path2)) return void 0;
  try {
...
function resolveStateDir(env, runDir) {
  return env.STATE_DIR || (0, import_path15.join)(runDir, "state");
}
...
function readEvents(eventsDir) {
  if (!(0, import_fs15.existsSync)(eventsDir)) return [];
...
var import_child_process3, import_crypto6, import_fs15, import_path15, import_util, RunnerV2CompletionUnsupportedError, RUN_LIFECYCLE_KEYS;
