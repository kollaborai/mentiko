...
function findCodeRootFrom(startDir, maxHops = 8) {
  ...
  for (let hop = 0; hop <= maxHops; hop++) {
    if ((0, import_fs2.existsSync)((0, import_path2.join)(dir, "lib", "runner-v2-direct-run.js"))) return dir;
...
function assertSameFanGroupDefinition(existing, expected) {
  const same = existing.event === expected.event && existing.runId === expected.runId && existing.fanInAgent === expected.fanInAgent && existing.waitFor === expected.waitFor && existing.quorum === expected.quorum && existing.onError === expected.onError && existing.workspacePath === expected.workspacePath && expected.fanOutAgents.every((agentId) => existing.fanOutAgents.includes(agentId));
  if (!same) throw new Error(`fan-group occurrence collision for ${expected.id}`);
}
function completeFanGroupMemberLocked(stateDir, input, acceptLaunch, resolveLaunchWorkspace) {
    ...
    if (plan.launch && resolveLaunchWorkspace) {
      ...
      plan = {
        ...
        group: {
          ...
          ...workspace.workspaceBaseCommit ? { workspaceBaseCommit: workspace.workspaceBaseCommit } : {}
        },
        launch: {
          ...plan.launch,
          env: {
...
function assertFanGroupState(value, expectedId) {
  if (!isRecord(value) || value.id !== expectedId || !isFanGroupStatus(value.status) || typeof value.event !== "string" || !Array.isArray(value.fanOutAgents) || !value.fanOutAgents.every(isNonEmptyString) || new Set(value.fanOutAgents).size !== value.fanOutAgents.length || !isWaitFor(value.waitFor) || !isNonNegativeInteger(value.quorum) || !isNonNegativeInteger(value.completed) || !isNonNegativeInteger(value.failed) || !isNonNegativeInteger(value.total) || value.total !== value.fanOutAgents.length || value.completed + value.failed > value.total || value.fanInAgent !== void 0 && !isNonEmptyString(value.fanInAgent) || value.onError !== void 0 && !isNonEmptyString(value.onError) || value.chainPath !== void 0 && !isNonEmptyString(value.chainPath) || value.runId !== void 0 && !isNonEmptyString(value.runId) || value.workspacePath !== void 0 && !isNonEmptyString(value.workspacePath) || value.workspaceBaseCommit !== void 0 && !isNonEmptyString(value.workspaceBaseCommit) || !isMembers(value.members, value.fanOutAgents, value.completed, value.failed)) {
...
var import_node_fs3, import_node_path3, import_ajv2, import_ajv_formats, RunRecordValidationError, compiledRunRecordValidator, compiledRunRecordSchemaFingerprint;
...
var import_fs10, StaleRunAgentAttemptError, TERMINAL_RUN_STATUSES, TERMINAL_AGENT_STATUSES, UNEXPLAINED_TERMINAL_REASON;
var init_run_state = __esm({
  "lib/runner-v2/run-state.ts"() {
    ...
    StaleRunAgentAttemptError = class extends Error {
      constructor(message) {
        super(message);
    ...
    UNEXPLAINED_TERMINAL_REASON = "terminal state written without a recorded reason (writer did not supply one)";
  }
});
...
// lib/notifications/terminal-notification-key.ts
var init_terminal_notification_key = __esm({
  "lib/notifications/terminal-notification-key.ts"() {
    "use strict";
  }
});
...
var require_bindings = __commonJS({
  "node_modules/bindings/bindings.js"(exports2, module2) {
    ...
    exports2.getRoot = function getRoot(file) {
      var dir = dirname15(file), prev;
      while (true) {
        if (dir === ".") {
          dir = process.cwd();
        }
        if (exists(join24(dir, "package.json")) || exists(join24(dir, "node_modules"))) {
          return dir;
        }
        if (prev === dir) {
          throw new Error(
...
var init_external_effects = __esm({
  "lib/runner-v2/external-effects.ts"() {
    ...
    init_terminal_notification_key();
    init_webhook_utils();
    init_outbound_webhook_delivery();
...
function adoptAgentAttemptForCompletion(input) {
  if (input.attemptId) {
    const exact = readRunnerV2AttemptState(input.runJsonPath).attempts.find(
      (attempt2) => attempt2.id === input.attemptId
    );
...
function resolveAgentAttemptForCompletion(input) {
...
function assertCompletionAttemptIdentity(attempt, input) {
  ...
  const boundSessions = [attempt.leaseId, attempt.processEvidence?.ptySessionId].filter((value) => Boolean(value));
...
function markAgentAttemptCompletedFromDecisionArtifact(input) {
  return markLatestAttemptCompleted({
    ...input,
...
function markLatestAttemptCompleted(input) {
  const attempt = input.attemptId ? readAttempt(input.runJsonPath, input.attemptId) : findLatestAttempt(input.runJsonPath, input.runId, input.agentId);
...
function routedLaunchJobId(input) {
  ...
  const digest2 = (0, import_node_crypto3.createHash)("sha256").update(stableSerialize2({ occurrenceId: input.occurrenceId, runId: input.runId, targetAgentIds })).digest("hex").slice(0, 24);
...
function routedLaunchJobIsAccepted(job) {
...
function parseLaunchJob(value) {
  const job = objectValue2(value);
...
function requireAbsoluteDirectory(path2, field) {
  if (!(0, import_node_path4.isAbsolute)(path2)) throw new WorkspaceSnapshotError(`${field} must be absolute: ${path2}`);
  const resolved = (0, import_node_fs4.realpathSync)((0, import_node_path4.resolve)(path2));
  if (!(0, import_node_fs4.lstatSync)(resolved).isDirectory()) {
    throw new WorkspaceSnapshotError(`${field} must be a directory: ${resolved}`);
  }
  return resolved;
}
function pathWithin(root, candidate) {
  const rel = (0, import_node_path4.relative)(root, candidate);
...
function captureGitWorkspaceSnapshot(input) {
  ...
  const gitCommonDir = requireAbsoluteDirectory(
    (0, import_node_path4.isAbsolute)(gitCommonDirRaw) ? gitCommonDirRaw : (0, import_node_path4.resolve)(repositoryRoot, gitCommonDirRaw),
    "git common directory"
  );
  const relativeWorkspacePath = (0, import_node_path4.relative)(repositoryRoot, sourceWorkspacePath) || ".";
  ...
  const sourceIndexSha256 = (0, import_node_crypto4.createHash)("sha256").update(runGitBytes(repositoryRoot, ["ls-files", "--stage", "-z", "--", relativeWorkspacePath])).digest("hex");
  ...
  const indexPath = (0, import_node_path4.join)(
    scratchDir,
    `.workspace-index-${process.pid}-${(0, import_node_crypto4.randomBytes)(8).toString("hex")}`
  );
  ...
  try {
    ...
    try {
      (0, import_node_fs4.unlinkSync)(indexPath);
    } catch (error) {
...
function compareGitWorkspaceSnapshots(baseline, observed) {
  ...
  return {
    ...
    patchSha256: (0, import_node_crypto4.createHash)("sha256").update(patch).digest("hex")
  };
}
function createWorkspaceSnapshotScratchDir(runDir) {
  if (!(0, import_node_path4.isAbsolute)(runDir)) throw new WorkspaceSnapshotError(`runDir must be absolute: ${runDir}`);
  const scratchDir = (0, import_node_path4.resolve)(runDir, ".internal", "workspace-snapshots");
  (0, import_node_fs4.mkdirSync)(scratchDir, { recursive: true, mode: 448 });
  return scratchDir;
}
var import_node_child_process, import_node_crypto4, import_node_fs4, import_node_path4, SNAPSHOT_VERSION, GIT_TIMEOUT_MS, GIT_MAX_BUFFER, WorkspaceSnapshotError;
...
function requireAbsoluteDirectory2(path2, field) {
  if (!(0, import_node_path5.isAbsolute)(path2)) throw new WorkspaceIsolationError(`${field} must be absolute: ${path2}`);
...
function canonicalGitCommonDir(repositoryRoot) {
  ...
  return requireAbsoluteDirectory2(
    (0, import_node_path5.isAbsolute)(raw) ? raw : (0, import_node_path5.resolve)(repositoryRoot, raw),
    "git common directory"
  );
}
function digest(value, length = 16) {
  return (0, import_node_crypto5.createHash)("sha256").update(value).digest("hex").slice(0, length);
}
...
function readJson(path2) {
  try {
    if ((0, import_node_fs5.lstatSync)(path2).isSymbolicLink()) {
...
function writeJsonOnce(path2, value) {
  (0, import_node_fs5.mkdirSync)((0, import_node_path5.dirname)(path2), { recursive: true, mode: 448 });
  if ((0, import_node_fs5.existsSync)(path2)) return readJson(path2);
  const tempPath = (0, import_node_path5.join)(
    (0, import_node_path5.dirname)(path2),
    `.${(0, import_node_path5.basename)(path2)}.${process.pid}.${(0, import_node_crypto5.randomBytes)(8).toString("hex")}.tmp`
  );
  try {
    (0, import_node_fs5.writeFileSync)(tempPath, `${JSON.stringify(value, null, 2)}
`, { flag: "wx", mode: 384 });
    try {
      (0, import_node_fs5.linkSync)(tempPath, path2);
      return value;
    } catch (error) {
      if (error.code !== "EEXIST") throw error;
    ...
    try {
      (0, import_node_fs5.unlinkSync)(tempPath);
    } catch (error) {
      if (error.code !== "ENOENT") throw error;
    }
  }
...
function initializeGitRunWorkspaceIsolation(input) {
  ...
  const isolationRoot = (0, import_node_path5.join)(runDir, ".internal", "workspace-isolation");
  const worktreesRoot = (0, import_node_path5.join)(isolationRoot, "worktrees");
  const statePath = (0, import_node_path5.join)(isolationRoot, "run-workspace.json");
  const refPrefix = runRefPrefix(input.runId);
  (0, import_node_fs5.mkdirSync)(worktreesRoot, { recursive: true, mode: 448 });
  return withExclusiveFileClaim((0, import_node_path5.join)(isolationRoot, "claims", "initialize.claim"), () => {
    if ((0, import_node_fs5.existsSync)(statePath)) {
...
function nodeRecordPath(runWorkspace, attemptId) {
  return (0, import_node_path5.join)(runWorkspace.isolationRoot, "nodes", `${digest(attemptId, 32)}.json`);
}
function assertNodeWorkspaceRecord(runWorkspace, node, expected) {
  const nodeKey = digest(expected.attemptId, 32);
  const expectedWorktreeRoot = (0, import_node_path5.join)(runWorkspace.worktreesRoot, nodeKey);
  const expectedWorkspacePath = runWorkspace.relativeWorkspacePath === "." ? expectedWorktreeRoot : (0, import_node_path5.join)(expectedWorktreeRoot, runWorkspace.relativeWorkspacePath);
...
function assertNodeWorkspace(runWorkspace, value, expected) {
  const node = assertNodeWorkspaceRecord(runWorkspace, value, expected);
  if (!(0, import_node_fs5.existsSync)(node.worktreeRoot)) {
  ...
  if (currentAttemptRef !== node.baseCommit) {
    ...
    if (!(0, import_node_fs5.existsSync)(resultArtifact)) {
      throw new WorkspaceIsolationError(
  ...
  (0, import_node_fs5.mkdirSync)(node.workspacePath, { recursive: true, mode: 448 });
  return node;
}
function readGitNodeWorkspace(input) {
  ...
  if (!(0, import_node_fs5.existsSync)(recordPath)) {
...
function reconcileNodeAttemptRef(runWorkspace, node, result) {
...
function finalizeGitNodeWorkspace(input) {
  ...
  const resultClaim = (0, import_node_path5.join)(
    runWorkspace.isolationRoot,
    "claims",
    "results",
  ...
  return withExclusiveFileClaim(resultClaim, () => {
    if ((0, import_node_fs5.existsSync)(artifactPath)) {
      const persisted2 = assertNodeResult(
        runWorkspace,
        node,
        readJson(artifactPath),
        artifactPath
      );
      reconcileNodeAttemptRef(runWorkspace, node, persisted2);
      return persisted2;
    }
...
function readNodeResultForIntegration(input) {
  ...
  if (!(0, import_node_fs5.existsSync)(resultPath)) {
...
function reconcilePendingIntegrationReceipts(runWorkspace) {
  const receiptsRoot = (0, import_node_path5.join)(runWorkspace.isolationRoot, "receipts", "integrations");
  if (!(0, import_node_fs5.existsSync)(receiptsRoot)) return;
  const receiptPaths = (0, import_node_fs5.readdirSync)(receiptsRoot).filter((name) => name.endsWith(".json")).sort().map((name) => (0, import_node_path5.join)(receiptsRoot, name));
...
function readGitNodeIntegrationResult(input) {
  ...
  return withExclusiveFileClaim(
    (0, import_node_path5.join)(runWorkspace.isolationRoot, "claims", "integration.claim"),
    () => {
      reconcilePendingIntegrationReceipts(runWorkspace);
      if (!(0, import_node_fs5.existsSync)(artifactPath)) return void 0;
      const { result } = readNodeResultForIntegration(input);
      const current = requiredRef(runWorkspace.sourceRepositoryRoot, runWorkspace.integrationRef);
      const integration = assertIntegrationGitEvidence({
        runWorkspace,
        result,
        integration: readJson(artifactPath),
        artifactPath,
        currentIntegrationCommit: current
      });
...
function integrateGitNodeWorkspaceResult(input) {
  ...
  const nodeClaim = (0, import_node_path5.join)(
    runWorkspace.isolationRoot,
    "claims",
    "integrations",
  ...
  return withExclusiveFileClaim(
    ...
    () => withExclusiveFileClaim(
      ...
      () => {
        ...
        if ((0, import_node_fs5.existsSync)(artifactPath)) {
          const persisted2 = assertIntegrationGitEvidence({
            runWorkspace,
            result,
            integration: readJson(artifactPath),
            artifactPath,
          ...
          reconcileIntegrationRef(runWorkspace, persisted2);
          return persisted2;
        }
...
function currentGitRunIntegrationCommitFromRunDir(input) {
  const runWorkspace = readGitRunWorkspaceIsolationFromRunDir(input);
  return runWorkspace ? requiredRef(runWorkspace.sourceRepositoryRoot, runWorkspace.integrationRef) : void 0;
}
function readGitRunWorkspaceIsolationFromRunDir(input) {
  const runDir = requireAbsoluteDirectory2(input.runDir, "runDir");
  const statePath = (0, import_node_path5.join)(runDir, ".internal", "workspace-isolation", "run-workspace.json");
  if (!(0, import_node_fs5.existsSync)(statePath)) return void 0;
...
function removeIntegratedGitNodeWorkspace(input) {
  ...
  const existed = (0, import_node_fs5.existsSync)(node.worktreeRoot);
  if (existed) {
...
function removePristineGitNodeWorkspace(input) {
  ...
  const recordPath = nodeRecordPath(runWorkspace, input.attemptId);
  if (!(0, import_node_fs5.existsSync)(recordPath)) return "already-removed";
  ...
  if (!(0, import_node_fs5.existsSync)(candidate.worktreeRoot)) {
...
function publicationArtifactPath(runWorkspace) {
  return (0, import_node_path5.join)(runWorkspace.isolationRoot, "receipts", "publication.json");
}
function publicationConflictArtifactPath(runWorkspace, sourceSnapshot) {
...
function assertPublicationResult(runWorkspace, baseline, integrationCommit, publication, artifactPath) {
  const validSourceSnapshot = (snapshot) => Boolean(
    snapshot && snapshot.version === 1 && snapshot.kind === "git" && snapshot.sourceWorkspacePath === runWorkspace.sourceWorkspacePath && snapshot.repositoryRoot === runWorkspace.sourceRepositoryRoot && snapshot.gitCommonDir === runWorkspace.gitCommonDir && snapshot.relativeWorkspacePath === runWorkspace.relativeWorkspacePath && typeof snapshot.sourceIndexSha256 === "string" && /^[a-f0-9]{64}$/.test(snapshot.sourceIndexSha256)
  );
  if (!publication || publication.version !== WORKSPACE_ISOLATION_VERSION || publication.kind !== "git-run-workspace-publication" || publication.runId !== runWorkspace.runId || publication.baselineCommit !== runWorkspace.baselineCommit || publication.integrationCommit !== integrationCommit || publication.artifactPath !== artifactPath || typeof publication.publishedAt !== "string" || !Number.isFinite(Date.parse(publication.publishedAt)) || !validSourceSnapshot(publication.sourceSnapshot)) {
  ...
  const validRollback = !publication.rollback || ["reverted", "not-needed", "failed"].includes(publication.rollback.status) && typeof publication.rollback.attemptedAt === "string" && Number.isFinite(Date.parse(publication.rollback.attemptedAt)) && validRollbackPaths(publication.rollback.revertedPaths) && validRollbackPaths(publication.rollback.preservedPaths) && !(publication.rollback.revertedPaths || []).some(
    (path2) => (publication.rollback?.preservedPaths || []).includes(path2)
  ) && (publication.rollback.status === "failed" ? typeof publication.rollback.error === "string" && publication.rollback.error.length > 0 : publication.rollback.error === void 0);
  if (!validRollback) {
  ...
  const validStatus = (() => {
    switch (publication.status) {
      case "no-changes":
        return artifactPath === publicationArtifactPath(runWorkspace) && integrationTree === runWorkspace.baselineTree && !publication.publishedSnapshot && !publication.rollback;
      case "source-changed":
        return artifactPath === publicationConflictArtifactPath(
          runWorkspace,
          publication.sourceSnapshot
        ) && !publication.publishedSnapshot && (publication.rollback?.status === "not-needed" ? sourceStillAtBaseline(baseline, publication.sourceSnapshot) : !sourceStillAtBaseline(baseline, publication.sourceSnapshot));
      case "already-published":
        return artifactPath === publicationArtifactPath(runWorkspace) && !publication.rollback && validSourceSnapshot(publication.publishedSnapshot) && sourceMatchesIntegrationAfterApply(
          baseline,
          publication.sourceSnapshot,
          integrationTree
      ...
      case "published":
        return artifactPath === publicationArtifactPath(runWorkspace) && !publication.rollback && validSourceSnapshot(publication.publishedSnapshot) && sourceStillAtBaseline(baseline, publication.sourceSnapshot) && sourceMatchesIntegrationAfterApply(
          baseline,
          publication.publishedSnapshot,
          integrationTree
        );
...
function validRollbackPaths(paths) {
  ...
  return Array.isArray(paths) && paths.every((path2) => typeof path2 === "string" && path2.length > 0 && !path2.includes("\0")) && new Set(paths).size === paths.length && JSON.stringify(paths) === JSON.stringify([...paths].sort());
}
function sourceStillAtBaseline(baseline, observed) {
  return observed.gitCommonDir === baseline.gitCommonDir && observed.relativeWorkspacePath === baseline.relativeWorkspacePath && observed.sourceHead === baseline.sourceHead && observed.sourceBranch === baseline.sourceBranch && (baseline.sourceIndexSha256 === void 0 || observed.sourceIndexSha256 === baseline.sourceIndexSha256) && observed.snapshotTree === baseline.snapshotTree;
}
function sourceMatchesIntegrationAfterApply(baseline, observed, integrationTree) {
  return observed.sourceHead === baseline.sourceHead && observed.sourceBranch === baseline.sourceBranch && (baseline.sourceIndexSha256 === void 0 || observed.sourceIndexSha256 === baseline.sourceIndexSha256) && observed.snapshotTree === integrationTree;
}
function rollbackUnracedPublicationPaths(input) {
  ...
  for (const path2 of paths) {
    ...
    if (observed.status === 1) {
      preservedPaths.push(path2);
      continue;
    }
    if (observed.status !== 0) {
      const detail = observed.stderr.toString("utf8").trim();
      preservedPaths.push(path2);
      errors.push(`${path2}: could not compare source with integration${detail ? `: ${detail}` : ""}`);
      continue;
    }
    ...
    if (pathPatch.status !== 0) {
      ...
      preservedPaths.push(path2);
      errors.push(`${path2}: could not build inverse patch${detail ? `: ${detail}` : ""}`);
      continue;
    }
    try {
      ...
      revertedPaths.push(path2);
    } catch (error) {
      preservedPaths.push(path2);
      errors.push(`${path2}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
...
function publishGitRunWorkspaceResult(input) {
  ...
  const publicationClaim = (0, import_node_path5.join)(
  ...
  return withExclusiveFileClaim(publicationClaim, () => {
        ...
        if (!sourceMatchesIntegrationAfterApply(input.baseline, verified, integrationTree)) {
          if (sourceStillAtBaseline(input.baseline, verified)) {
            ...
            try {
              ...
              const fullyReverted = pathRollback.preservedPaths.length === 0 && pathRollback.errors.length === 0;
              rollback = {
                ...
                ...fullyReverted ? {} : {
                  error: [
                    rollbackMessage,
                    ...pathRollback.preservedPaths.length > 0 ? [`preserved externally changed paths: ${pathRollback.preservedPaths.join(", ")}`] : [],
                    ...pathRollback.errors
                  ].join("; ")
                },
    ...
    const receiptPath = status === "source-changed" ? publicationConflictArtifactPath(runWorkspace, sourceSnapshot) : artifactPath;
    const candidate = {
      version: WORKSPACE_ISOLATION_VERSION,
    ...
    const persisted = (0, import_node_fs5.existsSync)(receiptPath) ? readJson(receiptPath) : writeJsonOnce(receiptPath, candidate);
    return assertPublicationResult(
      runWorkspace,
      input.baseline,
      integrationCommit,
...
var import_node_child_process2, import_node_crypto5, import_node_fs5, import_node_path5, WORKSPACE_ISOLATION_VERSION, GIT_TIMEOUT_MS2, GIT_MAX_BUFFER2, REF_ROOT, WorkspaceIsolationError;
...
// lib/runner-v2/process-session.ts
function validSessionId(value) {
  return Number.isSafeInteger(value) && value > 0;
}
function processSessionIsQuiescent(sessionId) {
  if (!validSessionId(sessionId)) return false;
  const result = (0, import_node_child_process3.spawnSync)("ps", ["-eo", "sess=,stat="], {
    encoding: "utf8",
...
function waitForProcessSessionQuiescence(input) {
...
var import_node_child_process3, DEFAULT_PROBE_TIMEOUT_MS, DEFAULT_QUIESCE_TIMEOUT_MS, DEFAULT_POLL_MS;
var init_process_session = __esm({
  "lib/runner-v2/process-session.ts"() {
    "use strict";
    import_node_child_process3 = require("node:child_process");
    DEFAULT_PROBE_TIMEOUT_MS = 2e3;
    DEFAULT_QUIESCE_TIMEOUT_MS = 1e4;
    DEFAULT_POLL_MS = 50;
  }
});
...
function applyEffect(effect, context) {
  ...
  if (effect.type === "event-side-effects") {
    ...
    completeFanGroupMemberLocked(context.stateDir, {
      ...
      const runDir = (0, import_path14.dirname)(context.runJsonPath);
...
function routedCliArgsForTargets(launch, configuredTargets, targetsToStart) {
  ...
  if (targetOffset < 0 || stableSerialize3(launch.cli.args.slice(targetOffset)) !== stableSerialize3(configuredTargets)) {
...
function durableLaunchReceipt(runJsonPath, targetAgentIds, env, options = {}) {
  ...
  if (options.acceptanceKey && occurrenceId) {
    ...
    if (job && routedLaunchJobIsAccepted(job) && job.occurrenceId === occurrenceId && job.runId === runId && targetAgentIds.every((agentId) => jobTargets.has(agentId))) {
      const targets2 = targetAgentIds.map((agentId) => {
        const attempt = [...attempts].reverse().find((candidate) => candidate.runId === runId && candidate.agentId === agentId && candidate.launchJobId === job.id);
  ...
  for (const agentId of targetAgentIds) {
    ...
    const ownedAdmissionAccepted = Boolean(options.acceptanceKey) && attempt.launchJobId === options.acceptanceKey && admissionAccepted;
    if (!recordedAccepted && !ownedAdmissionAccepted && !runningAccepted && !blockedAccepted && !newlyTerminalAccepted) return void 0;
...
function attemptFingerprint(attempt) {
  return (0, import_crypto5.createHash)("sha256").update(stableSerialize3(attempt)).digest("hex");
}
...
function readRoutedLaunchAcceptance(run, key) {
  ...
  const parsed = record;
  if (!Array.isArray(parsed.targets) || !parsed.targets.every((target) => target && typeof target === "object" && typeof target.agentId === "string" && (typeof target.jobId === "string" || typeof target.attemptId === "string" && typeof target.session === "string")) || typeof parsed.occurrenceId !== "string" || typeof parsed.runId !== "string") {
    return void 0;
  }
  return parsed;
}
function persistLaunchAcceptance(runJsonPath, key, env, receipt, attemptGuard) {
  ...
  updateRunJson(runJsonPath, (current) => {
    ...
    if (existing[key]) {
      ...
      for (const target of targets) {
        ...
        if (prior && prior.jobId !== target.jobId && stableSerialize3(prior) !== stableSerialize3(target)) {
          throw new Error(`conflicting routed launch acceptance receipt: ${key}`);
        }
      ...
      if (stableSerialize3(actual.targets) === stableSerialize3(mergedTargets)) return current;
      return {
        ...current,
        runnerV2: {
          ...runnerV2,
...
function inProgressLaunchTarget(runJsonPath, targetAgentIds, env, launchJobId) {
  if (targetAgentIds.length === 0) return void 0;
  let run;
  try {
    run = readRunJson(runJsonPath);
  ...
  return targetAgentIds.find((agentId) => {
    const attempt = [...attempts].reverse().find((candidate) => candidate.runId === runId && candidate.agentId === agentId);
    return attempt && (!launchJobId || attempt.launchJobId === launchJobId) && !TERMINAL_ATTEMPT_PHASES.has(attempt.phase);
  });
}
...
function bindCompletionOperationIdentity(operation, context) {
  ...
  const digest2 = (0, import_crypto5.createHash)("sha256").update(stableSerialize3({ occurrenceId, operation: payload })).digest("hex").slice(0, 32);
  return {
    ...operation,
    occurrenceId,
...
function assertAdapterAttemptCurrent(context) {
...
function circuitStatePath(dir, chainName, agentId) {
  ...
  const identityDigest = (0, import_crypto5.createHash)("sha256").update(stableSerialize3({ chainName, agentId })).digest("hex").slice(0, 24);
...
function killAgentSessions(sessionName, options = {}, dependencyOverrides = {}) {
  ...
  for (const name of [`monitor-${sessionName}`, sessionName]) {
    try {
      ...
      const processSessionQuiescent = name !== sessionName || options.processSessionId === void 0 || dependencies.waitForProcessSession({ sessionId: options.processSessionId });
...
function nextChainOperationKey(parentRunId, identity) {
  return (0, import_crypto5.createHash)("sha256").update(stableSerialize3({ parentRunId, path: identity.path, name: identity.name, chainId: identity.chainId })).digest("hex").slice(0, 24);
}
...
function externalEffectOperationId(operation, context) {
  ...
  const digest2 = (0, import_crypto5.createHash)("sha256").update(stableSerialize3(operation)).digest("hex").slice(0, 24);
...
var TERMINAL_COMPLETION_REASONS;
...
function completionAttemptGuard(input) {
...
function recordStaleCompletionAttempt(input, match, completionEvidence) {
...
function completeAgent(input) {
  ...
  const evidence = (reason) => ({ actor: "system", reason });
  const setAgentStatus = (status, statusReason) => updateRunAgent(
    input.runJsonPath,
    input.agent.id,
    status,
    input.now,
  ...
  const setRunStatus = (status, statusMessage, statusReason) => updateRunStatus(
    input.runJsonPath,
    status,
    statusMessage,
    input.now,
    ...
    statusReason ?? (statusMessage ? evidence(statusMessage) : void 0)
  );
  if (input.generation?.jobId && input.generation.generationKind && input.generation.importablePayload === false && (match.matched || input.completionRecoveryEvidence)) {
    ...
    const run2 = setRunStatus("failed", reason);
    markAgentAttemptFailedNoCompletion({
      runJsonPath: input.runJsonPath,
      runId: input.runId,
      agentId: input.agent.id,
  ...
  if (!match.matched || !match.event) {
    if (input.generation?.jobId && input.generation.generationKind && input.generation.importablePayload) {
      ...
      const run2 = setRunStatus("completed", void 0, evidence(generationDetail));
      markAgentAttemptCompletedFromGeneration({
        runJsonPath: input.runJsonPath,
        runId: input.runId,
        agentId: input.agent.id,
    ...
    if (input.decision) {
      const decisionDetail = `no matching completion event; decision ${input.decision.phase} artifact accepted`;
      setAgentStatus("complete", evidence(decisionDetail));
      const run2 = setRunStatus("completed", void 0, evidence(decisionDetail));
  ...
  if (!match.matched || !match.event) {
    ...
    if (input.completionRecoveryEvidence === "durable-marker" && expectedEvent) {
      ...
      setAgentStatus("failed", evidence(reason));
      const run3 = setRunStatus("failed", reason);
      markAgentAttemptFailedNoCompletion({
        runJsonPath: input.runJsonPath,
        runId: input.runId,
        agentId: input.agent.id,
    ...
    if (shouldCompleteEmptyEmitsAgent(input.agent.emits, hasDownstreamForAgent(input.chain, input.agent.id))) {
      ...
      const run3 = setRunStatus("completed", void 0, evidence(emptyEmitsDetail));
    ...
    if (input.retry) {
      ...
      const run3 = setRunStatus("stopped", exhaustedDetail);
    ...
    const run2 = setRunStatus("failed", noEventDetail);
    markAgentAttemptFailedNoCompletion({
      runJsonPath: input.runJsonPath,
      runId: input.runId,
      agentId: input.agent.id,
  ...
  if (loopGuard?.action === "complete") {
    const run2 = setRunStatus("completed", void 0, evidence(`loop guard completed the chain: ${loopGuard.reason} (${loopGuard.visitKey})`));
    return {
  ...
  if (loopGuard?.action === "stop") {
    const run2 = setRunStatus("stopped", `max rounds exceeded (${loopGuard.maxRounds})`);
    return {
...
function synthesizeCompletionEventFromHandoff(input) {
  if (!input.agent.emits) return null;
  const runDir = (0, import_node_path6.dirname)(input.runJsonPath);
  const artifactsDir = (0, import_node_path6.join)(runDir, "artifacts");
  const candidates = [
    (0, import_node_path6.join)(artifactsDir, `${input.agent.id}-summary.json`),
    (0, import_node_path6.join)(artifactsDir, `${input.agent.id}-summary.md`)
  ];
  ...
  const artifactPath = candidates.find((candidate) => {
    try {
      if (!(0, import_node_fs6.existsSync)(candidate)) return false;
      const stat = (0, import_node_fs6.statSync)(candidate);
...
function attemptStartTimeMs(runJsonPath, runId, agentId, attemptId) {
  let attempts;
  try {
    attempts = readRunnerV2AttemptState(runJsonPath).attempts.filter((attempt2) => attempt2.runId === runId && attempt2.agentId === agentId);
  } catch {
    return null;
  }
  if (attempts.length === 0) return null;
  const attempt = attemptId ? attempts.find((candidate) => candidate.id === attemptId) : attempts[attempts.length - 1];
...
function planCompletionEventSideEffects(triggeredEvent, allEvents, allAgentIds, owner) {
  ...
  const acceptedTrigger = triggeredPath ? captureRunnerEventAcceptedTrigger({
    eventsDir: (0, import_node_path7.dirname)(triggeredPath),
    file: triggeredPath,
...
function bindAcceptedEventOccurrence(base, occurrenceToken) {
  const digest2 = (0, import_node_crypto6.createHash)("sha256").update(base || "runner-v2-completion").update("\0").update(occurrenceToken).digest("hex").slice(0, 32);
  return `runner-v2-event-occurrence:${digest2}:v1`;
}
function stableFanGroupId(input) {
  const digest2 = (0, import_node_crypto6.createHash)("sha256").update(input.runId || "unknown-run").update("\0").update(input.event).update("\0").update(input.occurrenceId || "unknown-occurrence").digest("hex").slice(0, 32);
...
var import_node_crypto6, AGENT_COMPLETE_ACTIONS;
...
function monitorStatePaths(session, dir = MONITOR_STATE_DIR) {
  const base = (0, import_node_path8.join)(dir, session);
  return {
    state: `${base}_state`,
    stale: `${base}_stale`,
    nudges: `${base}_nudges`,
...
function captureHash(capture, lines = 20) {
  ...
  return (0, import_node_crypto7.createHash)("md5").update(tail).digest("hex");
}
var import_node_crypto7, import_node_os, import_node_path8, MONITOR_STATE_DIR, CROSS_RUN_ADOPTION_FRESHNESS_MS;
var init_monitor_io = __esm({
  "lib/runner-v2/monitor-io.ts"() {
    ...
    init_completion();
    init_events();
    MONITOR_STATE_DIR = (0, import_node_path8.join)((0, import_node_os.homedir)(), ".mentiko_monitor");
...
function resolveInternalAuthSecret(context) {
  ...
  return (0, import_node_crypto8.createHmac)("sha256", root).update(`mentiko-internal-api:${context}`, "utf8").digest("hex");
}
var import_node_crypto8, DERIVED_INTERNAL_AUTH_CONTEXTS;
...
function readRunScopedDecisionImportToken(runDir) {
  try {
    return (0, import_node_fs7.readFileSync)((0, import_node_path9.join)(runDir, ".internal", "decision-import-token"), "utf8").trim();
  } catch {
    return "";
  }
}
...
var import_node_fs7, import_node_path9, inFlightDecisionNudges;
...
function cleanupId(mode, attemptId) {
  return (0, import_node_crypto9.createHash)("sha256").update(`${mode}\0${attemptId}`).digest("hex");
}
...
function writeJsonOnce2(path2, value) {
  (0, import_node_fs8.mkdirSync)((0, import_node_path10.dirname)(path2), { recursive: true, mode: 448 });
  ...
  const temporary = (0, import_node_path10.join)(
    ...
    `.${(0, import_node_path10.basename)(path2)}.${process.pid}.${(0, import_node_crypto9.randomBytes)(8).toString("hex")}.tmp`
  );
  try {
...
function assertJob(value, runWorkspace, path2) {
  const job = value;
  ...
  if (!value || typeof value !== "object" || job.version !== CLEANUP_VERSION || job.kind !== "git-node-workspace-cleanup" || typeof job.id !== "string" || job.id !== expectedId || job.runId !== runWorkspace.runId || typeof job.agentId !== "string" || job.agentId.length === 0 || !attemptId || !mode || job.pendingPath !== path2 || path2 !== expectedPendingPath || typeof job.requestedAt !== "string" || !Number.isFinite(Date.parse(job.requestedAt))) {
    throw new Error(`invalid workspace cleanup job: ${path2}`);
  }
  return job;
}
function assertReceipt(value, job, path2) {
  const receipt = value;
...
function executeCleanupJob(input) {
  ...
  return withExclusiveFileClaim(claim, () => {
    if ((0, import_node_fs8.existsSync)(completed)) {
      const receipt2 = assertReceipt(readJson2(completed), input.job, completed);
    ...
    const receipt = assertReceipt(writeJsonOnce2(completed, candidate), input.job, completed);
    clearPending(input.job.pendingPath);
    return receipt;
  }, { waitTimeoutMs: 3e4 });
}
function cleanupGitNodeWorkspaceDurably(input) {
  ...
  const job = assertJob(writeJsonOnce2(path2, candidate), input.runWorkspace, path2);
...
var import_node_crypto9, import_node_fs8, import_node_path10, CLEANUP_VERSION;
...
var init_runner_control_env = __esm({
  "lib/runner-control-env.ts"() {
    "use strict";
...
function runRunnerV2CompletionEntrypoint(input) {
  ...
  if (duplicate) {
    ...
    if (!input.dryRun) {
      ...
      integration = completionAttempt ? integrateCompletedNodeWorkspace({
        run,
        runId,
        runDir,
        agentId: agent.id,
    ...
    if (!input.dryRun && generationDuplicate) {
      ...
      const attemptGuard = completionAttempt && completionAttemptCurrent ? completionAttemptGuard2(runId, agent.id, completionAttempt.id) : void 0;
      if (!completionAttempt || completionAttemptCurrent) {
        const completedReason = { actor: "system", reason: "completion entrypoint accepted the agent's completion handoff" };
        updateRunAgent(runJsonPath, agent.id, "complete", input.now, void 0, attemptGuard, completedReason);
        updateRunStatus(runJsonPath, "completed", void 0, input.now, void 0, attemptGuard, completedReason);
      }
    }
  ...
  try {
    ...
    const attemptGuard = completionAttemptGuard2(runId, agent.id, completionAttempt.id);
    ...
    const pipeline = runCompletionPipeline({
      ...
      onLoopMutation,
      retry: {
        policy: objectValue3(agent.retry) || objectValue3(chain.config?.retry),
    ...
    if (!input.dryRun && error instanceof GenerationImportError) {
      ...
      const recoveredGuard = completionAttemptGuard2(runId, agent.id, recoveredAttempt.id);
...
function removeAgentSessionsAndReleaseCapacity(input) {
  ...
  const competingAttempt = attempts.find((attempt) => attempt.id !== input.attemptId && attempt.runId === input.runId && !isTerminalAgentAttemptPhase(attempt.phase) && (attempt.leaseId === input.sessionName || attempt.processEvidence?.ptySessionId === input.sessionName));
  if (competingAttempt) {
    if (processSessionId === void 0 || waitForProcessSessionQuiescence({ sessionId: processSessionId })) {
...
function completionAttemptGuard2(runId, agentId, attemptId) {
...
function completionAttemptIsCurrent(input) {
  try {
...
function blockWorkspaceIntegrationConflict(input) {
  ...
  const attemptGuard = completionAttemptGuard2(input.runId, input.agentId, input.attemptId);
  const blockedReason = { actor: "system", reason: detail };
  updateRunAgent(input.runJsonPath, input.agentId, "blocked", input.now, void 0, attemptGuard, blockedReason);
  updateRunStatus(input.runJsonPath, "blocked", detail, input.now, void 0, attemptGuard, blockedReason);
  removeAgentSessionsAndReleaseCapacity({
    sessionName: input.sessionName,
    stateDir: input.stateDir,
    runId: input.runId,
...
function blockSourceWorkspaceChanged(input) {
  ...
  const detail = [
    ...
    ...input.publication.rollback?.status === "reverted" ? ["post-apply race detected; Mentiko result patch was reverted"] : input.publication.rollback?.status === "not-needed" ? ["post-apply race detected; source returned to the task-start snapshot before rollback"] : input.publication.rollback?.status === "failed" ? [
...
function maybeTriggerDecisionImportOnCompletion(input) {
  ...
  const metadata = objectValue3(input.run.metadata);
...
function maybeHandleQualityGateFailure(input) {
  ...
  if (!input.dryRun) {
    const failureReason = { actor: "system", reason: result.reason };
    updateRunAgent(
      input.runJsonPath,
      input.agent.id,
      "failed",
...
function generationImportPlan(run, runDir, agentId, env) {
  const metadata = objectValue3(run.metadata);
...
function resolveDecisionIdentity(run) {
  const metadata = objectValue3(run.metadata);
  ...
  if (!decisionId || !phase) return void 0;
  return { decisionId, phase };
}
function decisionCompletionPlan(run, runDir, agentId) {
  const identity = resolveDecisionIdentity(run);
  if (!identity) return void 0;
  const artifactPath = (0, import_path16.join)(runDir, "artifacts", "decision-result.json");
  if (!(0, import_fs16.existsSync)(artifactPath)) return void 0;
  const notBefore = generationArtifactNotBeforeMs(run, agentId);
  try {
    if (notBefore !== void 0 && (0, import_fs16.statSync)(artifactPath).mtimeMs < notBefore) return void 0;
    const payload = JSON.parse((0, import_fs16.readFileSync)(artifactPath, "utf8"));
...
function generationArtifactNotBeforeMs(run, agentId) {
  const runnerV2 = objectValue3(run.runnerV2);
  const attempts = Array.isArray(runnerV2?.attempts) ? runnerV2.attempts : [];
  const attemptStarts = attempts.flatMap((value) => {
    const attempt = objectValue3(value);
...
function completionAttemptOccurrenceId(input) {
  const digest2 = (0, import_crypto7.createHash)("sha256").update(stableSerialize4(input)).digest("hex").slice(0, 32);
  return `runner-v2-completion:${digest2}:v1`;
}
function stableSerialize4(value) {
  if (Array.isArray(value)) return `[${value.map(stableSerialize4).join(",")}]`;
  if (value && typeof value === "object") {
    const record = value;
    return `{${Object.keys(record).sort().map((key) => `${JSON.stringify(key)}:${stableSerialize4(record[key])}`).join(",")}}`;
  }
  return JSON.stringify(value) ?? "null";
}
...
function completionLivenessExtensionCount(run, agentId) {
  const runnerV2 = objectValue3(run.runnerV2);
  const liveness = objectValue3(runnerV2?.completionLiveness);
  const agent = objectValue3(liveness?.[agentId]);
...
function recordCompletionLivenessExtension(input) {
  return updateRunJson(input.runJsonPath, (current) => {
    ...
    const runnerV2 = objectValue3(current.runnerV2) || {};
    const liveness = objectValue3(runnerV2.completionLiveness) || {};
    const previous = objectValue3(liveness[input.agentId]);
    return {
      ...current,
      runnerV2: {
        ...runnerV2,
...
function readAgentSummaryArtifact(path2) {
  ...
  try {
    ...
    const summary = objectValue3(parsed);
...
function configuredSchedule(value) {
  ...
  return stringValue(objectValue3(value)?.cron);
}
...
var SHARED_TMP_DIR = (0, import_fs.realpathSync)("/tmp");
...
function isCompletionLaunchContextPath(path2) {
  ...
  return [REAL_TMP_DIR, SHARED_TMP_DIR].includes((0, import_path.dirname)(parent)) && /^mentiko-completion-context-[A-Za-z0-9_-]+$/.test(dirName);
}
