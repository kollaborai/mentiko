...
var import_fs10, StaleRunAgentAttemptError, TERMINAL_RUN_STATUSES, TERMINAL_AGENT_STATUSES;
...
function applyEffect(effect, context) {
  ...
  if (effect.type === "event-side-effects") {
    ...
    updateRunStatus(
      ...
      effect.status === "stopped" ? { actor: "system", reason: effect.reason } : void 0
    );
...
function completeAgent(input) {
  ...
  const setAgentStatus = (status) => updateRunAgent(
    input.runJsonPath,
    input.agent.id,
    status,
    input.now,
  ...
  const setRunStatus = (status, statusMessage) => updateRunStatus(
    input.runJsonPath,
    status,
    statusMessage,
    input.now,
  ...
  if (!match.matched || !match.event) {
    if (input.generation?.jobId && input.generation.generationKind && input.generation.importablePayload) {
      ...
      const run2 = setRunStatus("completed");
      markAgentAttemptCompletedFromGeneration({
        runJsonPath: input.runJsonPath,
        runId: input.runId,
        agentId: input.agent.id,
  ...
  if (!match.matched || !match.event) {
    ...
    if (shouldCompleteEmptyEmitsAgent(input.agent.emits, hasDownstreamForAgent(input.chain, input.agent.id))) {
      setAgentStatus("complete");
      const run3 = setRunStatus("completed");
  ...
  if (duplicate) {
    ...
    if (!input.dryRun && generationDuplicate) {
      ...
      if (!completionAttempt || completionAttemptCurrent) {
        updateRunAgent(runJsonPath, agent.id, "complete", input.now, void 0, attemptGuard);
        updateRunStatus(runJsonPath, "completed", void 0, input.now, void 0, attemptGuard);
      }
    }
  ...
  try {
    ...
    if (!input.dryRun && error instanceof GenerationImportError) {
      ...
      if (completionAttemptIsCurrent({
        ...
        updateRunAgent(runJsonPath, agent.id, "complete", input.now, void 0, recoveredGuard);
        updateRunStatus(runJsonPath, "failed", error.message, input.now, void 0, recoveredGuard);
      }
      removeAgentSessionsAndReleaseCapacity({
        sessionName: input.sessionName,
        stateDir,
...
function blockWorkspaceIntegrationConflict(input) {
  ...
  updateRunAgent(input.runJsonPath, input.agentId, "blocked", input.now, void 0, attemptGuard);
  updateRunStatus(input.runJsonPath, "blocked", detail, input.now, void 0, attemptGuard);
  removeAgentSessionsAndReleaseCapacity({
    sessionName: input.sessionName,
    stateDir: input.stateDir,
    runId: input.runId,
