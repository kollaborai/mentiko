...
var AGENT_RUNTIME_CONTRACT_FIELDS = [
  "prompt",
  "instructions",
  "role",
  "description",
...
function nestedText(value, depth = 0) {
  ...
  return Object.values(value).flatMap((item) => nestedText(item, depth + 1));
}
function normalizedRuntimeSegments(agent) {
  const source = AGENT_RUNTIME_CONTRACT_FIELDS.flatMap((field) => nestedText(agent[field])).map((value) => value.toLowerCase().replace(/[’']/g, "").replace(/[_-]+/g, " ").replace(/\s+/g, " ").trim()).filter(Boolean);
  return source.flatMap((value) => {
    const clauses = value.split(/[\n.;]+|,\s*(?:but|however|yet)\s+|\b(?:but|however|yet)\b/).map((part) => part.trim()).filter(Boolean);
    return [
      ...clauses,
...
function hardRequirement(segment) {
  return /\b(?:must|should|required?|requires?|verify|confirms?|checks?|ensures?|asserts?|expects?|waits?|until|remains?|stays?|equals?|matches?|contains?|holds?|stores?|populated|sets?|keeps?)\b/.test(segment) || /\b(?:has|have|needs?)\s+to\b/.test(segment);
}
function negatesOpenRequirement(segment) {
  return /\b(?:do not|dont|does not|doesnt|must not|should not|need not|never)\s+(?:(?:need|have)\s+to\s+)?(?:require|verify|confirm|check|ensure|expect|wait for|remain|stay|keep|be)\b.{0,100}\bopen\b/.test(segment) || /\b(?:not required|not require)\b.{0,100}\bopen\b/.test(segment);
}
function negatesAssigneeBinding(segment) {
  return /\b(?:do not|dont|does not|doesnt|must not|should not|need not|never)\s+(?:(?:need|have)\s+to\s+)?(?:require|contain|equal|match|hold|store|set|bind|use)\b.{0,100}\bchain(?: id| identifier| binding)?\b/.test(segment) || /\b(?:not required|not require)\b.{0,100}\bassignee\b/.test(segment);
}
function negatesTerminalRequirement(segment) {
  return /\b(?:do not|dont|does not|doesnt|must not|should not|need not|never)\s+(?:(?:need|have)\s+to\s+)?(?:require|verify|confirm|check|ensure|expect|wait for|be)\b.{0,120}\b(?:terminal|completed|complete|closed|reconciled|failed|stopped|cancelled)\b/.test(segment) || /\b(?:not required|not require)\b.{0,120}\b(?:terminal|completed|complete|closed|reconciled)\b/.test(segment) || /\b(?:run|task)\b.{0,40}\b(?:is|be)\s+not\s+(?:terminal|completed|complete|closed|reconciled)\b/.test(segment);
}
function requiresPreAdmissionTaskState(agent) {
  return normalizedRuntimeSegments(agent).some((segment) => {
    ...
    const openStateSignal = /\b(?:status|state)\b/.test(segment) || /\b(?:is|be|remain|stay|keep|equal|match)\w*\b.{0,35}\bopen\b/.test(segment);
    const requiresOpenTask = taskReference && hardRequirement(segment) && openStateSignal && lifecycleOpen && !negatesOpenRequirement(segment);
    const requiresAssigneeBinding = /\bassignee\b/.test(segment) && /\bchain(?: id| identifier| binding)?\b/.test(segment) && /\b(?:contains?|equals?|matches?|holds?|stores?|populated|set|binding|identifier|id)\b/.test(segment) && hardRequirement(segment) && !negatesAssigneeBinding(segment);
    return requiresOpenTask || requiresAssigneeBinding;
  });
}
function requiresOwnTerminalReconciliation(agent) {
  return normalizedRuntimeSegments(agent).some((segment) => {
    ...
    const linkedTaskRunState = /\btask\b/.test(segment) && /\b(?:last run status|last run id|task run scope|run outcome|run status)\b/.test(segment);
    const terminalState = /\b(?:terminal|completed|complete|closed|reconciled|failed|stopped|cancelled)\b/.test(segment);
    return terminalState && hardRequirement(segment) && (currentRun || linkedTask || linkedTaskRunState) && !negatesTerminalRequirement(segment);
  });
}
