...
// lib/runner-v2/completion-contract-cli.ts
var completion_contract_cli_exports = {};
__export(completion_contract_cli_exports, {
  runCompletionContractCli: () => runCompletionContractCli
});
module.exports = __toCommonJS(completion_contract_cli_exports);
...
function buildTypedCompletionContract(input) {
  ...
  const summaryJson = (0, import_node_path.join)(input.artifactsDir, `${agentId}-summary.json`);
  const summaryMarkdown = (0, import_node_path.join)(input.artifactsDir, `${agentId}-summary.md`);
  const emitCommand = emits ? `mentiko emit ${emits}` : "";
  const eventHandoff = emits ? input.coreGenerationChain ? ["Core generation handoff:", `- Write the authoritative generation payload to ${(0, import_node_path.join)(input.artifactsDir, "generation-result.json")}.`, "- Mentiko imports that file automatically when this run completes.", `- You may run "${emitCommand}" after writing the payload; the generation file remains authoritative.`] : ["Canonical event handoff:", "When completely finished, signal completion by running this command exactly:", `    ${emitCommand}`] : ["This agent has no declared completion event.", "Do not create or hand-write any .event file; the final completion marker is the only terminal signal."];
  return [
    "COMPLETION CONTRACT:",
...
// lib/runner-v2/completion-contract-cli.ts
var FLAGS = /* @__PURE__ */ new Set(["--agent-id", "--artifacts-dir", "--events-dir", "--run-id", "--emits", "--core-generation-chain"]);
function runCompletionContractCli(argv, write = (line) => console.log(line)) {
