...
function resolveJobRecordPaths(jobsDir, jobId2) {
  ...
  (0, import_node_fs.mkdirSync)(jobsDir, { recursive: true });
...
function getSecretByName(name) {
  if (!(0, import_node_fs2.existsSync)(SECRETS_DIR)) return null;
  for (const f of (0, import_node_fs2.readdirSync)(SECRETS_DIR).filter((x) => x.endsWith(".json"))) {
    try {
      const rec = JSON.parse((0, import_node_fs2.readFileSync)((0, import_node_path2.join)(SECRETS_DIR, f), "utf8"));
...
function resolveDefaultProfile() {
  ...
  let env = {};
  try {
    if (!(0, import_node_fs2.existsSync)(AGENT_PROFILES_DIR)) return { cli, cliArgs, env };
    const files = (0, import_node_fs2.readdirSync)(AGENT_PROFILES_DIR).filter((f) => f.endsWith(".json"));
    let defaultProfile = null;
    for (const file of files) {
      const profile = JSON.parse((0, import_node_fs2.readFileSync)((0, import_node_path2.join)(AGENT_PROFILES_DIR, file), "utf-8"));
...
var job;
try {
  const content = (0, import_node_fs2.readFileSync)(jobPath, "utf-8");
...
var resolvedProfile = resolveDefaultProfile();
...
function isDirectory(candidate) {
  ...
  try {
    return (0, import_node_fs2.existsSync)(candidate) && (0, import_node_fs2.statSync)(candidate).isDirectory();
  } catch {
    return false;
  }
}
function loadRegisteredWorkspaces() {
  ...
  if (!(0, import_node_fs2.existsSync)(workspacesPath)) return [];
  try {
    const workspaces = JSON.parse((0, import_node_fs2.readFileSync)(workspacesPath, "utf-8"));
