...
function resolveExactProfile(profilesDir, profileId, source = "explicit") {
  ...
  const path2 = (0, import_node_path.join)(profilesDir, `${profileId}.json`);
  if (!(0, import_node_fs.existsSync)(path2)) throw new Error(`Agent profile '${profileId}' does not exist at ${path2}`);
...
function resolveWorkspaceProfileId(projectRoot2, orgRoot2) {
  ...
  const workspacePath = (0, import_node_path.join)(orgRoot2, "workspaces.json");
  if (!(0, import_node_fs.existsSync)(workspacePath)) return void 0;
...
function runRunnerAgentProfileCli(argv, write = (line) => console.log(line)) {
  ...
  switch (command) {
    ...
    case "snapshot": {
      ...
      (0, import_node_fs2.writeFileSync)(required(values, "--output-path"), `${JSON.stringify({
