...
// Runtime context builder for automatic TASK_CONTEXT population
function buildRuntimeContext(input, chain, agent) {
  // Lightweight context builder when no task API is available
  const context = {
    RUN_ID: input?.runId || "",
    AGENT_ID: agent?.id || "",
    CHAIN_ID: chain?.id || "",
    ...
    CHAIN_OBJECTIVE: chain?.description || "",
    TIMESTAMP: new Date().toISOString(),
  };
  return JSON.stringify(context, null, 2);
}
function taskContextEnvironment(result, input, chain, agent) {
  // Enhanced taskContextEnvironment with fallbacks for missing task data
  if (!result || !result.task) {
    // Fallback to runtime-only context when task data unavailable
    const runtimeContext = buildRuntimeContext(input, chain, agent);
    return {
      TASK_ID: "",
      TASK_TITLE: "",
      TASK_DESCRIPTION: "",
