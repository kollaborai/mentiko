...
function emitRunnerEvent(input) {
  ...
  const eventsDir = input.eventsDir || config_default.eventsDir;
  ...
  (0, import_node_fs.mkdirSync)(eventsDir, { recursive: true });
...
function persistWithoutClobber(input) {
  const requestedPath = (0, import_node_path.join)(input.eventsDir, input.requestedFilename);
  try {
  ...
  for (let attempt = 0; ; attempt += 1) {
    ...
    const collisionPath = (0, import_node_path.join)(input.eventsDir, collisionFilename);
    try {
    ...
    const collisionContent = (0, import_node_fs.readFileSync)(collisionPath, "utf8");
