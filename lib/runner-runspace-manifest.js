...
// lib/runner-v2/runspace-manifest-cli.ts
var runspace_manifest_cli_exports = {};
__export(runspace_manifest_cli_exports, {
  runRunspaceManifestCli: () => runRunspaceManifestCli
});
module.exports = __toCommonJS(runspace_manifest_cli_exports);

// lib/runner-v2/runspace-manifest.ts
...
// lib/runner-v2/runspace-manifest.ts
function ensureRunspaceManifest(runsDir, runId, chain) {
  ...
  const manifestPath = (0, import_node_path2.join)(runspaceDir, "manifest.json");
  const expected = { run_id: canonicalRunId, chain, artifacts: [] };
  try {
    (0, import_node_fs2.writeFileSync)(manifestPath, `${JSON.stringify(expected)}
...
function containedDirectory(parent, child, label) {
  const path = (0, import_node_path2.join)(parent, child);
  if ((0, import_node_path2.relative)(parent, path).startsWith("..")) throw new Error(`${label} escapes configured root.`);
  if ((0, import_node_fs2.existsSync)(path)) {
    ...
    (0, import_node_fs2.mkdirSync)(path, { recursive: true, mode: 448 });
  }
  return (0, import_node_path2.resolve)(path);
}
function readExistingManifest(path, expected) {
  let value;
  try {
...
function isRunspaceManifest(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const record = value;
  return typeof record.run_id === "string" && typeof record.chain === "string" && Array.isArray(record.artifacts);
}
...
// lib/runner-v2/runspace-manifest-cli.ts
function runRunspaceManifestCli(argv, environment = process.env, write = (line) => console.log(line)) {
