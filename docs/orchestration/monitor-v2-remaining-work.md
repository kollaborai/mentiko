...
on a live TASK-093-shaped run.** `buildMonitorCommand` and routed launch scripts
invoke compiled `monitor-v2` directly. Completion and monitoring are typed and
fail-closed: missing context, a missing bundle, PTY spawn failure, unsupported
input, or missing child acceptance has no shell fallback. The remaining risk is
monitor parity breadth, not completion ownership.
...
- [x] **8. Wire typed monitor command** — typed bootstrap and routed launch scripts execute compiled `/context/lib/monitor-v2.js` with no selection flag or shell fallback.
- [x] **9. THE proof — live completion handoff behavior.** Proven with `node web/scripts/runner-v2-watched-proof.cjs /tmp/runner-v2-watched-monitor-v2-proof.json`, then re-proven with `MENTIKO_RUNNER_V2=1 MENTIKO_RUNNER_V2_COMPLETION=1 node web/scripts/runner-v2-watched-proof.cjs /tmp/runner-v2-watched-monitor-v2-default-on-proof.json`: typed plan launched, event latch fired, completion PTY spawned, attempt reached `completed_from_event`, run completed. The legacy/stuck-run safety net is now connected: reconcile invokes `recoverLateCompletionEvents` before terminal retry/audit handling and relaunches downstream through the typed executor plan.
...
- [x] typed monitor command has exact env and no shell alternate
...
- The monitor-v2 handoff proof, broader monitor e2e parity, typed fan-group accounting, and late-event recovery hookup have landed. `assessRunnerV2SwitchReadiness()` reports `ready`; runner-v2 monitor launch is typed-only. Legacy monitor cleanup remains a separately scoped migration.

### Shell completion fallback removal — done unconditionally
- Typed fan-group member accounting now exists: typed completion reads strict JSON fan-group membership, suppresses normal member routing, updates the JSON ledger under lock, and launches fan-in only from the claim winner. Legacy `.state` fan groups are rejected rather than parsed.
- Live proof: `MENTIKO_RUNNER_V2=1 MENTIKO_RUNNER_V2_COMPLETION=1 web/e2e/engine/engine-e2e-events.sh` produced run `run-1783437332756-3693ab01`; the legacy-oriented assertion script failed two stale assertions, but the typed artifact verifier passed against `state/fan-groups/dispatch-done-1783437361464.json` (`completed: 2`, members `worker_a`/`worker_b`, collector completed exactly once).
- Typed-only proof: `MENTIKO_RUNNER_V2=1 MENTIKO_RUNNER_V2_COMPLETION=1 web/e2e/engine/engine-e2e-events.sh` passed 24/24 and produced run `run-1783446649710-698fc703`; fan-group state `dispatch-done-1783446679178.json` completed both workers and launched the collector exactly once.
- Typed-only proof: `bash tests/bash/test-monitor-completion.sh` asserts no shell completion handoff after a typed completion PTY spawn failure. Live watched proof passed at `/tmp/runner-v2-watched-no-shell-completion-fallback-proof.json`; live fan-out/fan-in proof passed 24/24 with run `run-1783450309286-43e92f47` and fan-group state `dispatch-done-1783450338921.json`.
