...
The ledger separately reports live shell execution. `dataShapeShellSources()` combines a direct shell contract owner with any mapped lifecycle surface whose owner is explicitly `legacy-shell`; it excludes historical equivalents and invocation-only shell adapters. Current typed launch paths are the supported local `bin/mentiko run` CLI and scheduled background-worker/trigger-now launches. The direct legacy parallel path still runs `parallel-launcher.sh` → `chain-runner.sh`, and `parallel-coordinator.sh` delegates to that launcher; therefore `parallel-group-state` is shared and reports those three live shell paths until that orchestration is moved to TypeScript. `bin/mentiko graph` also invokes `chain-runner.sh --dry-run` as a diagnostic command. `lib/scheduler.sh` remains a sourceable compatibility boundary over the typed schedule contract; it is not the background scheduler daemon. Shell evidence in lineage therefore distinguishes typed contract ownership from the remaining executable orchestration path rather than flattening both into a misleading zero.
