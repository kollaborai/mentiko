...
No registered shape names a `.sh` file as a writer, reader, type source, or validator. `dataShapeShellSources()` returns empty for every entry, and the catalog test `has no documented data shape with a direct shell contract owner` fails the suite if that stops being true. That proves persisted-contract ownership only; it does not prove that every executable launch path is typed.

The catalog can report `Typed % = 100` for mapped persisted-shape surfaces while legacy executable orchestration remains. Current typed launch paths are the supported local `bin/mentiko run` CLI and scheduled background-worker/trigger-now launches. Current shell launch paths are the batch worker's per-chain `chain-runner.sh` invocation, typed adapter `next-chain` continuation, typed retry dispatch, and direct legacy `chain-runner.sh --parallel`. `bin/mentiko graph` also invokes `chain-runner.sh --dry-run` as a diagnostic command. `lib/scheduler.sh` remains a sourceable compatibility boundary over the typed schedule contract; it is not the background scheduler daemon. Shell evidence in lineage therefore means either a primitive adapter, an external product command, or one of these explicitly remaining legacy executable paths—not ownership of the persisted shape.
